...
private:
	...
	void						DropShard( shard_t *shard); //c4tnt: added
	void						Shatter( const idVec3 &point, const idVec3 &impulse, const int time );
	void						StartBroken( const float part, const float disperse); //c4tnt: added 
	void						DropFloatingIslands( const idVec3 &point, const idVec3 &impulse, const int time );
	void						DropFloatingIslands( const int* count ); //c4tnt: added
	void						Break( void );
