...
================
*/
void idEntityFx::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idEntityFx::Restore( idRestore_I *savefile ) {
	int i;
	int num;
	bool hasObject;
			...
			case FX_SHOCKWAVE: {
				...
				if ( !useAction->shakeStarted ) {
					...
					if ( !projectileDef ) {
						...
						ent->PostEventMS( &EV_Remove, NULL, ent->spawnArgs.GetInt( "duration" ) );
					}
				}
				break;
			}
...
void idEntityFx::Spawn( void ) {
	...
	if ( !spawnArgs.GetBool( "triggered" ) ) {
		...
		if ( spawnArgs.GetBool( "test" ) || spawnArgs.GetBool( "start" ) || spawnArgs.GetFloat ( "restart" ) ) {
			PostEventMS( &EV_Activate, NULL, 0, this );
		}
	}
}
...
void idEntityFx::Event_ClearFx( void ) {
	...
	if ( spawnArgs.GetBool("test") ) {
		...
		if ( spawnArgs.GetFloat( "restart" ) || !spawnArgs.GetBool( "triggered")) {
			...
			if ( rest == 0.0f ) {
				PostEventSec( &EV_Remove, NULL, 0.1f );
			} else {
				rest *= gameLocal.random.RandomFloat();
				PostEventSec( &EV_Activate, NULL, rest, this );
			}
		}
	}
}
...
void idEntityFx::Event_Trigger( idEntity *activator ) {
	...
	if ( spawnArgs.GetString( "fx", "", &fx) ) {
		...
		PostEventMS( &EV_Fx_KillFx, NULL, Duration() );
	...
	PostEventSec( &EV_Fx_Action, NULL, fxActionDelay, activator );
}
