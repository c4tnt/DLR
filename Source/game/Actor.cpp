...
=====================
*/
void idAnimState::Save( idSave_I *savefile ) const {
...
=====================
*/
void idAnimState::Restore( idRestore_I *savefile ) {
...
idActor::~idActor( void ) {
	...
	if ( head.GetEntity() ) {
		...
		head.GetEntity()->PostEventMS( &EV_Remove, NULL, 0 );
	}
...
void idActor::Spawn( void ) {
	...
	if (!spawnArgs.GetInt( "gib_health_treshold", "0", gib_health_lim )) {
		gib_health_lim = health;
	}
	gib_time_lim	= SEC2MS( spawnArgs.GetFloat( "gib_time_treshold", "0.2" ) );

	last_damage		= gameLocal.time;
...
void idActor::SetupHead( void ) {
	...
	if ( headModel[ 0 ] ) {
		...
		// copy slowmo param to the head (for d3xp)
...
================
*/
void idActor::Save( idSave_I *savefile ) const {
	idActor *ent;
	int i;
	...
	savefile->WriteInt( last_damage );
	savefile->WriteInt( gib_health_lim );
	savefile->WriteInt( gib_time_lim );

	savefile->WriteInt( damageGroups.Num() );
	for ( i = 0; i < damageGroups.Num(); i++ ) {
		savefile->WriteString( damageGroups[i].name );
		savefile->WriteFloat( damageGroups[i].damageScale );
		savefile->WriteFloat( damageGroups[i].damageCap );
	}

	savefile->WriteInt( damageIndex.Num() );
	for( i = 0; i < damageIndex.Num(); i++ ) {
		savefile->WriteInt( damageIndex[ i ] );
	}
...
//		savefile->WriteString( state->Name() );
	...
	if ( idealState ) {
		...
		savefile->WriteString( token );
//		savefile->WriteString( idealState->Name() );
	} else {
		savefile->WriteString( "" );
	}
...
================
*/
void idActor::Restore( idRestore_I *savefile ) {
	int i, num;
	idActor *ent;
	...
	savefile->ReadInt( last_damage );
	savefile->ReadInt( gib_health_lim );
	savefile->ReadInt( gib_time_lim );

	savefile->ReadInt( num );
	damageGroups.SetGranularity( 1 );
	damageGroups.SetNum( num );
	for( i = 0; i < num; i++ ) {
		savefile->ReadString( damageGroups[ i ].name );
		savefile->ReadFloat( damageGroups[ i ].damageScale );
		savefile->ReadFloat( damageGroups[ i ].damageCap );
	}

	savefile->ReadInt( num );
	damageIndex.SetNum( num );
	for( i = 0; i < num; i++ ) {
		savefile->ReadInt( damageIndex[ i ] );
	}
...
void idActor::ShutdownThreads( void ) {
	...
	if ( scriptThread ) {
		...
		scriptThread->PostEventMS( &EV_Remove, NULL, 0 );
		delete scriptThread;
		scriptThread = NULL;
	}
}
...
void idActor::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, 
	...
	// There are a lot of hardcode. And ROE too
	...
	damage = GetDamageForLocation( damage, health, location );
	...
	if ( damage > 0 ) {
		...
		//Check the health against any damage cap that is currently set ( d3xp ability )
		...
		if ( health <= 0 ) {
			...
			if ( last_damage < gameLocal.time ) {
				health = -damage;
			}
			last_damage = gameLocal.time + gib_time_lim;

			if ( !alreadyDead )
				Killed( inflictor, attacker, damage, dir, location );

			if ( ( health < -gib_health_lim ) && spawnArgs.GetBool( "gib" ) && damageDef->GetBool( "gib" ) ) {
...
bool idActor::Pain( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	painAnim = "";
	if ( animPrefix.Length() ) {
		if ( damageGroup.Length() ) {		// && ( damageGroup != "legs" ) c4tnt: allow the _legs pain anim
	...
	} else if ( damageGroup.Length() ) {	// && ( damageGroup != "legs" ) c4tnt: allow the _legs pain anim
		sprintf( painAnim, "pain_%s", damageGroup.c_str() );
		if ( !animator.HasAnim( painAnim ) ) {
			sprintf( painAnim, "pain_%s", damageGroup.c_str() );
			if ( !animator.HasAnim( painAnim ) ) {
...
void idActor::SetupDamageGroups( void ) {
	...
	int						jointnum;
	idStrList				tmpDGroups;
	int						GI;
	damageGroup_t			*dgPtr;
	
	// create damage zones
	damageIndex.SetNum( animator.NumJoints() );
	...
	while ( arg ) {
		...
		GI = tmpDGroups.FindIndex( groupname );
		
		if ( GI == -1 ) {
			GI = tmpDGroups.Append( groupname );
		}

		for( i = 0; i < jointList.Num(); i++ ) {
			jointnum = jointList[ i ];
			damageIndex[ jointnum ] = GI;
		}
		jointList.Clear();
	...
	// setup damage zones
	damageGroups.Clear();
	for( i = 0; i < tmpDGroups.Num(); i++ ) {
		dgPtr = &damageGroups.Alloc();
		dgPtr->name = tmpDGroups[i];
		dgPtr->damageScale = spawnArgs.GetFloat( va( "damage_scale %s" , dgPtr->name.c_str() ), "1" );
		dgPtr->damageCap = spawnArgs.GetFloat( va( "damage_cap %s" , dgPtr->name.c_str() ), "0" );
	}
...
=====================
*/
int idActor::GetDamageForLocation( int damage, int health, int location ) {
int idx;
float cap;

	if ( ( location < 0 ) || ( location >= damageIndex.Num() ) ) {
		return damage;
	}
	
	idx = damageIndex[ location ];

	if ( ( idx < 0 ) || ( idx >= damageGroups.Num() ) ) {
		return damage;
	}

	cap = damageGroups[ idx ].damageCap;
	if ( cap > 0 && health < cap ) return 0;

	return (int)ceil( damage * damageGroups[ idx ].damageScale );
}
...
int idx;
	...
	return damageGroups[ idx ].name.c_str();
}
...
/*
=====================
idActor::Event_PlayFootStepSound
=====================
*/
...
=====================
*/
void idActor::Event_GetPainAnim( idEventReturn *returnContext ) {
	if ( !painAnim.Length() ) {
		returnContext->FinallyString( "pain" );
	} else {
		returnContext->FinallyString( painAnim );
	}
...
===============
*/
void idActor::Event_PlayAnim( idEventReturn *returnContext, int channel, const char *animname ) {
	animFlags_t	flags;
	idEntity *headEnt;
	int	anim;
...
===============
*/
void idActor::Event_PlayCycle( idEventReturn *returnContext, int channel, const char *animname ) {
	animFlags_t	flags;
	int			anim;
...
===============
*/
void idActor::Event_IdleAnim( idEventReturn *returnContext, int channel, const char *animname ) {
	int anim;
...
===============
*/
void idActor::Event_GetBlendFrames( idEventReturn *returnContext, int channel ) {
	switch( channel ) {
	case ANIMCHANNEL_HEAD :
		returnContext->FinallyInt( headAnim.animBlendFrames );
		break;

	case ANIMCHANNEL_TORSO :
		returnContext->FinallyInt( torsoAnim.animBlendFrames );
		break;

	case ANIMCHANNEL_LEGS :
		returnContext->FinallyInt( legsAnim.animBlendFrames );
		break;
...
===============
*/
void idActor::Event_GetAnimState( idEventReturn *returnContext, int channel ) {
	const char *state;

	state = GetAnimState( channel );
	returnContext->FinallyString( state );
...
===============
*/
void idActor::Event_InAnimState( idEventReturn *returnContext, int channel, const char *statename ) {
	bool instate;

	instate = InAnimState( channel, statename );
	returnContext->FinallyInt( instate );
}
...
===============
*/
void idActor::Event_AnimDone( idEventReturn *returnContext, int channel, int blendFrames ) {
	bool result;
...
================
*/
void idActor::Event_HasAnim( idEventReturn *returnContext, int channel, const char *animname ) {
	if ( GetAnim( channel, animname ) != NULL ) {
		returnContext->FinallyFloat( 1.0f );
	} else {
...
================
*/
void idActor::Event_ChooseAnim( idEventReturn *returnContext, int channel, const char *animname ) {
	int anim;
	...
	if ( anim ) {
		if ( channel == ANIMCHANNEL_HEAD ) {
			if ( head.GetEntity() ) {
				returnContext->FinallyString( head.GetEntity()->GetAnimator()->AnimFullName( anim ) );
				return;
			}
		} else {
			returnContext->FinallyString( animator.AnimFullName( anim ) );
			return;
		}
	}
...
================
*/
void idActor::Event_AnimLength( idEventReturn *returnContext, int channel, const char *animname ) {
	int anim;
	...
	if ( anim ) {
		if ( channel == ANIMCHANNEL_HEAD ) {
			if ( head.GetEntity() ) {
				returnContext->FinallyFloat( MS2SEC( head.GetEntity()->GetAnimator()->AnimLength( anim ) ) );
				return;
			}
		} else {
			returnContext->FinallyFloat( MS2SEC( animator.AnimLength( anim ) ) );
			return;
		}		
	}
...
================
*/
void idActor::Event_AnimDistance( idEventReturn *returnContext, int channel, const char *animname ) {
	int anim;
	...
	if ( anim ) {
		if ( channel == ANIMCHANNEL_HEAD ) {
			if ( head.GetEntity() ) {
				returnContext->FinallyFloat( head.GetEntity()->GetAnimator()->TotalMovementDelta( anim ).Length() );
				return;
			}
		} else {
			returnContext->FinallyFloat( animator.TotalMovementDelta( anim ).Length() );
			return;
		}
	}
...
================
*/
void idActor::Event_HasEnemies( idEventReturn *returnContext ) {
	bool hasEnemy;

	hasEnemy = HasEnemies();
	returnContext->FinallyInt( hasEnemy );
}
...
================
*/
void idActor::Event_NextEnemy( idEventReturn *returnContext, idEntity *ent ) {
	idActor *actor;
...
================
*/
void idActor::Event_ClosestEnemyToPoint( idEventReturn *returnContext, const idVec3 &pos ) {
...
=====================
*/
void idActor::Event_GetState( idEventReturn *returnContext ) {
	if ( state ) {
		returnContext->FinallyString( state->Name() );
	} else {
		returnContext->FinallyString( "" );
	}
}
...
=====================
*/
void idActor::Event_GetHead( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( head.GetEntity() );
}


/*
================
idActor::Event_SetDamageGroupScale (ROE)
================
*/
void idActor::Event_SetDamageGroupScale( const char* groupName, float scale) {

	for( int i = 0; i < damageGroups.Num(); i++ ) {
		if ( damageGroups[ i ].name == groupName ) {
			damageGroups[ i ].damageScale = scale;
		}
	}
}
...
/*
================
idActor::Event_SetDamageGroupScaleAll (ROE)
================
*/
...
/*
================
idActor::Event_Event_GetDamageGroupScale (ROE)
================
*/
void idActor::Event_GetDamageGroupScale( idEventReturn *returnContext, const char* groupName ) {

	for( int i = 0; i < damageGroups.Num(); i++ ) {
		if ( damageGroups[ i ].name == groupName ) {
			returnContext->FinallyFloat(damageGroups[i].damageScale);
			return;
		}
	}
...
/*
================
idActor::Event_SetDamageCap (ROE)
================
*/
...
/*
================
idActor::Event_SetWaitState (ROE)
================
*/
...
/*
================
idActor::Event_GetWaitState (ROE)
================
*/

void idActor::Event_GetWaitState( idEventReturn *returnContext ) {
	if(WaitState()) {
		returnContext->FinallyString(WaitState());
	} else {
		returnContext->FinallyString("");
	}
}