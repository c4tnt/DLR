...
================
*/
void idSecurityCamera::Save( idSave_I *savefile ) const {
...
================
*/
void idSecurityCamera::Restore( idRestore_I *savefile ) {
...
void idSecurityCamera::Spawn( void ) {
	...
	if ( spawnArgs.GetBool( "spotLight" ) ) {
		PostEventMS( &EV_SecurityCam_AddLight, NULL, 0 );
	}
	...
	if ( thinkFlags & TH_THINK ) {
		if (CanSeePlayer()) {
			if (alertMode == SCANNING) {
				...
				PostEventSec(&EV_SecurityCam_Alert, NULL, sightTime);
			}
		} else {
			if (alertMode == ALERT) {
				float	sightResume;
				...
				PostEventSec( &EV_SecurityCam_ContinueSweep, NULL, sightResume );
			}
...
void idSecurityCamera::StartSweep( void ) {
	...
	sweepEnd = sweepStart + speed;
   	PostEventMS( &EV_SecurityCam_Pause, NULL, speed );
...
void idSecurityCamera::Event_ContinueSweep( void ) {
	...
	sweepEnd = sweepStart + speed;
   	PostEventMS( &EV_SecurityCam_Pause, NULL, speed * (1.0 - pct));
	StartSound( "snd_moving", SND_CHANNEL_BODY, 0, false, NULL );
	SetAlertMode(SCANNING);
	sweeping = true;
}
...
void idSecurityCamera::Event_Alert( void ) {
	...
	PostEventSec( &EV_SecurityCam_ContinueSweep, NULL, wait );
}
...
void idSecurityCamera::Event_Pause( void ) {
	...
	StartSound( "snd_stop", SND_CHANNEL_BODY, 0, false, NULL );
   	PostEventSec( &EV_SecurityCam_ReverseSweep, NULL, sweepWait );
}
