...
void idItem::Present( void ) {
	...
	if ( !fl.hidden && ( entFlags & EUF_PULSE ) ) {
...
void idItem::Spawn( void ) {
	...
	if ( spawnArgs.GetBool( "dropToFloor" ) ) {
		PostEventMS( &EV_DropToFloor, NULL, 0 );
	}
	...
	if ( giveTo.Length() ) {
		...
		entFlags |= EUF_ONSTEP | EUF_TRIGGED;	//Allow take item
		PostEventMS( &EV_Touch, NULL, 0, ent, NULL );
	}
	...
	if ( !spawnArgs.GetBool( "triggerFirst" ) ) {
		entFlags |= EUF_TRIGGED;
	}
	...
	if ( !spawnArgs.GetBool( "no_trigtake" ) ) {
		entFlags |= EUF_TRIGTAKE;
	}

	if ( spawnArgs.GetBool( "useable" ) ) {
		entFlags |= EUF_ONUSE;
	}
	entFlags |= EUF_ONUSE;

	if ( spawnArgs.GetBool( "tracetouch" ) ) {
		entFlags |= EUF_CHTRACE;
	}
...
void idItem::GetAttributes( idDict &attributes ) {
	...
	for( i = 0; i < spawnArgs.GetNumKeyVals(); i++ ) {
		...
		if ( arg->GetKey().Left( 4 ) == "inv_" ) {
			attributes.Set( arg->GetKey().c_str(4), arg->GetValue() );
		}
	}
}
...
void idItem::Event_Touch( idEntity *other, trace_t *trace ) {
	...
	if ( (entFlags & (EUF_TRIGGED|EUF_ONSTEP)) == (EUF_TRIGGED|EUF_ONSTEP) ) {
...
================
*/
void idItem::Event_Use( idEntity *other, trace_t *trace ) {
	if ( !other->IsType( idPlayer::Type ) ) {
		return;
	}

	if ( (entFlags & (EUF_TRIGGED|EUF_ONUSE)) == (EUF_TRIGGED|EUF_ONUSE) ) {
		Pickup( static_cast<idPlayer *>(other) );
	}
}
...
void idItem::Event_Trigger( idEntity *activator ) {
	...
	if ( !(entFlags & EUF_TRIGGED) ) {
		entFlags |= EUF_TRIGGED;
		return;
	}

	if ( ( entFlags & EUF_TRIGTAKE ) && activator && activator->IsType( idPlayer::Type ) ) {
...
/*
================
idItem::ConfirmUseDescr
================
*/
bool idItem::ConfirmUseDescr( use_descr_t* Desc )
{
	if ( !(entFlags & EUF_ONUSE) || !(entFlags & EUF_TRIGGED) )
		return false;	// Not useable

	if ( Desc->trace.c.contents & CONTENTS_BODY )	//Do not allow to use bbox
		return false;

	Desc->CursorType = CURTYPE_TAKE;
	Desc->ent = this;
	Desc->ShowInCursor = spawnArgs.GetBool( "use_cursor", "1");
	Desc->ShowInHud = spawnArgs.GetBool( "use_hud", "1");
	Desc->LowerWeapon = false;
	spawnArgs.GetString( "use_act", "", Desc->strAct);
	if (!spawnArgs.GetString( "use_obj", "Item", Desc->strObj)) {
		spawnArgs.GetString( "inv_name", "", Desc->strObj);
	}
	Desc->subIDX = 0;
...
================
*/
void idItemPowerup::Save( idSave_I *savefile ) const {
...
================
*/
void idItemPowerup::Restore( idRestore_I *savefile ) {
...
================
*/
void idObjective::Save( idSave_I *savefile ) const {
...
================
*/
void idObjective::Restore( idRestore_I *savefile ) {
	savefile->ReadVec3( playerPos );
	PostEventMS( &EV_CamShot, NULL, 250 );
}
...
================
*/
void idObjective::Event_CamShot( void ) {
	const char *camName;
	...
	if ( player ) {
		...
		if ( spawnArgs.GetString( "inv_objective", NULL ) ) {
	 		if ( player && player->hud ) {
				...
				PostEventMS( &EV_GetPlayerPos, NULL, 2000 );
			}
		}
	}
}
...
================
*/
void idObjective::Event_GetPlayerPos( void ) {
	idPlayer *player = gameLocal.GetLocalPlayer();
	if ( player ) {
		playerPos = player->GetPhysics()->GetOrigin();
		PostEventMS( &EV_HideObjective, NULL, 100, player );
	}
}
...
================
*/
void idMoveableItem::Save( idSave_I *savefile ) const {
...
================
*/
void idMoveableItem::Restore( idRestore_I *savefile ) {
...
/*
================
idMoveableItem::ConfirmUseDescr
================
*/
bool idMoveableItem::ConfirmUseDescr( use_descr_t* Desc )
{
	if ( !(entFlags & EUF_ONUSE) || !(entFlags & EUF_TRIGGED) )
		return false;	// Not useable
	...
	if ( Desc->trace.c.contents & CONTENTS_TRIGGER )	//Skip entity trigger
		return false;
...
=================
*/
impactEffect_t idMoveableItem::Collide( const trace_t &collision, const idVec3 &velocity ) {
	float v, f;
...
================
*/
void idObjectiveComplete::Save( idSave_I *savefile ) const {
	savefile->WriteVec3( playerPos );
}
...
================
*/
void idObjectiveComplete::Restore( idRestore_I *savefile ) {
	savefile->ReadVec3( playerPos );
}
...
================
*/
void idObjectiveComplete::Event_GetPlayerPos( void ) {
	idPlayer *player = gameLocal.GetLocalPlayer();
	if ( player ) {
		playerPos = player->GetPhysics()->GetOrigin();
		PostEventMS( &EV_HideObjective, NULL, 100, player );
