...
//#else
//		if ( !declManager->FindType( DECL_ENTITYDEF, token, false ) ) {
//#endif
			...
			switch( command.type ) {
				...
				case FC_TRIGGER: {
					...
					if ( target ) {
						...
						target->ProcessEvent( &EV_Activate, NULL, ent );
						target->TriggerGuis();
					} else {
				...
				case FC_TRIGGER_SMOKE_PARTICLE: {
					ent->ProcessEvent( &AI_TriggerParticles, NULL, command.string->c_str() );
					break;
				}
				case FC_MELEE: {
					ent->ProcessEvent( &AI_AttackMelee, NULL, command.string->c_str() );
					break;
				}
				case FC_DIRECTDAMAGE: {
					ent->ProcessEvent( &AI_DirectDamage, NULL, command.string->c_str() );
					break;
				}
				case FC_BEGINATTACK: {
					ent->ProcessEvent( &AI_BeginAttack, NULL, command.string->c_str() );
					break;
				}
				case FC_ENDATTACK: {
					ent->ProcessEvent( &AI_EndAttack, NULL );
					break;
				}
				case FC_MUZZLEFLASH: {
					ent->ProcessEvent( &AI_MuzzleFlash, NULL, command.string->c_str() );
					break;
				}
				case FC_CREATEMISSILE: {
					ent->ProcessEvent( &AI_CreateMissile, NULL, command.string->c_str() );
					break;
				}
				case FC_LAUNCHMISSILE: {
					ent->ProcessEvent( &AI_AttackMissile, NULL, command.string->c_str() );
					break;
				}
				case FC_FIREMISSILEATTARGET: {
					ent->ProcessEvent( &AI_FireMissileAtTarget, NULL, modelDef->GetJointName( command.index ), command.string->c_str() );
					break;
				}
				case FC_LAUNCH_PROJECTILE: {
					ent->ProcessEvent( &AI_LaunchProjectile, NULL, command.string->c_str() );
					break;
				}
				case FC_TRIGGER_FX: {
					ent->ProcessEvent( &AI_TriggerFX, NULL, modelDef->GetJointName( command.index ), command.string->c_str() );
					break;
				}
				case FC_START_EMITTER: {
					...
					if(index >= 0) {
						...
						ent->ProcessEvent( &AI_StartEmitter, NULL, name.c_str(), modelDef->GetJointName( command.index ), particle.c_str() );
					}
				}

				case FC_STOP_EMITTER: {
					ent->ProcessEvent( &AI_StopEmitter, NULL, command.string->c_str() );
				}
				case FC_FOOTSTEP : {
					ent->ProcessEvent( &EV_Footstep, NULL );
					break;
				}
				case FC_LEFTFOOT: {
					ent->ProcessEvent( &EV_FootstepLeft, NULL );
					break;
				}
				case FC_RIGHTFOOT: {
					ent->ProcessEvent( &EV_FootstepRight, NULL );
					break;
				}
				case FC_ENABLE_EYE_FOCUS: {
					ent->ProcessEvent( &AI_EnableEyeFocus, NULL );
					break;
				}
				case FC_DISABLE_EYE_FOCUS: {
					ent->ProcessEvent( &AI_DisableEyeFocus, NULL );
					break;
				}
				case FC_DISABLE_GRAVITY: {
					ent->ProcessEvent( &AI_DisableGravity, NULL );
					break;
				}
				case FC_ENABLE_GRAVITY: {
					ent->ProcessEvent( &AI_EnableGravity, NULL );
					break;
				}
				case FC_JUMP: {
					ent->ProcessEvent( &AI_JumpFrame, NULL );
					break;
				}
				case FC_ENABLE_CLIP: {
					ent->ProcessEvent( &AI_EnableClip, NULL );
					break;
				}
				case FC_DISABLE_CLIP: {
					ent->ProcessEvent( &AI_DisableClip, NULL );
					break;
				}
				case FC_ENABLE_WALK_IK: {
					ent->ProcessEvent( &EV_EnableWalkIK, NULL );
					break;
				}
				case FC_DISABLE_WALK_IK: {
					ent->ProcessEvent( &EV_DisableWalkIK, NULL );
					break;
				}
				case FC_ENABLE_LEG_IK: {
					ent->ProcessEvent( &EV_EnableLegIK, NULL, command.index );
					break;
				}
				case FC_DISABLE_LEG_IK: {
					ent->ProcessEvent( &EV_DisableLegIK, NULL, command.index );
					break;
				}
				case FC_RECORDDEMO: {
					if ( command.string ) {
...
=====================
*/
void idAnimBlend::Save( idSave_I *savefile ) const {
	int i;
...
=====================
*/
void idAnimBlend::Restore( idRestore_I *savefile, const idDeclModelDef *modelDef ) {
	int	i;
...
=====================
*/
void idAnimator::Save( idSave_I *savefile ) const {
	int i;
	int j;
...
=====================
*/
void idAnimator::Restore( idRestore_I *savefile ) {
	int i;
	int j;
	int num;
...
void idAnimator::GetDelta( int fromtime, int totime, idVec3 &delta ) const {
	...
	if ( modelDef->Joints()[ 0 ].channel ) {
		if ( modelDef->Joints()[ 0 ].channel == ANIMCHANNEL_AF ) {
			delta.Zero();
		} else {
...
bool idAnimator::FrameHasChanged( int currentTime ) const {
	...
	for( i = 0; i < ANIM_NumAnimChannels; i++ ) {
		for( j = 0; j < ANIM_MaxAnimsPerChannel; j++, blend++ ) {
			if ( blend->FrameHasChanged( currentTime ) || !blend->IsDone( currentTime ) ) {
				return true;
			}
		}
	}
...
bool idAnimator::CreateFrame( int currentTime, bool force ) {
	...
	// !c4tnt! This part (AFPose\Frame and other) is very expensive and will be removed
