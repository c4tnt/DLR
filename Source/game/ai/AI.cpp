...
=====================
*/
void idMoveState::Save( idSave_I *savefile ) const {
	savefile->WriteInt( (int)moveType );
	savefile->WriteInt( (int)moveCommand );
	if ( moveCommand != MOVE_NONE ) {
...
=====================
*/
void idMoveState::Restore( idRestore_I *savefile ) {
...
=====================
*/
void idAI::Save( idSave_I *savefile ) const {
	int i;
	...
	savefile->WriteFloat( infightchance );

	savefile->WriteFloat( projectile_height_to_distance_ratio );

//	savefile->WriteComment ("projectiles");
...
=====================
*/
void idAI::Restore( idRestore_I *savefile ) {
	bool		restorePhysics;
	int			i;
	int			num;
	idBounds	bounds;
	...
	savefile->ReadFloat( infightchance );
...
//	}
	...
	for(int i = 0; i < funcEmitters.Num(); i++) {
		...
		if(emitter->particle) {
			...
			emitter->particle->PostEventMS(&EV_Remove, NULL, 0 );
		}
	}
	funcEmitters.Clear();
...
=====================
*/
void idAI::HSave( idSave_I *savefile ) {
}
...
=====================
*/
void idAI::HRestore( idRestore_I *savefile ) {
...
void idAI::Spawn( void ) {
	...
	if ( rank <= 0 ) {
		spawnArgs.GetFloat( "infight_chance", "0", infightchance );
	} else {
		spawnArgs.GetFloat( "infight_chance", "0.5", infightchance );
	}
...
/*
===================
idAI::ConfirmUseDescr
===================
*/
bool idAI::ConfirmUseDescr( use_descr_t* Desc ) {

	talkState_t	ts = GetTalkState();

	if (ts < TALK_OK || !Desc)
		return false;	//Disallow talking

	if ( !use_combat_bbox && Desc->trace.c.contents & CONTENTS_BODY )	//Do not allow to use bbox when it no really needed
		return false;

	if ( ts == TALK_OK ) {
		Desc->CursorType = CURTYPE_TALK;
	} else {
		Desc->CursorType = CURTYPE_TALK_BUSY;
	}
	Desc->ent = this;
	Desc->ShowInCursor = true;
	Desc->ShowInHud = true;
	Desc->LowerWeapon = true;
	Desc->strObj = spawnArgs.GetString( "npc_name", "" );
	Desc->strAct = spawnArgs.GetString( "npc_action", "#str_02036"); //Localized string "Talk" by default
	Desc->subIDX = 0;	//Not used
	Desc->stickyTimer = gameLocal.time + 500;
	return true;
}
/*
===================
...
*/
	...
	if(ai_showHealth.GetBool()) {
		...
		aboveHead += this->GetEyePosition();
		if ( fl.hidden ) {
			gameRenderWorld->DrawText( va( "%d", ( int )health), aboveHead, 0.5f, colorDkGrey, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
		} else {
			if ( fl.takedamage ) {
				if(damageCap > 0 && health <= damageCap) {
					gameRenderWorld->DrawText( va( "%d", ( int )health), aboveHead, 0.5f, colorRed, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
				} else {
					gameRenderWorld->DrawText( va( "%d", ( int )health), aboveHead, 0.5f, colorGreen, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
				}
			} else {
				gameRenderWorld->DrawText( va( "%d", ( int )health), aboveHead, 0.5f, colorOrange, gameLocal.GetLocalPlayer()->viewAngles.ToMat3() );
			}
		}
	}
}
...
void idAI::KickObstacles( const idVec3 &dir, float force, idEntity *alwaysKick ) {
	...
	for ( i = 0; i < numListedClipModels; i++ ) {
		...
		if ( obEnt->IsType( idMoveable::Type ) && obEnt->GetPhysics()->IsPushable() ) {
			...
			delta.Normalize(); //c4tnt: more precious
...
int idAI::ReactionTo( const idEntity *ent ) {
	...
	if ( actor->rank <= rank && gameLocal.random.Chance( infightchance ) ) {
		return ATTACK_ON_DAMAGE;
	}

	// monsters will fight with their attacker
	if ( actor->IsType( idAI::Type ) && static_cast<const idAI *>(actor)->GetEnemy() == this ) {
		return ATTACK_ON_DAMAGE;
	}
...
void idAI::PlayCinematic( void ) {
	...
	if ( current_cinematic >= num_cinematics ) {
		...
		fl.neverDormant = false;	//warning - not SAFE!!!
		return;
	}
	...
	ProcessEvent( &AI_PlayAnim, NULL, ANIMCHANNEL_TORSO, animname );
...
void idAI::TalkTo( idActor *actor ) {
	...
	if ( team == 1 && actor->team != team ) {
		ProcessEvent( &EV_Activate, NULL, actor );
	}
...
=====================
*/
idEntity *idAI::GetEnemy( void ) const {
...
void idAI::UpdateEnemyPosition( void ) {
	...
	if ( ai_debugMove.GetBool() || ai_debugTracking.GetBool() ) {
		gameRenderWorld->DebugBounds( colorOrange, enemyEnt->GetPhysics()->GetBounds(), lastReachableEnemyPos, gameLocal.msec );
		gameRenderWorld->DebugBounds( colorCyan, enemyEnt->GetPhysics()->GetBounds(), lastVisibleReachableEnemyPos, gameLocal.msec );
		gameRenderWorld->DebugBounds( colorPink, enemyEnt->GetPhysics()->GetBounds(), lastVisibleEnemyPos, gameLocal.msec );
	}
}
...
/*
=====================
idAI::ReplyProjectile
=====================
*/
idProjectile *idAI::ReplyProjectile( const idVec3 &pos, const idVec3 &dir ) {
	idEntity *ent;
	idEntity *prj_ent;
	const char *clsname;

	if ( !(prj_ent = projectile.GetEntity()) ) {
		return NULL;
	}

	gameLocal.SpawnEntityDef( prj_ent->spawnArgs, &ent, false );
	if ( !ent ) {
		clsname = prj_ent->spawnArgs.GetString( "classname" );
		gameLocal.Error( "Could not spawn entityDef '%s'", clsname );
	}
...
/*
=====================
idAI::CreateProjectileWDef
=====================
*/
idProjectile *idAI::CreateProjectileWDef( const idDict &pDef, const idVec3 &pos, const idVec3 &dir ) {
	idEntity *ent;
	const char *clsname;

	if ( !projectile.GetEntity() ) {
		gameLocal.SpawnEntityDef( pDef, &ent, false );
		if ( !ent ) {
			clsname = projectileDef->GetString( "classname" );
			gameLocal.Error( "Could not spawn entityDef '%s'", clsname );
		}
...
void idAI::RemoveProjectile( void ) {
	if ( projectile.GetEntity() ) {
		projectile.GetEntity()->PostEventMS( &EV_Remove, NULL, 0 );
		projectile = NULL;
	}
}
...
idProjectile *idAI::LaunchProjectile( const char *jointname, idEntity *target, bool clampToAttackCone ) {
	...
	for( i = 0; i < num_projectiles; i++ ) {
		...
		if ( i < num_projectiles - 1 ) {
			ReplyProjectile( muzzle, dir );
		}
	}
	projectile = NULL;
...
bool idAI::AttackMelee( const char *meleeDefName ) {
	...
	if ( enemyEnt->IsType( idPlayer::Type ) && gameLocal.skill.GetBoolKey("savingThrow", "0") ) {
		int	damage, armor;
...
idEntity* idAI::StartEmitter( const char* name, const char* joint, const char* particle ) {
	...
	newEmitter.particle->PostEventMS(&EV_Activate, NULL, 0, this);
...
=====================
*/
void idCombatNode::Save( idSave_I *savefile ) const {
...
=====================
*/
void idCombatNode::Restore( idRestore_I *savefile ) {
...
=====================
*/
void idCombatNode::HSave( idSave_I *savefile ) const {

}

/*
=====================
idCombatNode::HRestore
=====================
*/
void idCombatNode::HRestore( idRestore_I *savefile ) {
}
...
=====================
*/
bool idCombatNode::EntityInView( idEntity *actor, const idVec3 &pos ) {
