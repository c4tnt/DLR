...
const idEventDef AI_CustomMissile( "customMissile", "ssd", 'E' );
...
const idEventDef AI_LaunchProjectile( "launchProjectile", "ss" ); // ROE ( NOT ROE ACTUALLY !!! )
...
const idEventDef AI_AddHealth( "addHealth", "f" );		//c4tnt: Add some health
...
const idEventDef AI_WaitMove( "waitMove", NULL, '*' );
...
CLASS_DECLARATION( idActor, idAI )
	...
	EVENT( AI_CustomMissile,					idAI::Event_CustomMissile )
...
=====================
*/
void idAI::Event_FindEnemy( idEventReturn *returnContext, int useFOV ) {
	int			i;
	idEntity	*ent;
	idActor		*actor;
	...
	if ( gameLocal.InPlayerPVS( this ) ) {
		for ( i = 0; i < gameLocal.numClients ; i++ ) {
			...
			if ( CanSee( actor, useFOV != 0 ) ) {
				returnContext->FinallyEntity( actor );
...
=====================
*/
void idAI::Event_FindEnemyAI( idEventReturn *returnContext, int useFOV ) {
	idEntity	*ent;
	idEntity	*bestEnemy;
	float		bestDist;
	float		dist;
	...
	for ( ent = gameLocal.activeEntities.Next(); ent != NULL; ent = ent->activeNode.Next() ) {
		...
		if ( ( ent->health <= 0 ) || !( ReactionTo( ent ) & ATTACK_ON_SIGHT ) ) {
			continue;
		}
	...
	returnContext->FinallyEntity( bestEnemy );
...
=====================
*/
void idAI::Event_FindEnemyInCombatNodes( idEventReturn *returnContext ) {
	int				i, j;
	idCombatNode	*node;
	idEntity		*ent;
	idEntity		*targetEnt;
...
=====================
*/
void idAI::Event_ClosestReachableEnemyOfEntity( idEventReturn *returnContext, idEntity *team_mate ) {
	idActor *actor;
	idActor *ent;
	idEntity	*bestEnt;
	...
	returnContext->FinallyEntity( bestEnt );
}
...
=====================
*/
void idAI::Event_HeardSound( idEventReturn *returnContext, int ignore_team ) {
...
/*
=====================
idAI::Event_CustomMissile
=====================
*/
void idAI::Event_CustomMissile( idEventReturn *returnContext, const char *entityDefName, const char *jointname, int UseSpawnArgs ) {
	idVec3 muzzle;
	idMat3 axis;
	...
	if ( UseSpawnArgs ) {
		idDict w_spawnargs;
		if ( projDef != NULL ) w_spawnargs.Copy( *projDef );
		w_spawnargs.TransferKeyValues(gameLocal.spawnArgs);
		CreateProjectileWDef( w_spawnargs, muzzle, viewAxis[ 0 ] * physicsObj.GetGravityAxis() );
	} else {
		if ( projDef == NULL ) {
			gameLocal.Warning( "%s (%s) - projectile def %s not found", name.c_str(), GetEntityDefName(), entityDefName );
			returnContext->FinallyEntity( NULL );
			return;
		}
		CreateProjectileWDef( *projDef, muzzle, viewAxis[ 0 ] * physicsObj.GetGravityAxis() );
	}
	...
	returnContext->FinallyEntity( projectile.GetEntity() );
}
...
=====================
*/
void idAI::Event_CreateMissile( idEventReturn *returnContext, const char *jointname ) {
	idVec3 muzzle;
	idMat3 axis;
...
=====================
*/
void idAI::Event_AttackMissile( idEventReturn *returnContext, const char *jointname ) {
	idProjectile *proj;

	proj = LaunchProjectile( jointname, enemy.GetEntity(), true );
	returnContext->FinallyEntity( proj );
...
=====================
*/
void idAI::Event_FireMissileAtTarget( idEventReturn *returnContext, const char *jointname, const char *targetname ) {
	idEntity		*aent;
	idProjectile	*proj;
...
=====================
*/
void idAI::Event_LaunchMissile( idEventReturn *returnContext, const idVec3 &org, const idAngles &ang ) {
	idVec3		start;
	trace_t		tr;
	idBounds	projBounds;
...
=====================
*/
void idAI::Event_LaunchProjectile( const char *entityDefName, const char *jointname ) {
...
=====================
*/
void idAI::Event_AttackMelee( idEventReturn *returnContext, const char *meleeDefName ) {
	bool hit;
	
	hit = AttackMelee( meleeDefName );
	returnContext->FinallyInt( hit );
...
=====================
*/
void idAI::Event_RandomPath( idEventReturn *returnContext ) {
	idPathCorner *path;

	path = idPathCorner::RandomPath( this, NULL );
	returnContext->FinallyEntity( path );
...
=====================
*/
void idAI::Event_MeleeAttackToJoint( idEventReturn *returnContext, const char *jointname, const char *meleeDefName ) {
	jointHandle_t	joint;
	idVec3			start;
	idVec3			end;
	idMat3			axis;
...
=====================
*/
void idAI::Event_CanBecomeSolid( idEventReturn *returnContext ) {
	int			i;
	int			num;
	idEntity *	hit;
	idClipModel *cm;
	...
	for ( i = 0; i < num; i++ ) {
		...
		if ( physicsObj.ClipContents( cm ) ) {
			returnContext->FinallyFloat( false );
			
			return;
		}
	}

	returnContext->FinallyFloat( true );
...
=====================
*/
void idAI::Event_BecomeRagdoll( idEventReturn *returnContext ) {
	bool result;

	result = StartRagdoll();
	returnContext->FinallyInt( result );
...
=====================
*/
void idAI::Event_GetHealth( idEventReturn *returnContext ) {
...
=====================
*/
void idAI::Event_GetCurrentYaw( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( current_yaw );
...
=====================
*/
void idAI::Event_MoveStatus( idEventReturn *returnContext ) {
	returnContext->FinallyInt( move.moveStatus );
...
=====================
*/
void idAI::Event_FacingIdeal( idEventReturn *returnContext ) {
	bool facing = FacingIdeal();
	returnContext->FinallyInt( facing );
...
=====================
*/
void idAI::Event_WaitAction( idEventReturn *returnContext, const char *waitForState ) {
...
=====================
*/
void idAI::Event_GetCombatNode( idEventReturn *returnContext ) {
	int				i;
	float			dist;
	idEntity		*targetEnt;
	idCombatNode	*node;
	...
	returnContext->FinallyEntity( bestNode );
}
...
=====================
*/
void idAI::Event_EnemyInCombatCone( idEventReturn *returnContext, idEntity *ent, int use_current_enemy_location ) {
	idCombatNode	*node;
	bool			result;
	idEntity		*enemyEnt = enemy.GetEntity();
...
=====================
*/
void idAI::Event_WaitMove( idEventReturn *returnContext ) {
...
=====================
*/
void idAI::Event_GetJumpVelocity( idEventReturn *returnContext, const idVec3 &pos, float speed, float max_height ) {
	idVec3 start;
	idVec3 end;
	idVec3 dir;
	float dist;
	...
	if ( !enemyEnt ) {
		returnContext->FinallyVector( vec3_zero );
	...
	if ( result ) {
		returnContext->FinallyVector( dir * speed );
	} else {
		returnContext->FinallyVector( vec3_zero );
	}
...
=====================
*/
void idAI::Event_EntityInAttackCone( idEventReturn *returnContext, idEntity *ent ) {
	float	attack_cone;
	idVec3	delta;
	float	yaw;
	float	relYaw;
...
=====================
*/
void idAI::Event_CanSeeEntity( idEventReturn *returnContext, idEntity *ent ) {
	if ( !ent ) {
		returnContext->FinallyInt( false );
	...
	returnContext->FinallyInt( cansee );
...
=====================
*/
void idAI::Event_GetTalkTarget( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( talkTarget.GetEntity() );
...
=====================
*/
void idAI::Event_EnemyRange( idEventReturn *returnContext ) {
	float dist;
	idEntity *enemyEnt = enemy.GetEntity();
...
=====================
*/
void idAI::Event_EnemyRange2D( idEventReturn *returnContext ) {
	float dist;
	idEntity *enemyEnt = enemy.GetEntity();
...
=====================
*/
void idAI::Event_GetEnemy( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( enemy.GetEntity() );
...
=====================
*/
void idAI::Event_GetEnemyPos( idEventReturn *returnContext ) {
	returnContext->FinallyVector( lastVisibleEnemyPos );
...
=====================
*/
void idAI::Event_GetEnemyEyePos( idEventReturn *returnContext ) {
	returnContext->FinallyVector( lastVisibleEnemyPos + lastVisibleEnemyEyeOffset );
...
=====================
*/
void idAI::Event_PredictEnemyPos( idEventReturn *returnContext, float time ) {
	predictedPath_t path;
	idEntity *enemyEnt = enemy.GetEntity();

	// if no enemy set
	if ( !enemyEnt ) {
		returnContext->FinallyVector( physicsObj.GetOrigin() );
	...
	returnContext->FinallyVector( path.endPos );
...
=====================
*/
void idAI::Event_CanHitEnemy( idEventReturn *returnContext ) {
	trace_t	tr;
	idEntity *hit;
	...
	if ( gameLocal.time == lastHitCheckTime ) {
		returnContext->FinallyInt( lastHitCheckResult );
...
=====================
*/
void idAI::Event_CanHitEnemyFromAnim( idEventReturn *returnContext, const char *animname ) {
	int		anim;
	idVec3	dir;
	idVec3	local_dir;
	idVec3	fromPos;
	...
	if ( enemyEnt->GetPhysics()->GetAbsBounds().IntersectsBounds( physicsObj.GetAbsBounds().Expand( 16.0f ) ) ) {
		Event_CanHitEnemy(returnContext);
		return;
	}
...
=====================
*/
void idAI::Event_CanHitEnemyFromJoint( idEventReturn *returnContext, const char *jointname ) {
	trace_t	tr;
	idVec3	muzzle;
	idMat3	axis;
	idVec3	start;
...
=====================
*/
void idAI::Event_EnemyPositionValid( idEventReturn *returnContext ) {
	bool result;
...
=====================
*/
void idAI::Event_TestChargeAttack( idEventReturn *returnContext ) {
	trace_t trace;
	idActor *enemyEnt = enemy.GetEntity();
	predictedPath_t path;
	idVec3 end;
	...
	if ( ( path.endEvent == 0 ) || ( path.blockingEntity == enemyEnt ) ) {
		...
		returnContext->FinallyFloat( time );
	} else {
		returnContext->FinallyFloat( 0.0f );
	}
...
=====================
*/
void idAI::Event_TestAnimMoveTowardEnemy( idEventReturn *returnContext, const char *animname ) {
	int				anim;
	predictedPath_t path;
	idVec3			moveVec;
	float			yaw;
	...
	returnContext->FinallyInt( path.endEvent == 0 );
...
=====================
*/
void idAI::Event_TestAnimMove( idEventReturn *returnContext, const char *animname ) {
	int				anim;
	predictedPath_t path;
	idVec3			moveVec;
...
=====================
*/
void idAI::Event_TestMoveToPosition( idEventReturn *returnContext, const idVec3 &position ) {
	predictedPath_t path;
...
=====================
*/
void idAI::Event_TestMeleeAttack( idEventReturn *returnContext ) {
...
=====================
*/
void idAI::Event_TestAnimAttack( idEventReturn *returnContext, const char *animname ) {
	int				anim;
	predictedPath_t path;
	...
	returnContext->FinallyInt( path.blockingEntity && ( path.blockingEntity == enemy.GetEntity() ) );
...
=====================
*/
void idAI::Event_Shrivel( idEventReturn *returnContext, float shrivel_time ) {
	float t;
...
=====================
*/
void idAI::Event_NumSmokeEmitters( idEventReturn *returnContext ) {
	returnContext->FinallyInt( particles.Num() );
...
=====================
*/
void idAI::Event_GetTurnDelta( idEventReturn *returnContext ) {
	float amount;

	if ( turnRate ) {
		amount = idMath::AngleNormalize180( ideal_yaw - current_yaw );
		returnContext->FinallyFloat( amount );
	} else {
		returnContext->FinallyFloat( 0.0f );
	}
...
=====================
*/
void idAI::Event_GetMoveType( idEventReturn *returnContext ) {
	returnContext->FinallyInt( move.moveType );
...
=====================
*/
void idAI::Event_GetClosestHiddenTarget( idEventReturn *returnContext, const char *type ) {
	int	i;
	idEntity *ent;
	idEntity *bestEnt;
	float time;
...
=====================
*/
void idAI::Event_GetRandomTarget( idEventReturn *returnContext, const char *type ) {
	int	i;
	int	num;
	int which;
	idEntity *ent;
	...
	returnContext->FinallyEntity( ents[ which ] );
...
================
*/
void idAI::Event_TravelDistanceToPoint( idEventReturn *returnContext, const idVec3 &pos ) {
	float time;
...
================
*/
void idAI::Event_TravelDistanceToEntity( idEventReturn *returnContext, idEntity *ent ) {
	float time;
...
================
*/
void idAI::Event_TravelDistanceBetweenPoints( idEventReturn *returnContext, const idVec3 &source, const idVec3 &dest ) {
	float time;
...
================
*/
void idAI::Event_TravelDistanceBetweenEntities( idEventReturn *returnContext, idEntity *source, idEntity *dest ) {
	float time;
...
void idAI::Event_ThrowMoveable( void ) {
	...
	if ( moveable ) {
		moveable->Unbind();
		moveable->PostEventMS( &EV_SetOwner, NULL, 200, NULL );
	}
}
...
void idAI::Event_ThrowAF( void ) {
	...
	if ( af ) {
		af->Unbind();
		af->PostEventMS( &EV_SetOwner, NULL, 200, NULL );
	}
}
...
================
*/
void idAI::Event_GetAngles( idEventReturn *returnContext ) {
	returnContext->FinallyVector( idVec3( 0.0f, current_yaw, 0.0f ) );
...
void idAI::Event_Kill( void ) {
	PostEventMS( &AI_RealKill, NULL, 0 );
}
...
================
*/
void idAI::Event_GetObstacle( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( move.obstacle.GetEntity() );
...
================
*/
void idAI::Event_PushPointIntoAAS( idEventReturn *returnContext, const idVec3 &pos ) {
	int		areaNum;
	idVec3	newPos;
	...
	if ( areaNum ) {
		...
		returnContext->FinallyVector( newPos );
	} else {
		returnContext->FinallyVector( pos );
	}
...
================
*/
void idAI::Event_GetTurnRate( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( turnRate );
...
=====================
*/
void idAI::Event_FindActorsInBounds( idEventReturn *returnContext, const idVec3 &mins, const idVec3 &maxs ) {
	idEntity *	ent;
...
================
*/
void idAI::Event_CanReachPosition( idEventReturn *returnContext, const idVec3 &pos ) {
	aasPath_t	path;
	int			toAreaNum;
	int			areaNum;
...
================
*/
void idAI::Event_CanReachEntity( idEventReturn *returnContext, idEntity *ent ) {
	aasPath_t	path;
	int			toAreaNum;
	int			areaNum;
	idVec3		pos;
...
================
*/
void idAI::Event_CanReachEnemy( idEventReturn *returnContext ) {
	aasPath_t	path;
	int			toAreaNum;
	int			areaNum;
	idVec3		pos;
...
================
*/
void idAI::Event_GetReachableEntityPosition( idEventReturn *returnContext, idEntity *ent ) {
	int		toAreaNum;
	idVec3	pos;
	...
	if ( move.moveType != MOVETYPE_FLY ) {
		if ( !ent->GetFloorPos( 64.0f, pos ) ) {
			...
			return returnContext->FinallyVector( vec3_zero );
		}
		if ( ent->IsType( idActor::Type ) && static_cast<idActor *>( ent )->OnLadder() ) {
...
void idAI::Event_StartEmitter( idEventReturn *returnContext, const char* name, const char* joint, const char* particle ) {
	idEntity *ent = StartEmitter(name, joint, particle);
	returnContext->FinallyEntity(ent);
}

void idAI::Event_GetEmitter( idEventReturn *returnContext, const char* name ) {
	returnContext->FinallyEntity(GetEmitter(name));
}
