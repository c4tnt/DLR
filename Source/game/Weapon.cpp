...
const idEventDef EV_Weapon_LaunchProjectilesEllipse( "launchProjectilesEllipse", "dffff" ); //ROE
...
================
*/
void idWeapon::Save( idSave_I *savefile ) const {
...
================
*/
void idWeapon::Restore( idRestore_I *savefile ) {
	...
	WEAPON_ATTACKMOD.LinkTo(	scriptObject, "WEAPON_MODE" );
...
//	gameLocal.Printf("Weapon %s get own def: %s\n", GetName(), objectname);	//c4tnt: just debug
...
void idWeapon::OwnerDied( void ) {
	...
	PostEventMS( &EV_Weapon_Clear, NULL, 0 );
}
...
================
*/
void idWeapon::BeginAttack( const int mode ) {
...
void idWeapon::AlertMonsters( void ) {
	...
	if ( tr.fraction < 1.0f ) {
		...
		if ( ent->IsType( idAI::Type ) ) {
			...
			ent->ProcessEvent( &EV_Touch, NULL, owner, &tr );
		}
	}
...
/*
================
idWeapon::GetAmmoNumForNameSafe
================
*/
ammo_t idWeapon::GetAmmoNumForNameSafe( const char *ammoname ) {
	int num;
	const idDict *ammoDict;
...
===============
*/
void idWeapon::Event_GetOwner( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( owner );
}
...
===============
*/
void idWeapon::Event_AmmoInClip( idEventReturn *returnContext ) {
	int ammo = AmmoInClip();
	returnContext->FinallyFloat( ammo );	
}
...
===============
*/
void idWeapon::Event_AmmoAvailable( idEventReturn *returnContext ) {
	int ammoAvail = owner->inventory.HasAmmo( ammoType, ammoRequired );
	returnContext->FinallyFloat( ammoAvail );
}
...
===============
*/
void idWeapon::Event_TotalAmmoCount( idEventReturn *returnContext ) {
...
===============
*/
void idWeapon::Event_ClipSize( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( clipSize );	
}
...
===============
*/
void idWeapon::Event_AutoReload( idEventReturn *returnContext ) {
	assert( owner );
	if ( gameLocal.isClient ) {
		returnContext->FinallyFloat( 0.0f );
		return;
	}
	returnContext->FinallyFloat( gameLocal.userInfo[ owner->entityNumber ].GetBool( "ui_autoReload" ) );
}
...
===============
*/
void idWeapon::Event_PlayAnim( idEventReturn *returnContext, int channel, const char *animname ) {
	int anim;
...
===============
*/
void idWeapon::Event_PlayCycle( idEventReturn *returnContext, int channel, const char *animname ) {
	int anim;
...
===============
*/
void idWeapon::Event_AnimDone( idEventReturn *returnContext, int channel, int blendFrames ) {
...
===============
*/
void idWeapon::Event_GetBlendFrames( idEventReturn *returnContext, int channel ) {
	returnContext->FinallyInt( animBlendFrames );
}
...
================
*/
void idWeapon::Event_GetLightParm( idEventReturn *returnContext, int parmnum ) {
	if ( ( parmnum < 0 ) || ( parmnum >= MAX_ENTITY_SHADER_PARMS ) ) {
		gameLocal.Error( "shader parm index (%d) out of range", parmnum );
	}

	returnContext->FinallyFloat( muzzleFlash.shaderParms[ parmnum ] );
}
...
================
*/
void idWeapon::Event_CreateProjectile( idEventReturn *returnContext ) {
	if ( !gameLocal.isClient ) {
		projectileEnt = NULL;
		...
		returnContext->FinallyEntity( projectileEnt );
	} else {
...
void idWeapon::Event_LaunchProjectiles( int num_projectiles, float spread, float fuseOffset, float launchPower, float dmgPower ) {
	...
	if ( gameLocal.isClient ) {
		...
		// toss the brass
		PostEventMS( &EV_Weapon_EjectBrass, NULL, brassDelay );
	}
...
=====================
*/
void idWeapon::Event_Melee( idEventReturn *returnContext ) {
	idEntity	*ent;
	trace_t		tr;
...
=====================
*/
void idWeapon::Event_GetWorldModel( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( worldModel.GetEntity() );
}
...
===============
*/
void idWeapon::Event_IsInvisible( idEventReturn *returnContext ) {
	if ( !owner ) {
		returnContext->FinallyFloat( 0 );
		return;
	}
	returnContext->FinallyFloat( owner->PowerUpActive( INVISIBILITY ) ? 1 : 0 );
}
