...
================
*/
void idBrittleFracture::Save( idSave_I *savefile ) const {
	int i, j;
...
================
*/
void idBrittleFracture::Restore( idRestore_I *savefile ) {
	int i, j , num;
...
void idBrittleFracture::Think( void ) {
	...
	for ( i = 0; i < shards.Num(); i++ ) {
		...
		if ( droppedTime != -1 ) {
			if ( ( droppedTime == -2 ) || ( gameLocal.time - droppedTime > SHARD_ALIVE_TIME ) ) { //c4tnt: added manual remove
				RemoveShard( i );
				i--;
			}
			fading = true;
...
void idBrittleFracture::DropShard( shard_t *shard, const idVec3 &point, const idVec3 &dir, const float impulse, const int time ) {
	...
	// c4tnt* fixed impulse translation
	if (dist > minShatterRadius)
	{
		f = dist > maxShatterRadius ? 1.0f : idMath::Sqrt( dist - minShatterRadius ) * ( 1.0f / idMath::Sqrt( maxShatterRadius - minShatterRadius ) );
		shard->physicsObj.ApplyImpulse2( 0, origin, impulse * dir, (1.0f - f)* linearVelocityScale, (f * angularVelocityScale)); 
//		shard->physicsObj.SetAngularVelocity( dir2.Cross( dir ) * (f * angularVelocityScale) );
	}else{
		shard->physicsObj.ApplyImpulse2( 0, origin, impulse * dir, linearVelocityScale, 0.0f ); 
	}
...
/*
================
c4tnt: drop shard silent version (only removes the shards)
idBrittleFracture::DropShard
================
*/
void idBrittleFracture::DropShard( shard_t *shard) {
	int i, j;
	shard_t *neighbour;
...
void idBrittleFracture::DropFloatingIslands( const idVec3 &point, const idVec3 &impulse, const int time ) {
	...
	dir = impulse;
	float impulselen = dir.Normalize();
	...
	for ( i = 0; i < shards.Num(); i++ ) {
		...
		if ( !touchesEdge ) {
			for ( j = 0; j < queueEnd; j++ ) {
				DropShard( queue[j], point, dir, impulselen, time );
			}
		}
	}
}
...
/*
================
c4tnt: silent variance
idBrittleFracture::DropFloatingIslands
================
*/
void idBrittleFracture::DropFloatingIslands( const int* count ) {
	int i, j, numIslands;
	int queueStart, queueEnd;
	shard_t *curShard, *nextShard, **queue;
	bool touchesEdge;
...
void idBrittleFracture::Break( void ) {
	...
	health = 0; //c4tnt: remove some bugs
}
