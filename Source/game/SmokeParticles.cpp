...
idSmokeParticles::idSmokeParticles( void ) {
	...
	memset( asmokes, 0, sizeof( asmokes ) );
	freeSmokes = NULL;
	numActiveSmokes = 0;
...
bool idSmokeParticles::EmitSmokePrt( const idDeclParticle *smoke, const int startTime, const float diversity, const idVec3 &origin, const idMat3 &axis ) {
	...
	int i;
	for ( i = 0 ; i < MAX_AUTO_SMOKES; i++ ) {
		spawnedSmoke_t *active = &asmokes[i];
		if ( active->smoke == NULL ) {
			active->smoke = smoke;
			active->startTime = startTime;
			active->diversity = diversity;
...
bool idSmokeParticles::UpdateSmokePrt() {
bool result = false;
bool b;
	...
	for ( i = 0 ; i < MAX_AUTO_SMOKES; i++ ) {
		spawnedSmoke_t *active = &asmokes[i];
		if ( active->smoke != NULL ) {
			b = EmitSmoke( active->smoke, active->startTime, active->diversity, active->origin, active->axis );
			if (!b) {
				active->smoke = NULL; //Remove this particle
			}
			result |= b;
		}
	}
...
================
*/
bool idSmokeParticles::EmitSmoke( const idDeclParticle *smoke, const int systemStartTime, const float diversity, const idVec3 &origin, const idMat3 &axis,const int lastActive ) {
	...
	for ( int stageNum = 0; stageNum < smoke->stages.Num(); stageNum++ ) {
		...
		atimeMsec = gameLocal.time - systemStartTime;

		deltaMsec = atimeMsec - stage->timeOffset;
		if ( stage->timeOffset > deltaMsec) {
			continue;
		}

		while (deltaMsec > stage->cycleMsec) deltaMsec -= stage->cycleMsec; //MOD by particle cycle time
		modOffset = atimeMsec - deltaMsec + stage->timeOffset;
		
		while (modOffset > lastActive) modOffset -= stage->cycleMsec;
		if (modOffset < 0) modOffset = 0;
		deltaMsec = atimeMsec - modOffset;
		...
		for ( prevCount++ ; prevCount <= nowCount ; prevCount++ ) {
			...
			newSmoke->privateStartTime = systemStartTime + modOffset + prevCount * finalParticleTime / stage->totalParticles;
