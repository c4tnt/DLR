...
const idEventDef EV_SplitProjectile( "SplitProjectile", "svv", 'e' );
...
CLASS_DECLARATION( idEntity, idProjectile )
	...
	EVENT( EV_SplitProjectile,		idProjectile::Event_SplitProjectile )
...
void idProjectile::Spawn( void ) {
	...
	scriptProjectile = scriptObject.HasObject();
...
================
*/
void idProjectile::Save( idSave_I *savefile ) const {
...
================
*/
void idProjectile::Restore( idRestore_I *savefile ) {
...
void idProjectile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float launchPower, const float dmgPower ) {
	...
	projectileFlags.througthGlass		= spawnArgs.GetBool( "blow_glasses" );
	...
	gravVec.Normalize(); //c4tnt: more precious
	...
	if ( !gameLocal.isClient ) {
		if ( fuse <= 0 ) {
			...
			RunPhysics();
			PostEventMS( &EV_Remove, NULL, spawnArgs.GetInt( "remove_time", "1500" ) );
		} else if ( spawnArgs.GetBool( "detonate_on_fuse" ) ) {
			fuse -= timeSinceFire;
			if ( fuse < 0.0f ) {
				fuse = 0.0f;
			}
			PostEventSec( &EV_Explode, NULL, fuse );
		} else {
			fuse -= timeSinceFire;
			if ( fuse < 0.0f ) {
				fuse = 0.0f;
			}
			PostEventSec( &EV_Fizzle, NULL, fuse );
		}
	}
...
void idProjectile::Think( void ) {
	...
	// add the particles
	if ( smokeFly != NULL && smokeFlyTime && (!IsHidden() || state == FIZZLED) ) {
...
=================
*/
impactEffect_t idProjectile::Collide( const trace_t &collision, const idVec3 &velocity ) {
	idEntity	*ent;
	idEntity	*ignore;
	...
	if ( physicsObj.GetBouncyness() ) {
		retn = IMP_BOUNCE;
	} else {
		retn = IMP_TOUCH;
	}

	if (projectileFlags.througthGlass) {
		if ( collision.c.material != NULL ) {
			if ( collision.c.material->GetSurfaceType() & SURFTYPE_GLASS ) {	//c4tnt:: now we can shot througth a glass
				retn = IMP_THROUGTH;
			}
		}
		if ( collision.c.entityNum && gameLocal.entities[collision.c.entityNum]) {
			if (gameLocal.entities[collision.c.entityNum]->IsType(idBrittleFracture::Type))
			{
				retn = IMP_THROUGTH;
			}
		}
	...
	/*if ( ent ) {
		...
		if ( ent->IsType(idAFAttachment::Type) && static_cast<idAFAttachment *>( ent )->GetBindMaster()->IsType(idPlayer::Type) ) {
			owner = ent;
			return IMP_BOUNCE;
		}
	}*/
	...
	if ( collision.c.material != NULL ) {
		if ( collision.c.material->Coverage() == MC_PERFORATED && !ent->IsType( idActor::Type ) ) {								//c4tnt:: test with an alpha-tested
			return IMP_THROUGTH;
		}
	}
	...
	if ( collision.c.material != NULL ) {
		if ( collision.c.material->GetSurfaceFlags() & SURF_NOIMPACT ) {
...
//			common->DPrintf( "Projectile collision no impact\n" );
	...
	if ( gameLocal.isMultiplayer && ent->IsType( idDoor::Type ) && !static_cast< idDoor * >(ent)->IsOpen() && !ent->spawnArgs.GetBool( "no_touch" ) ) {
		ent->ProcessEvent( &EV_Activate, NULL, this );
	}
...
void idProjectile::DefaultDamageEffect( idEntity *soundEnt, const idDict &projectileDef, const trace_t &collision, const idVec3 &velocity ) {
	const char *decal, *sound, *prt, *typeName;
	...
	// show bulelt smoke
	prt = projectileDef.GetString( va( "smoke_puff_%s", typeName ) );
	if ( *prt == '\0' ) {
		prt = projectileDef.GetString( "smoke_puff" );
	}
	if ( *prt != '\0' ) {
		gameLocal.AxisAlongDir( collision.c.normal, gameLocal.random.RandomFloat() * idMath::TWO_PI, axis );
		gameLocal.smokeParticles->EmitSmokePrt( (const idDeclParticle*)declManager->FindType(DECL_PARTICLE, prt, false), gameLocal.time, gameLocal.random.RandomFloat(), collision.c.point, axis );
	}
}
...
================
*/
void idProjectile::Event_GetProjectileState( idEventReturn *returnContext ) {
	returnContext->FinallyInt( state );
}

/*
================
idProjectile::ExplodeShrapnel
================
*/
void idProjectile::ExplodeShrapnel( const char *shrapnelDefName, int num_debris, int prj_mode, float prj_cone, const trace_t &collision ) {

	idVec3				start, rdir, side;
	...
	for ( int i = 0; i < num_debris; i++ ) {
		...
		switch ( prj_mode ) {
		case 0:
			// radial for projectile dir
			rnd = gameLocal.random.RandomFloat() * idMath::TWO_PI;
			side = axis[1] * sin( rnd ) + axis[2] * cos( rnd );
			rnd = gameLocal.random.RandomFloat() * prj_cone;
			rdir = axis[0] * cos( rnd ) + side * sin( rnd );
			break;
		}
		rdir.Normalize();

		projDef = gameLocal.FindEntityDefDict( shrapnelDefName );

		gameLocal.SpawnEntityDef( *projDef, &ent, false );
		if ( !ent || !ent->IsType( idProjectile::Type ) ) {
			gameLocal.Error( "%s is not an idProjectile", shrapnelDefName );
		}

		idProjectile *debris = static_cast<idProjectile *>(ent);
		debris->Create( owner.GetEntity(), start, rdir );
		debris->Launch( start, rdir, vec3_origin );
	}
}
/*
================
...
void idProjectile::Explode( const trace_t &collision, idEntity *ignore ) {
	const char *fxname, *light_shader, *sndExplode, *debrisDefName;
	...
	if ( !projectileFlags.noSplashDamage ) {
		...
		if ( delay ) {
			...
			PostEventSec( &EV_RadiusDamage, NULL, delay, ignore );
		} else {
	...
	if ( fxdebris ) {
		const idDict *debris;
		if ( spawnArgs.GetString( "def_debris", "", &debrisDefName )) {
			debris = gameLocal.FindEntityDefDict( debrisDefName, false );
			if ( debris ) {
				int amount = gameLocal.random.RandomInt( fxdebris );
				if ( amount < fxdebris/2 + 1 ) amount = fxdebris/2 + 1;
		...
		if ( spawnArgs.GetString( "def_shrapnel", "", &debrisDefName )) {
			debris = gameLocal.FindEntityDefDict( debrisDefName, false );
			if ( debris ) {
				int amount = gameLocal.random.RandomInt( fxdebris );
				if ( amount < fxdebris/2 + 1 ) amount = fxdebris/2 + 1;
				for ( int i = 0; i < amount; i++ ) {
					...
					idDebris *debris_ent = static_cast<idDebris *>(ent);
					debris_ent->Create( owner.GetEntity(), physicsObj.GetOrigin(), dir.ToMat3() );
					debris_ent->Launch();
				}
			}
		}
	...
	PostEventMS( &EV_Remove, NULL, removeTime );
}
...
/*
=====================
idProjectile::Event_SplitProjectile
=====================
*/
void idProjectile::Event_SplitProjectile( idEventReturn *returnContext, const char *entityDefName, const idVec3 &offset, const idVec3 &dir ) {
	idVec3				start, rdir;
	const idDict		*projDef;
	idMat3				axis;
	idEntity			*ent;
	...
	gameLocal.SpawnEntityDef( *projDef, &ent, false );
	if ( !ent ) {
		clsname = projDef->GetString( "classname" );
	...
	axis = physicsObj.GetAxis();
	start = physicsObj.GetOrigin() + offset*axis;
	rdir = dir*axis;
	rdir.Normalize();

	proj->Create( owner.GetEntity(), start, rdir );
	proj->Launch( start, rdir, vec3_origin );
...
/*
================
idProjectile::ConstructScriptObject
================
*/
idThread *idProjectile::ConstructScriptObject( void ) {
//	idThread *thread;
...
================
*/
void idGuidedProjectile::Save( idSave_I *savefile ) const {
	enemy.Save( savefile );
...
================
*/
void idGuidedProjectile::Restore( idRestore_I *savefile ) {
...
================
*/
void idSoulCubeMissile::Save( idSave_I *savefile ) const {
...
================
*/
void idSoulCubeMissile::Restore( idRestore_I *savefile ) {
	...
	if ( state == LAUNCHED ) {
		...
		if ( ( seekPos - physicsObj.GetOrigin() ).Length() < 32.0f ) {
			if ( returnPhase ) {
				...
				Hide();
				PostEventSec( &EV_Remove, NULL, 2.0f );
...
================
*/
void idBFGProjectile::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idBFGProjectile::Restore( idRestore_I *savefile ) {
	int i, num;
...
================
*/
void idBFGProjectile::Event_RemoveBeams( void ) {
	FreeBeams();
	UpdateVisuals();
}
...
void idBFGProjectile::Explode( const trace_t &collision, idEntity *ignore ) {
	...
	if ( !gameLocal.isClient ) {
		if ( ignore != NULL ) {
			PostEventMS( &EV_RemoveBeams, NULL, 750 );
		} else {
			PostEventMS( &EV_RemoveBeams, NULL, 0 );
		}
	}
...
=================
*/
void idDebris::Save( idSave_I *savefile ) const {
	owner.Save( savefile );
...
=================
*/
void idDebris::Restore( idRestore_I *savefile ) {
	owner.Restore( savefile );
...
=================
*/
impactEffect_t idDebris::Collide( const trace_t &collision, const idVec3 &velocity ) {
	if ( sndBounce != NULL ) {
