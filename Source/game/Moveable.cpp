...
void idMoveable::Spawn( void ) {
	...
	PostEventMS( &EV_SetOwnerFromSpawnArgs, NULL, 0 );
}
...
================
*/
void idMoveable::Save( idSave_I *savefile ) const {
...
================
*/
void idMoveable::Restore( idRestore_I *savefile ) {
...
/*
================
idMoveable::ConfirmUseDescr
================
*/
bool idMoveable::ConfirmUseDescr( use_descr_t* Desc )
{
	if (spawnArgs.GetBool( "nouse", "0"))
		return false;	// Not useable

	if ( !(Desc->trace.c.contents & MASK_SHOT_RENDERMODEL) )
		return false;
	...
	spawnArgs.GetString( "use_act", "Take", Desc->strAct);
	spawnArgs.GetString( "use_obj", "Item", Desc->strObj);
...
void idMoveable::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( explode ) {
		if ( brokenModel == "" ) {
			PostEventMS( &EV_Remove, NULL, 1000 );
		}
	}
...
void idMoveable::Event_SetOwnerFromSpawnArgs( void ) {
	...
	if ( spawnArgs.GetString( "owner", "", owner ) ) {
		ProcessEvent( &EV_SetOwner, NULL, gameLocal.FindEntity( owner ) );
	}
}
...
================
*/
void idMoveable::Event_IsAtRest( idEventReturn *returnContext ) {
	returnContext->FinallyInt( physicsObj.IsAtRest() );
}
...
================
*/
void idBarrel::Save( idSave_I *savefile ) const {
...
================
*/
void idBarrel::Restore( idRestore_I *savefile ) {
...
const idEventDef EV_SetStability( "setStability", "f" );
...
CLASS_DECLARATION( idBarrel, idExplodingBarrel )
	...
	EVENT( EV_SetStability,				idExplodingBarrel::Event_Stability )
	EVENT( AI_Burn,						idExplodingBarrel::Event_Burn )
END_CLASS
...
================
*/
void idExplodingBarrel::Save( idSave_I *savefile ) const {
	...
	savefile->WriteFloat( UnstableLevel );
}
...
================
*/
void idExplodingBarrel::Restore( idRestore_I *savefile ) {
	...
	savefile->ReadFloat( UnstableLevel );
...
void idExplodingBarrel::Spawn( void ) {
	...
	fl.takedamage = true;
	UnstableLevel = spawnArgs.GetFloat( "unstable", "0" );
	fl.networkSync = true;
...
/*
================
idExplodingBarrel::SetStability (ROE)
================
*/
void idExplodingBarrel::SetStability( float stability ) {
	UnstableLevel = stability;
}

/*
================
idExplodingBarrel::IsStable (ROE)
================
*/
bool idExplodingBarrel::IsStable( void ) {
	return (UnstableLevel <= 0.0f);
}
...
void idExplodingBarrel::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( f > 0.0f && state == NORMAL ) {
		state = BURNING;
		PostEventSec( &EV_Explode, NULL, f );
		StartSound( "snd_burn", SND_CHANNEL_ANY, 0, false, NULL );
		AddParticles( spawnArgs.GetString ( "model_burn", "" ), true );
		return;
	} else {
	...
	if ( f > 0.0f ) {
		PostEventSec( &EV_Respawn, NULL, f );
	} else {
...
void idExplodingBarrel::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, 
	...
	if ( damageDef->FindKey( "radius" ) && GetPhysics()->GetContents() != 0 && GetBindMaster() == NULL ) {
		PostEventMS( &EV_Explode, NULL, 400 );
	} else {
...
================
*/
void idExplodingBarrel::Event_TriggerTargets( void ) {
	ActivateTargets( this );
}
...
================
*/
void idExplodingBarrel::Event_Explode( void ) {
...
================
*/
void idExplodingBarrel::Event_Stability( float unstable ) {
	if ( state == NORMAL ) {
		UnstableLevel = unstable;
	}
}

/*
================
idExplodingBarrel::Event_Burn
================
*/
void idExplodingBarrel::Event_Burn( ) {
	StartBurning();
}
...
================
*/
void idExplodingBarrel::Event_Respawn( void ) {
	int i;
	...
	if ( minRespawnDist ) {
		...
		if ( minDist < minRespawnDist ) {
			PostEventSec( &EV_Respawn, NULL, spawnArgs.GetInt( "respawn_again", "10" ) );
			return;
		}
	}
