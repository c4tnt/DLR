...
extern const idEventDef EV_SetGuiParmDirect;
extern const idEventDef EV_SetGuiFloatDirect;
...
extern const idEventDef EV_Player_FlashlightMonsters;
extern const idEventDef EV_Player_AlertMonsters;
...
const int PLAYER_GUI_HUD = MAX_RENDERENTITY_GUI + 1;
const int PLAYER_GUI_CURSOR = MAX_RENDERENTITY_GUI + 2;
const int PLAYER_GUI_PDA = MAX_RENDERENTITY_GUI + 3;
...
// environment status
enum {
	SUBMERGED_AIR = 0,			// in the air, no additional effects
	SUBMERGED_WATER_A,			// in the water, use water settings A
	SUBMERGED_WATER_B,			// in the water, use water settings B
	SUBMERGED_HAZE,				// in the hazardous atmosphere, use haze settings ( envirosuit protection )
	SUBMERGED_LAVA_A,			// in the lava, use lava settings A
	SUBMERGED_LAVA_B,			// in the lava, use lava settings B
	SUBMERGED_VACUUM,			// in the doom3 vacuum, use vacuum settings
	SUBMERGED_VACUUM_GIB		// in the instagib vacuum
};
...
public:
	...
	//c4tnt: Must be deprecated
	//int						weapon_soulcube; // discarded yet
	int						weapon_pda;
	int						weapon_fists;
	//default weapon. 
	int						weapon_default;
	...
	idEntityPtr<idProjectile> soulCubeProjectile; //c4tnt: it must be deprecated too
...
public:
	...
	void					UpdateAir( void );
	void					UpdateLiquids( void );

	virtual bool			HandleSingleGuiCommand( idEntity *entityGui, idLexer *src, int eg_num );
	bool					GuiActive( void ) { return focusDescr.CursorType == CURTYPE_GUI; }//focusGUIent != NULL; }

	void					PerformImpulse( int impulse );
	void					ScriptImpulse( int impulse );	// Send this impulse to player scriptobject

	void					Spectate( bool spectate );

	void					TogglePDA( void );
	void					ToggleScoreboard( void );
	void					RouteGuiMouse( idUserInterface *gui, int dx, int dy );
	bool					OpenFSGui( int Index );
	void					CloseFSGui( void );
	void					ListFSGui( void );
	bool					DrawFSGui( bool afterpass );
	idUserInterface*		GetFSGui( int Index);
...
private:
	...
	float pm_minviewpitch;						//*
	float pm_maxviewpitch;						//*
	float pm_normalviewheight;					//*
	float pm_crouchviewheight;					//*
	float pm_deadviewheight;					//*
	float pm_crouchrate;						//*
	float pm_bboxwidth;							//*
	float pm_crouchbob;							//*
	float pm_walkbob;							//*
	...
	int						airless;
	int						airTics;				// set to max_airTics at start, drops in vacuum
	int						lastAirDamage;
	...
	// Focus
	int						focusTime;
	use_descr_t				focusDescr;
	idUserInterface *		cursor;

	// Lights
	renderLight_t			worldDebugLight;
	int						worldDebugLightHandle;

	// Fullscreen GUIs
	idList<idFSGui>			fullscreen_guis;
	int						fullscreen_gui_opened;
	int						pda_index;			//PDA
	int						inventory_index;	//Inventory
	int						objectives_index;	//Objectives GUI
	...
	void					FireWeapon( const int WeaponMode );
	void					Weapon_Combat( void );
	void					Weapon_FSGUI( idUserInterface *ui );
...
//  Cursor
	...
	bool					CursorEnter( bool UpdateOnly );
	void					CursorLeave( void );

	void					UpdateLocation( void );
	idUserInterface *		ActiveGui( void );
	idUserInterface *		ActiveFullscreenGui( void );
...
//	void					ExtractEmailInfo( const idStr &email, const char *scan, idStr &out );
	...
	void					UseCursor( bool State, bool Held );

//  gui
	virtual void			ProjectOntoGUI( idUserInterface *gui, bool update ) const;

//	events
	void					Event_GetButtons( idEventReturn *returnContext ); //return d
	void					Event_GetMove( idEventReturn *returnContext ); //return v
	void					Event_GetViewAngles( idEventReturn *returnContext ); //return v
	void					Event_StopFxFov( void );
	void					Event_EnableWeapon( void );
	void					Event_DisableWeapon( void );
	void					Event_GetCurrentWeapon( idEventReturn *returnContext ); //return s
	void					Event_GetPreviousWeapon( idEventReturn *returnContext ); //return s
	void					Event_SelectWeapon( const char *weaponName );
	void					Event_GetWeaponEntity( idEventReturn *returnContext ); //return e
	void					Event_OpenPDA( void );
	void					Event_PDAAvaliable( idEventReturn *returnContext );
	void					Event_InPDA( idEventReturn *returnContext ); //return d
	void					Event_ExitTeleporter( void );
	void					Event_HideTip( void );
	void					Event_LevelTrigger( void );
	void					Event_Gibbed( void );
	void					Event_GetIdealWeapon( idEventReturn *returnContext ); //return s
	...
	void					Event_SetHealthPool( float newHealth );
	void					Event_GetHealthPool( idEventReturn *returnContext ); //return f
	void					Event_SetArmor( float newArmor );
	void					Event_GetArmor( idEventReturn *returnContext ); //return f
	void					Event_GetMaxHealth( idEventReturn *returnContext ); //return f
	void					Event_GetMaxArmor( idEventReturn *returnContext ); //return f

	void					Event_WeaponAvailable( idEventReturn *returnContext, const char* name );
	void					Event_SetPowerupTime( int powerup, int time );
	void					Event_IsPowerupActive( idEventReturn *returnContext, int powerup );
	void					Event_StartWarp();
	void					Event_SetBanish();

	void					Event_FlashlightMonsters();
	void					Event_AlertMonsters();
	void					Event_GetFocusEntity( idEventReturn *returnContext ); //return e
	void					Event_GetFocusType( idEventReturn *returnContext ); //return i
...
ID_INLINE void idPlayer::EnableStamina( bool b ) {
	if ( b ) {
		if ( !max_stamina )
...
ID_INLINE bool idPlayer::viewLocked( ) {
	return influenceActive != INFLUENCE_NONE || privateCameraView || gameLocal.GetCamera() || (gameLocal.cinematicState & CIN_CUT);
}

ID_INLINE bool idPlayer::controlsLocked( ) {
	return influenceActive > INFLUENCE_LEVEL1 || privateCameraView || gameLocal.GetCamera() || (gameLocal.cinematicState & CIN_CUT);
}
