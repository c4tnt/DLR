...
#define TH_RELIABLEFLAGS	7

// Cursor_use descrtption
#define CURTYPE_NONE			0		// unfocused

#define CURTYPE_TALK			1		// talk normal
#define CURTYPE_TALK_BUSY		2		// talk, busy

#define CURTYPE_USE				3		// use normally
#define CURTYPE_USE_BUSY		4		// use in progress
#define CURTYPE_USE_LOCKED		5		// use, locked

#define CURTYPE_TAKE			6		// take

#define CURTYPE_USEMULTI		7		// use multi

#define CURTYPE_VEHICLE			8		// vehicle
#define CURTYPE_VEHICLE_BUSY	9		// vehicle occupied
#define CURTYPE_VEHICLE_LOCKED	10		// vehicle locked

#define CURTYPE_GUI				11		// on gui
#define CURTYPE_INTERNALGUI		12		// on internal fullscreen gui

typedef struct {
	int						CursorType;		// Internal cursor type
	int						subIDX;			// For guis and other multipart objects
	int						stickyTimer;	// Unstick only when stickyTimer < gameLocal.time 
	idEntityPtr<idEntity>	ent;			// Entity in a focus
	bool					ShowInHud;		// Send notification to a HUD
	bool					ShowInCursor;	// Send notification to a cursor
	bool					LowerWeapon;	// Lower weapon and interact with object
	bool					Useable;		// Can be used with USE key
	trace_t					trace;
	idStr					strObj;		// Object name ( Npc name, Object description )
	idStr					strAct;		// Action ( "Talk", "Open" and more )
} use_descr_t;
...
public:
	...
	// debug
	// Hilight entity
	void					DebugHilight ( idVec4 &color, float lifetime ) const;
	...
	virtual bool			GetPhysicsToSoundTransform( idVec3 &origin, idMat3 &axis );
							// called from the physics object when colliding, should return CRE_ATREST
	...
	bool					HandleGuiCommands( idEntity *entityGui, const char *cmds, int eg_num );
	virtual bool			HandleSingleGuiCommand( idEntity *entityGui, idLexer *src, int eg_num );
	virtual void			ProjectOntoGUI( idUserInterface *gui ) const;

	// c4tnt: useable items
	virtual bool			ConfirmUseDescr( use_descr_t* Desc );
	virtual void			GUISetUseData( idUserInterface * hud );

	// c4tnt: spot GUIs
	void					Get3DSpotPosition( idVec3& org );
	idUserInterface*		GetSpotGUI( void ) const;
	bool					SetSpotGUI( const char* gui, float elev );

	// targets
	void					FindTargets( void );
	void					RemoveNullTargets( void );
	virtual void			ActivateTargets( idEntity *activator ) const;
	void					ActivateGUIS( ) const;
	idUserInterface*		FindGUI( int index ) const;
...
private:
	...
	idUserInterface*		spotGUI;							// An entity's spot GUI
	float					spotGUIElevation;					// How much is the GUI spot elevated above the origin
	idVec2					spotGUISize;
...
private:
	...
	// events
	void					Event_GetName( idEventReturn *returnContext ); //return s
	void					Event_SetName( const char *name );
	void					Event_FindTargets( void );
	void					Event_ActivateTargets( idEntity *activator );
	void					Event_NumTargets( idEventReturn *returnContext ); //return f
	void					Event_GetTarget( idEventReturn *returnContext, float index ); //return e
	void					Event_AddTarget( idEntity *newtarget );
	void					Event_Untarget( idEntity *target );
	void					Event_RandomTarget( idEventReturn *returnContext, const char *ignore ); //return e
	...
	void					Event_GetShaderParm( idEventReturn *returnContext, int parmnum ); //return f
	...
	void					Event_GetColor( idEventReturn *returnContext ); //return v
	void					Event_IsHidden( idEventReturn *returnContext ); //return d
	void					Event_Hide( void );
	void					Event_Show( void );
	void					Event_CacheSoundShader( const char *soundName );
	void					Event_StartSoundShader( idEventReturn *returnContext, const char *soundName, int channel ); //return f
	void					Event_StopSound( int channel, int netSync );
	void					Event_StartSound( idEventReturn *returnContext, const char *soundName, int channel, int netSync ); //return f
	void					Event_FadeSound( int channel, float to, float over );
	void					Event_GetWorldOrigin( idEventReturn *returnContext ); //return v
	void					Event_SetWorldOrigin( idVec3 const &org );
	void					Event_GetOrigin( idEventReturn *returnContext ); //return v
	...
	void					Event_SetDirection( idVec3 const &Dir, float relAngle, int ordConst );
	void					Event_SetLinearVelocity( const idVec3 &velocity );
	void					Event_GetLinearVelocity( idEventReturn *returnContext ); //return v
	void					Event_SetAngularVelocity( const idVec3 &velocity );
	void					Event_GetAngularVelocity( idEventReturn *returnContext ); //return v
	void					Event_SetSize( const idVec3 &mins, const idVec3 &maxs );
	void					Event_GetSize( idEventReturn *returnContext ); //return v
	void					Event_GetMins( idEventReturn *returnContext ); //return v
	void					Event_GetMaxs( idEventReturn *returnContext ); //return v
	void					Event_Touches( idEventReturn *returnContext, idEntity *ent ); //return d
	void					Event_SetGuiParm( const char *key, const char *val );
	void					Event_SetGuiFloat( const char *key, float f );
	void					Event_GetNextKey( idEventReturn *returnContext, const char *prefix, const char *lastMatch ); //return s
	void					Event_ClearKey( const char *key );
	void					Event_SetKey( const char *key, const char *value );
	void					Event_GetKey( idEventReturn *returnContext, const char *key ); //return s
	void					Event_GetIntKey( idEventReturn *returnContext, const char *key ); //return f
	void					Event_GetFloatKey( idEventReturn *returnContext, const char *key ); //return f
	void					Event_GetVectorKey( idEventReturn *returnContext, const char *key ); //return v
	void					Event_GetEntityKey( idEventReturn *returnContext, const char *key ); //return e
	void					Event_RestorePosition( void );
	void					Event_UpdateCameraTarget( void );
	void					Event_DistanceTo( idEventReturn *returnContext, idEntity *ent ); //return f
	void					Event_DistanceToPoint( idEventReturn *returnContext, const idVec3 &point ); //return f
	void					Event_StartFx( const char *fx );
	void					Event_WaitFrame( void );
	void					Event_Wait( float time );
	void					Event_HasFunction( idEventReturn *returnContext, const char *name ); //return d
	void					Event_CallFunction( const char *name );
	void					Event_SetNeverDormant( int enable );
	void					Event_GetMass( idEventReturn *returnContext, int body ); //return f
	void					Event_IsInLiquid( idEventReturn *returnContext ); //return d

	void					Event_SetGui( int guiNum, const char *guiName);
	void					Event_PrecacheGui( const char *guiName );
	void					Event_GetGuiParm( idEventReturn *returnContext, int guiNum, const char *key ); //return s
	void					Event_GetGuiParmFloat( idEventReturn *returnContext, int guiNum, const char *key ); //return f
	void					Event_GuiNamedEvent( int guiNum, const char *event );
	void					Event_SetGuiParmDirect( int index, const char *key, const char *val );
	void					Event_SetGuiFloatDirect( int index, const char *key, float f );
...
private:
	void					Event_GetJointHandle( idEventReturn *returnContext, const char *jointname ); //return d
	...
	void 					Event_GetJointPos( idEventReturn *returnContext, jointHandle_t jointnum ); //return v
	void 					Event_GetJointAngle( idEventReturn *returnContext, jointHandle_t jointnum ); //return v
};
