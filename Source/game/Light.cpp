...
const idEventDef EV_Light_FadeTo( "fadeToLight", "vf" );
...
CLASS_DECLARATION( idEntity, idLight )
	...
	EVENT( EV_Light_FadeTo,			idLight::Event_FadeTo )
END_CLASS
...
================
*/
void idLight::Save( idSave_I *savefile ) const {
...
================
*/
void idLight::Restore( idRestore_I *savefile ) {
...
================
*/
void idLight::Event_GetLightParm( idEventReturn *returnContext, int parmnum ) {
	if ( ( parmnum < 0 ) || ( parmnum >= MAX_ENTITY_SHADER_PARMS ) ) {
		gameLocal.Error( "shader parm index (%d) out of range", parmnum );
	}

	returnContext->FinallyFloat( renderLight.shaderParms[ parmnum ] );
}
...
/*
================
idLight::Event_FadeTo
================
*/
void idLight::Event_FadeTo( idVec3 &color, float time ) {
	idVec4		fadeColor;
