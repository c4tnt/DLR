...
public :
	...
	void					ExplodeShrapnel( const char *shrapnelDefName, int num_debris, int prj_mode, float prj_cone, const trace_t &collision );
	void					Fizzle( void );
...
protected:
	...
	// Scripted projectile stuff
	bool					scriptProjectile;
	idThread *				scriptThread;
	const function_t		*script_state;
	const function_t		*script_idealState;
...
private:
	...
	void					Event_GetProjectileState( idEventReturn *returnContext ); //return d
	void					Event_SplitProjectile( idEventReturn *returnContext, const char *entityDefName, const idVec3 &offset, const idVec3 &dir );
