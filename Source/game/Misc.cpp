...
================
*/
void idPlayerStart::Save( idSave_I *savefile ) const {
...
================
*/
void idPlayerStart::Restore( idRestore_I *savefile ) {
...
void idPlayerStart::Event_TeleportStage( idEntity *_player ) {
	...
	switch ( teleportStage ) {
		case 0:
			...
			teleportStage++;
			PostEventSec( &EV_TeleportStage, NULL, teleportDelay, player );
			break;
		case 1:
			gameSoundWorld->FadeSoundClasses( 0, 0.0f, 0.25f );
			teleportStage++;
			PostEventSec( &EV_TeleportStage, NULL, 0.25f, player );
			break;
		case 2:
...
void idPlayerStart::TeleportPlayer( idPlayer *player ) {
	...
	if ( f && ent ) {
		...
		if ( !gameLocal.isClient ) {
			player->PostEventSec( &EV_Player_ExitTeleporter, NULL, f );
		}
	} else {
...
================
*/
void idActivator::Save( idSave_I *savefile ) const {
...
================
*/
void idActivator::Restore( idRestore_I *savefile ) {
...
=====================
*/
void idPathCorner::Event_RandomPath( idEventReturn *returnContext ) {
	idPathCorner *path;
...
================
*/
void idDamagable::Save( idSave_I *savefile ) const {
...
================
*/
void idDamagable::Restore( idRestore_I *savefile ) {
...
void idDamagable::BecomeBroken( idEntity *activator ) {
	...
	if ( spawnArgs.GetBool( "hideWhenBroken" ) ) {
		Hide();
		PostEventMS( &EV_RestoreDamagable, NULL, nextTriggerTime - gameLocal.time );
		BecomeActive( TH_THINK );
	}
}
...
void idExplodable::Event_Explode( idEntity *activator ) {
	...
	PostEventMS( &EV_Remove, NULL, 2000 );
...
================
*/
void idForceField::Save( idSave_I *savefile ) const {
...
================
*/
void idForceField::Restore( idRestore_I *savefile ) {
...
void idForceField::Event_Activate( idEntity *activator ) {
	...
	if ( spawnArgs.GetFloat( "wait", "0.01", wait ) ) {
		PostEventSec( &EV_Toggle, NULL, wait );
	}
}
...
================
*/
void idAnimated::Save( idSave_I *savefile ) const {
...
================
*/
void idAnimated::Restore( idRestore_I *savefile ) {
...
void idAnimated::Spawn( void ) {
	...
	if ( wait >= 0 ) {
		PostEventSec( &EV_Activate, NULL, wait, this );
	}
}
...
void idAnimated::PlayNextAnim( void ) {
	...
	if ( len >= 0 ) {
		PostEventMS( &EV_AnimDone, NULL, len, current_anim_index );
	}
...
void idAnimated::Event_Activate( idEntity *_activator ) {
	...
	activated = true;
	activator = _activator;
	ProcessEvent( &EV_Animated_Start, NULL );
}
...
void idAnimated::Event_Start( void ) {
	...
	if ( anim ) {
		...
		len = animator.CurrentAnim( ANIMCHANNEL_ALL )->PlayLength();
		if ( len >= 0 ) {
			PostEventMS( &EV_AnimDone, NULL, len, 1 );
		}
	}
...
void idAnimated::Event_LaunchMissilesUpdate( int launchjoint, int targetjoint, int numshots, int framedelay ) {
	...
	if ( numshots > 0 ) {
		PostEventMS( &EV_LaunchMissilesUpdate, NULL, FRAME2MS( framedelay ), launchjoint, targetjoint, numshots - 1, framedelay );
	}
}
...
void idAnimated::Event_LaunchMissiles( const char *projectilename, const char *sound, const char *launchjoint, const char *targetjoint, int numshots, int framedelay ) {
	...
	ProcessEvent( &EV_LaunchMissilesUpdate, NULL, launch, target, numshots - 1, framedelay );
}

/*
=====================
idAnimated::Event_SetAnimation (ROE)
=====================
*/
...
/*
=====================
idAnimated::Event_GetAnimationLength (ROE)
=====================
*/
void idAnimated::Event_GetAnimationLength( idEventReturn *returnContext ) {
	float length = 0;
	...
	returnContext->FinallyFloat(length);
}
...
===============
*/
void idStaticEntity::Save( idSave_I *savefile ) const {
...
===============
*/
void idStaticEntity::Restore( idRestore_I *savefile ) {
...
===============
*/
void idFuncEmitter::Save( idSave_I *savefile ) const {
...
===============
*/
void idFuncEmitter::Restore( idRestore_I *savefile ) {
...
void idFuncSplat::Event_Activate( idEntity *activator ) {
	...
	PostEventSec( &EV_Splat, NULL, spawnArgs.GetFloat( "splatDelay", "0.25" ) );
...
const idEventDef EV_Shoot( "<shoot>", "" ); 
const idEventDef EV_Reload( "TrapReload", "" ); 
const idEventDef EV_Aim( "Aim", "E" ); 

CLASS_DECLARATION( idEntity, idFuncShooter )
	EVENT( EV_Shoot,		idFuncShooter::Event_Shoottime )
	EVENT( EV_Activate,		idFuncShooter::Event_Activate )
	EVENT( EV_Enable,		idFuncShooter::Event_Enable )
	EVENT( EV_Disable,		idFuncShooter::Event_Disable )
	EVENT( EV_PostSpawn,	idFuncShooter::Event_MatchTarget )
	EVENT( EV_Reload,		idFuncShooter::Event_Reload )
	EVENT( EV_Aim,			idFuncShooter::Event_Aim )
END_CLASS
...
void idFuncShooter::Spawn( void ) {
	...
	{
		gameLocal.Error("projectile not found on a %s", name);
	}

	projectile_spread = spawnArgs.GetFloat( "projectile_spread", "0" );
	num_projectiles = spawnArgs.GetInt( "num_projectiles", "1" );
	attack_accuracy = spawnArgs.GetInt( "attack_accuracy", "0" );

	clip_size = spawnArgs.GetInt( "clip_size", "0" );
	proj_loaded = clip_size;
	if (spawnArgs.GetBool( "start_off", "0" )) {
		state = SHS_DISABLED;
	}else{
		state = SHS_READY;
	}
	spawnArgs.GetFloat( "push", "0", push );
...
/*
=====================
idFuncShooter::Event_MatchTarget
=====================
*/
void idFuncShooter::Event_MatchTarget( void ) {
	int i;
	...
	for( i = 0; i < targets.Num(); i++ ) {
		...
		if ( targetEnt && targetEnt->IsType(idTarget::Type)) {
			break;
		}
	}
...
/*
=====================
idFuncShooter::CreateProjectile
=====================
*/
idProjectile *idFuncShooter::CreateProjectile( const idVec3 &pos, const idVec3 &dir ) {
	idEntity *ent;
	const char *clsname;
...
/*
=====================
idFuncShooter::AimToEntity
=====================
*/
void idFuncShooter::AimToEntity( idEntity* ent ) {
	if (ent == NULL) {
		//Aim towards
		aimDir = GetPhysics()->GetAxis()[0];
	}else{
		//Aim to entity
		aimDir = ent->GetPhysics()->GetOrigin();
		aimDir -= GetPhysics()->GetOrigin();
		aimDir.Normalize();
	}
}
...
=====================
*/
int idFuncShooter::Shoot( int ProjNum ) {
	idProjectile*	Proj;
	idVec3			Start;
	...
	projCount = num_projectiles;
	if (projCount > ProjNum) projCount = ProjNum;
	...
	for( i = 0; i < projCount; i++ ) {
		...
		Proj = CreateProjectile(Start, dir);
		Proj->Launch( Start, dir, dir*push );
		ProjNum--;
		if (ProjNum <= 0) break;
	}

	StartSound( "snd_shoot", SND_CHANNEL_WEAPON, 0, false, NULL );

	return ProjNum - projCount;
}

/*
================
idFuncShooter::Event_Shoottime
================
*/
void idFuncShooter::Event_Shoottime( void ) {
	...
	{
	case SHS_READY:
		//Spin up sound if needed
		StopSound( SND_CHANNEL_ANY, false );
		StartSound( "snd_spinup", SND_CHANNEL_ANY, 0, false, &SndLength );
		if (SndLength > 0) {
			state = SHS_SPINUP;
			PostEventMS( &EV_Shoot, NULL, SndLength ); //Delay for a sound playing
			break;
		}
	case SHS_SHOOT:
		state = SHS_SHOOT;
		proj_loaded = Shoot( proj_loaded );
		if (proj_loaded > 0) {
			//Next shoot
			PostEventSec( &EV_Shoot, NULL, spawnArgs.GetFloat( "shoot_time", "0.25" ) );
		}else{
			StartSound( "snd_reload", SND_CHANNEL_ANY, 0, false, &SndLength );
			state = SHS_RELOAD;
			PostEventSec( &EV_Shoot, NULL, spawnArgs.GetFloat( "reload_time", "0.25" ) + MS2SEC(SndLength) );
		}
		break;
	case SHS_SPINUP:
		StopSound( SND_CHANNEL_ANY, false );
		state = SHS_SHOOT;
		PostEventMS( &EV_Shoot, NULL, 0 ); //Shoot at the next frame
		break;
	case SHS_RELOAD:
		StopSound( SND_CHANNEL_ANY, false );
		proj_loaded = clip_size;
		if (proj_loaded < 1) proj_loaded = 1;
		state = SHS_READY;
		ActPtr = activatorPtr.GetEntity();
		if (!ActPtr)
			ActPtr = this;

		ActivateTargets( ActPtr );
		break;
	case SHS_DISABLED:
		StopSound( SND_CHANNEL_ANY, false );
		StartSound( "snd_disabled", SND_CHANNEL_ANY, 0, false, NULL );
		//Disabled sound
		break;
	default:
		break;
...
void idFuncShooter::Event_Activate( idEntity *activator ) {
	if (gameLocal.GetTime() > nextTriggerTime) {
		...
		if (state == SHS_READY) {
			...
			if (delay > 0) {
				PostEventSec( &EV_Shoot, NULL, delay );
			}else{
				Event_Shoottime ();
			}
		}
...
/*
================
idFuncShooter::Event_Enable
================
*/
void idFuncShooter::Event_Enable( void ) {
	state = SHS_READY;
	idEvent::CancelEvents( this, &EV_Shoot );
}
/*
================
idFuncShooter::Event_Disable
================
*/
void idFuncShooter::Event_Disable( void ) {
	state = SHS_DISABLED;
	idEvent::CancelEvents( this, &EV_Shoot );
}
/*
================
idFuncShooter::Event_Reload
================
*/
void idFuncShooter::Event_Reload( void ) {
int SndLength = 0;
	
	if (state == SHS_SHOOT || state == SHS_SPINUP) {
		StartSound( "snd_reload", SND_CHANNEL_ANY, 0, false, &SndLength );
		state = SHS_RELOAD;
		idEvent::CancelEvents( this, &EV_Shoot );
		PostEventSec( &EV_Shoot, NULL, spawnArgs.GetFloat( "reload_time", "0.25" ) + MS2SEC(SndLength) );
...
/*
================
idFuncShooter::Event_Aim
================
*/
void idFuncShooter::Event_Aim( idEntity* to ) {
	AimToEntity( to );
}
...
===============
*/
void idFuncSmoke::Save(	idSave_I *savefile ) const {
...
===============
*/
void idFuncSmoke::Restore( idRestore_I *savefile ) {
...
================
*/
void idTextEntity::Save( idSave_I *savefile ) const {
...
================
*/
void idTextEntity::Restore( idRestore_I *savefile ) {
...
================
*/
void idVacuumSeparatorEntity::Save( idSave_I *savefile ) const {
...
================
*/
void idVacuumSeparatorEntity::Restore( idRestore_I *savefile ) {
	int state;
...
void idVacuumEntity::Spawn() {
	...
	int area = gameRenderWorld->PointInArea( org );
	gameLocal.vacuumAreas.AddUnique( area );
...
===============
*/
void idBeam::Save( idSave_I *savefile ) const {
...
===============
*/
void idBeam::Restore( idRestore_I *savefile ) {
...
===============
*/
void idShaking::Save( idSave_I *savefile ) const {
	savefile->WriteBool( active );
...
===============
*/
void idShaking::Restore( idRestore_I *savefile ) {
	savefile->ReadBool( active );
...
===============
*/
void idEarthQuake::Save( idSave_I *savefile ) const {
	savefile->WriteInt( nextTriggerTime );
...
===============
*/
void idEarthQuake::Restore( idRestore_I *savefile ) {
	savefile->ReadInt( nextTriggerTime );
...
void idEarthQuake::Spawn( void ) {
	...
	if ( !triggered ){
		PostEventSec( &EV_Activate, NULL, spawnArgs.GetFloat( "wait" ), this );
	}
	BecomeInactive( TH_THINK );
}
...
void idEarthQuake::Event_Activate( idEntity *activator ) {
	...
	if ( !triggered && activator != this ){
		...
		if (disabled) {
			...
			PostEventSec( &EV_Activate, NULL, wait + random * gameLocal.random.CRandomFloat(), this );
		}
	}
...
===============
*/
void idFuncPortal::Save( idSave_I *savefile ) const {
	savefile->WriteInt( (int)portal );
...
===============
*/
void idFuncPortal::Restore( idRestore_I *savefile ) {
	savefile->ReadInt( (int &)portal );
...
===============
*/
void idFuncAASPortal::Save( idSave_I *savefile ) const {
	savefile->WriteBool( state );
}
...
===============
*/
void idFuncAASPortal::Restore( idRestore_I *savefile ) {
	savefile->ReadBool( state );
...
===============
*/
void idFuncAASObstacle::Save( idSave_I *savefile ) const {
	savefile->WriteBool( state );
}
...
===============
*/
void idFuncAASObstacle::Restore( idRestore_I *savefile ) {
	savefile->ReadBool( state );
...
===============
*/
void idFuncRadioChatter::Save( idSave_I *savefile ) const {
...
===============
*/
void idFuncRadioChatter::Restore( idRestore_I *savefile ) {
...
void idFuncRadioChatter::Event_Activate( idEntity *activator ) {
	...
	PostEventSec( &EV_ResetRadioHud, NULL, time, player );
...
===============
*/
void idPhantomObjects::Save( idSave_I *savefile ) const {
	int i;
...
===============
*/
void idPhantomObjects::Restore( idRestore_I *savefile ) {
	int num;
	int i;
...
void idPhantomObjects::Think( void ) {
	...
	targetEnt = target.GetEntity();
	if ( !targetEnt || ( targetEnt->health <= 0 ) || ( end_time && ( gameLocal.time > end_time ) ) || gameLocal.cinematicState != CIN_NO ) {
		BecomeInactive( TH_THINK );
	}
