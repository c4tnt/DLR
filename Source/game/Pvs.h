...
typedef enum {
	PHT_DONTCARE			= 0,	// Removes hint
	PHT_INVISIBLE			= 1,	// Target invisible from source
	PHT_VISIBLE				= 2		// Target visible from source 
} phtType_t;
...
public:
	...
	qhandle_t			AreaPitHint( int Area );												// all backward portals will blocked
	qhandle_t			AreaHint( int srcArea, int dstArea, phtType_t type );					// sets visibility from one area to another
	qhandle_t			AreaPortalHint( int srcArea, qhandle_t dstPortal, phtType_t type );		// sets visibility from area to portal
	qhandle_t			OccluderHint( srfTriangles_t surfTris, phtType_t type );				// adds new portal occluder

	phtType_t			GetHint( qhandle_t handle );
	bool				UpdateHint( qhandle_t handle, phtType_t newType );
	bool				ClearHint( qhandle_t handle );
