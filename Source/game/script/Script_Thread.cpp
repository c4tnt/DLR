...
const idEventDef EV_Thread_SetCallback( "<script_setcallback>", NULL, 'd' );
...
const idEventDef EV_Thread_SetSkillVar( "setSkillVar", "ss" );//c4tnt: NEW!
const idEventDef EV_Thread_GetSkillVar( "getSkillVar", "ss", 's' );//c4tnt: NEW!
...
const idEventDef EV_Thread_ResetSpawnArgs( "resetSpawnArgs" );//c4tnt: NEW!
const idEventDef EV_Thread_SelectSpawnDef( "selectSpawnDef", "s" );//c4tnt: NEW!
...
const idEventDef EV_Thread_Vector( "Vector", "fff", 'v' );	//c4tnt: NEW!
const idEventDef EV_Thread_VecLerp( "VecLerp", "vvf", 'v' ); //c4tnt: NEW!
const idEventDef EV_Thread_VecProjectPlane( "VecProjectPlane", "vv", 'v' ); //c4tnt: NEW!
const idEventDef EV_Thread_VecTrunc( "VecTrunc", "vf", 'v' ); //c4tnt: NEW!
const idEventDef EV_Thread_VecSnap( "VecSnap", "v", 'v' ); //c4tnt: NEW!
...
const idEventDef EV_Thread_ChangeMusic( "changeMusic", "sf" );
...
const idEventDef EV_Thread_HUDMSG_All( "messageAll", "svvf" );
const idEventDef EV_Thread_Text_Align( "messageAlign", "f" );
const idEventDef EV_Thread_Text_Scale( "messageScale", "f" );
const idEventDef EV_Thread_Text_Spacing( "messageSpacing", "f" );
const idEventDef EV_Thread_Text_Font( "messageFont", "s" );
const idEventDef EV_Thread_Text_Color( "messageColor", "vf" );

const idEventDef EV_Thread_InfluenceActive( "influenceActive", NULL, 'd' );
const idEventDef EV_Thread_Luck( "luck", "f", 'd' );
const idEventDef EV_Thread_SpawnParticle( "spawnParticle", "sv" );
const idEventDef EV_Thread_SetGameoverSeq( "setGameoverScript", "s" );
...
CLASS_DECLARATION( idClass, idThread )
	...
	EVENT( EV_Thread_SetSkillVar,			idThread::Event_SetSkillvar )
	EVENT( EV_Thread_GetSkillVar,			idThread::Event_GetSkillvar )
	...
	EVENT( EV_Thread_ResetSpawnArgs,		idThread::Event_ResetSpawnArgs )
	EVENT( EV_Thread_SelectSpawnDef,		idThread::Event_SelectSpawnDef )
	...
	EVENT( EV_Thread_Vector,				idThread::Event_Vector )
	EVENT( EV_Thread_VecLerp,				idThread::Event_VecLerp )
	EVENT( EV_Thread_VecProjectPlane,		idThread::Event_VecProjectPlane )
	EVENT( EV_Thread_VecTrunc,				idThread::Event_VecTrunc )
	EVENT( EV_Thread_VecSnap,				idThread::Event_VecSnap )
	...
	EVENT( EV_Thread_ChangeMusic,			idThread::Event_ChangeMusic )
	...
	EVENT( EV_Thread_Luck,					idThread::Event_Luck )
	EVENT( EV_Thread_HUDMSG_All,			idThread::Event_HUDMSG_All )
	EVENT( EV_Thread_Text_Align,			idThread::Event_Text_Align )
	EVENT( EV_Thread_Text_Scale,			idThread::Event_Text_Scale )
	EVENT( EV_Thread_Text_Spacing,			idThread::Event_Text_Spacing )
	EVENT( EV_Thread_Text_Font,				idThread::Event_Text_Font )
	EVENT( EV_Thread_Text_Color,			idThread::Event_Text_Color )
	EVENT( EV_Thread_SpawnParticle,			idThread::Event_SpawnParticle )
	EVENT( EV_Thread_SetGameoverSeq,		idThread::Event_SetGameoverSeq )
END_CLASS
...
idThread::idThread( const idScriptContext *scon, const char* threadName) {
	assert( scon );
...
================
*/
void idThread::Save( idSave_I *savefile ) const {
...
================
*/
void idThread::Restore( idRestore_I *savefile ) {
...
void idThread::DelayedStart( int delay ) {
	...
	PostEventMS( &EV_Thread_Execute, NULL, delay );
}
...
bool idThread::Execute( void ) {
	...
	if ( done ) {
		...
		if ( waitingUntil > lastExecuteTime ) {
			PostEventMS( &EV_Thread_Execute, NULL, waitingUntil - lastExecuteTime );
		} else if ( interpreter.MultiFrameEventInProgress() ) {
			PostEventMS( &EV_Thread_Execute, NULL, gameLocal.msec );
		}
	}
...
================
*/
void	idThread::ParamI( const int &parm ) {
	interpreter.Push( parm );
}

void	idThread::ParamF( const float &parm ) {
	interpreter.Push( parm );
}

void	idThread::ParamS( const char* parm ) {
	interpreter.PushString( parm );
}

void	idThread::ParamV( const idVec3 &parm ) {
	interpreter.Push( parm );
}

void	idThread::ParamE( const idEntity *parm ) {
	if ( !parm ) {
		interpreter.Push( (int)0 );
	} else {
		interpreter.Push( (int)(parm->entityNumber + 1) );
	}
}

bool idThread::CompileParamD( const idDict &parms, const idTypeDef* func_type ) {
int argc;
int argn;
const char* argname;
...
const idEntity* ent;
	...
	for (argn = 0; argn < argc; argn++) {
		...
		argname = func_type->GetParmName(argn);
		switch ( argtype->Type() )
		{
		case ev_boolean:
			ParamI(parms.GetBool(argname));
			break;
		case ev_vector:
			ParamV(parms.GetVector(argname));
			break;
		case ev_float:
			ParamF(parms.GetFloat(argname));
			break;
		case ev_string:
			ParamS(parms.GetString(argname));
			break;
		case ev_entity:
			ent = gameLocal.FindEntity(parms.GetString(argname));
			ParamE(ent);
			break;
		default:
			gameLocal.Error( "idThread::CompileParamD unsupported function parameter type" );
		}
	}
	return true;
}
...
idEventReturn* RC;
	...
	if ( ent && ent->RespondsTo( EV_Thread_SetCallback ) ) {
		RC = idEventReturn::Alloc( this );
		ent->ProcessEvent( &EV_Thread_SetCallback, RC );
		if ( RC->GetReturnedInt() ) {
			Pause();
			waitingFor = ent->entityNumber;
		}
		RC->Checkout( EVRS_HANDLED );
	}
}
...
void idThread::Event_Trigger( idEntity *ent ) {
	if ( ent ) {
		...
		ent->ProcessEvent( &EV_Activate, NULL, gameLocal.GetLocalPlayer() );
		ent->TriggerGuis();
	}
}
...
================
*/
void idThread::Event_GetCvar( idEventReturn *returnContext, const char *name ) const {
	returnContext->FinallyString( cvarSystem->GetCVarString( name ) );
...
/*
================
idThread::Event_SetSkillvar
================
*/
void idThread::Event_SetSkillvar( const char *name, const char *value ) const {
	gameLocal.skill.SetKey( name, value );
}

/*
================
idThread::Event_GetSkillvar
================
*/
void idThread::Event_GetSkillvar( idEventReturn *returnContext, const char *name, const char *defstr ) const {
	returnContext->FinallyString( gameLocal.skill.GetKey( name, defstr ) );
}
/*
================
...
================
*/
void idThread::Event_Random( idEventReturn *returnContext, float range ) const {
	float result;

	result = gameLocal.random.RandomFloat();
	returnContext->FinallyFloat( range * result );
...
================
*/
void idThread::Event_GetTime( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( MS2SEC( gameLocal.realClientTime ) );
...
================
*/
void idThread::Event_GetEntity( idEventReturn *returnContext, const char *name ) {
	int			entnum;
	idEntity	*ent;
	...
	if ( name[ 0 ] == '*' ) {
		...
		returnContext->FinallyEntity( gameLocal.entities[ entnum ] );
		
	} else {
		ent = gameLocal.FindEntity( name );
		returnContext->FinallyEntity( ent );
...
================
*/
void idThread::Event_Spawn( idEventReturn *returnContext, const char *classname ) {
	idEntity *ent;
...
/*
================
idThread::Event_SelectSpawnDef
================
*/
void idThread::Event_SelectSpawnDef( const char *classname ) {

	const idDeclEntityDef *def = gameLocal.FindEntityDef( classname, false );

	if ( !def ) {
		Warning( "Unknown classname '%s' on SelectSpawnDef.", classname );
		return;
	}
	spawnArgs.Clear();
...
/*
================
idThread::Event_ResetSpawnArgs
================
*/
void idThread::Event_ResetSpawnArgs( ) {
	spawnArgs.Clear();
}
...
================
*/
void idThread::Event_SpawnString( idEventReturn *returnContext, const char *key, const char *defaultvalue ) {
	const char *result;

	spawnArgs.GetString( key, defaultvalue, &result );
	returnContext->FinallyString( result );
...
================
*/
void idThread::Event_SpawnFloat( idEventReturn *returnContext, const char *key, float defaultvalue ) {
	float result;

	spawnArgs.GetFloat( key, va( "%f", defaultvalue ), result );
	returnContext->FinallyFloat( result );
...
================
*/
void idThread::Event_SpawnVector( idEventReturn *returnContext, const char *key, idVec3 &defaultvalue ) {
	idVec3 result;

	spawnArgs.GetVector( key, va( "%f %f %f", defaultvalue.x, defaultvalue.y, defaultvalue.z ), result );
	returnContext->FinallyVector( result );
...
================
*/
void idThread::Event_GetPersistantString( idEventReturn *returnContext, const char *key ) {
	const char *result;
...
================
*/
void idThread::Event_GetPersistantFloat( idEventReturn *returnContext, const char *key ) {
	float result;
...
================
*/
void idThread::Event_GetPersistantVector( idEventReturn *returnContext, const char *key ) {
	idVec3 result;
...
================
*/
void idThread::Event_AngToForward( idEventReturn *returnContext, idAngles &ang ) {
	returnContext->FinallyVector( ang.ToForward() );
...
================
*/
void idThread::Event_AngToRight( idEventReturn *returnContext, idAngles &ang ) {
	idVec3 vec;

	ang.ToVectors( NULL, &vec );
	returnContext->FinallyVector( vec );
...
================
*/
void idThread::Event_AngToUp( idEventReturn *returnContext, idAngles &ang ) {
	idVec3 vec;
...
================
*/
void idThread::Event_GetSine( idEventReturn *returnContext, float angle ) {
	returnContext->FinallyFloat( idMath::Sin( DEG2RAD( angle ) ) );
...
================
*/
void idThread::Event_GetCosine( idEventReturn *returnContext, float angle ) {
	returnContext->FinallyFloat( idMath::Cos( DEG2RAD( angle ) ) );
...
================
*/
void idThread::Event_GetSquareRoot( idEventReturn *returnContext, float theSquare ) {
	returnContext->FinallyFloat( idMath::Sqrt( theSquare ) );
...
================
*/
void idThread::Event_VecNormalize( idEventReturn *returnContext, idVec3 &vec ) {
	idVec3 n;

	n = vec;
	n.Normalize();
	returnContext->FinallyVector( n );
...
================
*/
void idThread::Event_VecLength( idEventReturn *returnContext, idVec3 &vec ) {
	returnContext->FinallyFloat( vec.Length() );
...
================
*/
void idThread::Event_VecDotProduct( idEventReturn *returnContext, idVec3 &vec1, idVec3 &vec2 ) {
	returnContext->FinallyFloat( vec1 * vec2 );
...
================
*/
void idThread::Event_VecCrossProduct( idEventReturn *returnContext, idVec3 &vec1, idVec3 &vec2 ) {
	returnContext->FinallyVector( vec1.Cross( vec2 ) );
...
================
*/
void idThread::Event_VecToAngles( idEventReturn *returnContext, idVec3 &vec ) {
	idAngles ang = vec.ToAngles();
	returnContext->FinallyVector( idVec3( ang[0], ang[1], ang[2] ) );
}

/*
================
idThread::Event_VecProjectPlane
================
*/
void idThread::Event_VecProjectPlane( idEventReturn *returnContext, idVec3 &vec, idVec3 &normal ) {
	idVec3 result;
	result = vec;
	result.ProjectOntoPlane( normal );
	returnContext->FinallyVector( result );
}

/*
================
idThread::Event_VecTrunc
================
*/
void idThread::Event_VecTrunc( idEventReturn *returnContext, idVec3 &vec, float max ) {
	idVec3 result;
	result = vec;
	result.Truncate( max );
	returnContext->FinallyVector( result );
...
/*
================
idThread::Event_VecSnap
================
*/
void idThread::Event_VecSnap( idEventReturn *returnContext, idVec3 &vec ) {
	idVec3 result;
	result = vec;
	result.Snap();
	returnContext->FinallyVector( result );
...
/*
================
idThread::Event_VecLerp
================
*/
void idThread::Event_VecLerp( idEventReturn *returnContext, idVec3 &zero, idVec3 &one, float part ) {
	idVec3 vec;
	vec.Lerp( zero, one, part );
	returnContext->FinallyVector( vec );
}

/*
================
idThread::Event_Vector
================
*/
void idThread::Event_Vector( idEventReturn *returnContext, float x, float y, float z ) {
	returnContext->FinallyVector( idVec3( x, y, z ) );
}
...
================
*/
void idThread::Event_Trace( idEventReturn *returnContext, const idVec3 &start, const idVec3 &end, const idVec3 &mins, const idVec3 &maxs, int contents_mask, idEntity *passEntity ) {
	...
	returnContext->FinallyFloat( trace.fraction );
...
================
*/
void idThread::Event_TracePoint( idEventReturn *returnContext, const idVec3 &start, const idVec3 &end, int contents_mask, idEntity *passEntity ) {
	gameLocal.clip.TracePoint( trace, start, end, contents_mask, passEntity );
	returnContext->FinallyFloat( trace.fraction );
...
================
*/
void idThread::Event_GetTraceFraction( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( trace.fraction );
...
================
*/
void idThread::Event_GetTraceEndPos( idEventReturn *returnContext ) {
	returnContext->FinallyVector( trace.endpos );
...
================
*/
void idThread::Event_GetTraceNormal( idEventReturn *returnContext ) {
	if ( trace.fraction < 1.0f ) {
		returnContext->FinallyVector( trace.c.normal );
	} else {
		returnContext->FinallyVector( vec3_origin );
	}
...
================
*/
void idThread::Event_GetTraceEntity( idEventReturn *returnContext ) {
	if ( trace.fraction < 1.0f ) {
		returnContext->FinallyEntity( gameLocal.entities[ trace.c.entityNum ] );
	} else {
		returnContext->FinallyEntity( ( idEntity * )NULL );
	}
...
================
*/
void idThread::Event_GetTraceJoint( idEventReturn *returnContext ) {
	if ( trace.fraction < 1.0f && trace.c.id < 0 ) {
		...
		if ( af && af->IsType( idAFEntity_Base::Type ) && af->IsActiveAF() ) {
			returnContext->FinallyString( af->GetAnimator()->GetJointName( CLIPMODEL_ID_TO_JOINT_HANDLE( trace.c.id ) ) );
...
================
*/
void idThread::Event_GetTraceBody( idEventReturn *returnContext ) {
	if ( trace.fraction < 1.0f && trace.c.id < 0 ) {
		idAFEntity_Base *af = static_cast<idAFEntity_Base *>( gameLocal.entities[ trace.c.entityNum ] );
		if ( af && af->IsType( idAFEntity_Base::Type ) && af->IsActiveAF() ) {
			...
			if ( body ) {
				returnContext->FinallyString( body->GetName() );
...
/*
================
idThread::Event_ChangeMusic
================
*/
void idThread::Event_ChangeMusic( const char *text, float fadeout ) {
	gameSoundWorld->PlayShaderDirectly( text );
}
...
================
*/
void idThread::Event_StrLen( idEventReturn *returnContext, const char *string ) {
	int len;

	len = strlen( string );
	returnContext->FinallyInt( len );
...
================
*/
void idThread::Event_StrLeft( idEventReturn *returnContext, const char *string, int num ) {
	int len;
	...
	len = strlen( string );
	if ( len < num ) {
		returnContext->FinallyString( string );
...
================
*/
void idThread::Event_StrRight( idEventReturn *returnContext, const char *string, int num ) {
	int len;
	...
	returnContext->FinallyString( string + len - num );
...
================
*/
void idThread::Event_StrSkip( idEventReturn *returnContext, const char *string, int num ) {
	int len;
	...
	returnContext->FinallyString( string + num );
}
...
================
*/
void idThread::Event_StrMid( idEventReturn *returnContext, const char *string, int start, int num ) {
	int len;
...
================
*/
void idThread::Event_StrToFloat( idEventReturn *returnContext, const char *string ) {
	float result;
...
================
*/
void idThread::Event_IsClient( idEventReturn *returnContext ) { 
	
	returnContext->FinallyFloat( gameLocal.isClient );
}
...
================
*/
void idThread::Event_IsMultiplayer( idEventReturn *returnContext ) { 
	
	returnContext->FinallyFloat( gameLocal.isMultiplayer );
}
...
================
*/
void idThread::Event_GetFrameTime( idEventReturn *returnContext ) { 
	
	returnContext->FinallyFloat( MS2SEC( gameLocal.msec ) );
}
...
================
*/
void idThread::Event_GetTicsPerSecond( idEventReturn *returnContext ) { 
	
	returnContext->FinallyFloat( USERCMD_HZ );
}
...
================
*/
void idThread::Event_InfluenceActive( idEventReturn *returnContext ) {
	idPlayer *player;

	player = gameLocal.GetLocalPlayer();
	if ( player && player->GetInfluenceLevel() ) {
		returnContext->FinallyInt( true );
	} else {
		returnContext->FinallyInt( false );
	}
...
================
*/
void idThread::Event_Luck( idEventReturn *returnContext, const float chance ) {
	if ( gameLocal.random.Chance( chance ) ) {
		returnContext->FinallyInt( true );
	} else {
		returnContext->FinallyInt( false );
	}
...
/*
================
idThread::Event_HUDMSG_All
================
*/
void idThread::Event_HUDMSG_All( const char *text, const idVec3 &origin, const idVec3 &box, const float lifetime ) {
	idVec4 _color;
	const char* fontname;
	float scale;
	float spacing;
	...
	fontname = textArgs.GetString( "font", "" );
	textArgs.GetVec4( "color", "1 1 1 1", _color );
	scale = textArgs.GetFloat( "scale", "1" );
	spacing = textArgs.GetFloat( "spacing", "0" );
	align = textArgs.GetInt( "align", "0" );

	gameLocal.PushHudMessage( origin, box, _color, scale, spacing, align, text, fontname, SEC2MS(lifetime) );
}

/*
================
idThread::Event_Text_Align
================
*/
void idThread::Event_Text_Align( const float align ) {
	textArgs.SetInt( "align", align );
}

/*
================
idThread::Event_Text_Scale
================
*/
void idThread::Event_Text_Scale( const float scale ) {
	textArgs.SetFloat( "scale", scale );
}

/*
================
idThread::Event_Text_Spacing
================
*/
void idThread::Event_Text_Spacing( const float spacing ) {
	textArgs.SetFloat( "spacing", spacing );
}

/*
================
idThread::Event_Text_Font
================
*/
void idThread::Event_Text_Font( const char* font ) {
	textArgs.Set( "font", font );
}

/*
================
idThread::Event_Text_Color
================
*/
void idThread::Event_Text_Color( const idVec3 &color, const float _alpha ) {
	textArgs.SetVec4( "color", idVec4(color.x, color.y, color.z, _alpha) );
}
...
void idThread::Event_SpawnParticle( const char *particleName, const idVec3 &origin ) {
	const idDecl *decl;
	decl = declManager->FindType( DECL_PARTICLE, particleName );
	if ( !decl || decl->GetType() != DECL_PARTICLE ) {
		return;
	}

	gameLocal.smokeParticles->EmitSmoke( static_cast<const idDeclParticle *>( decl ), gameLocal.time, gameLocal.random.RandomFloat(), origin, idMat3( 1,0,0,0,0,0,0,0,0 ) );
}

/*
================
idThread::Event_SetGameoverSeq
================
*/
void idThread::Event_SetGameoverSeq( const char *funcName ) {
	gameLocal.SetGameOverSequence( funcName );
}
