...
================
*/
impactEffect_t idPhysics_RigidBody::CollisionImpulse( const trace_t &collision, idVec3 &impulse ) {
...
void idPhysics_RigidBody::ContactFriction( float deltaTime ) {
	...
	idVec3 massCenter, r, velocity, sidevelocity, normal, impulse, normalVelocity;
	idEntity* ent;
	...
	for ( i = 0; i < contacts.Num(); i++ ) {
		...
		if ( ent && ent != self ) {
			sidevelocity = ent->GetPhysics()->PointVelocity( contacts[i].id, contacts[i].point );
		} else {
			sidevelocity = vec3_origin;
		}

		// calculate velocity at contact point
		linearVelocity = inverseMass * current.i.linearMomentum;
		angularVelocity = inverseWorldInertiaTensor * current.i.angularMomentum;
		velocity = linearVelocity + angularVelocity.Cross(r) - sidevelocity;
...
idEntity *e;
	...
	if ( rb_showContacts.GetBool() ) {
		for ( i = contactEntities.Num() - 1; i >= 0; i-- ) {
			e = contactEntities[i].GetEntity();
			if ( e ) {
				if ( e->IsAtRest() ) {
					contactEntities[i].GetEntity()->DebugHilight( idVec4( 1.0f, 1.0f, 0.0f, 0.5f ), 0 );
				} else {
					contactEntities[i].GetEntity()->DebugHilight( idVec4( 0.0f, 1.0f, 0.0f, 0.5f ), 0 );
				}
			}
		}
	}
...
//	integrator = new idODE_RK4Adaptive( sizeof(rigidBodyIState_t) / sizeof(float), RigidBodyDerivatives, this );
//	integrator = new idODE_RK4( sizeof(rigidBodyIState_t) / sizeof(float), RigidBodyDerivatives, this );
//	integrator = new idODE_Midpoint( sizeof(rigidBodyIState_t) / sizeof(float), RigidBodyDerivatives, this );
...
================
*/
void idPhysics_RigidBody_SavePState( idSave_I *savefile, const rigidBodyPState_t &state ) {
	byte fl;
	...
	fl = 0;
	if ( state.pushVelocity != vec6_zero ) fl += 1;
	if ( state.externalForce != vec3_zero ) fl += 2;
	if ( state.externalTorque != vec3_zero ) fl += 4;

	savefile->WriteByte( fl );

	if ( fl & 1 ) savefile->WriteVec6( state.pushVelocity );
	if ( fl & 2 ) savefile->WriteVec3( state.externalForce );
	if ( fl & 4 ) savefile->WriteVec3( state.externalTorque );
...
================
*/
void idPhysics_RigidBody_RestorePState( idRestore_I *savefile, rigidBodyPState_t &state ) {
	byte fl;
	...
	savefile->ReadByte( fl );

	if ( fl & 1 ) savefile->ReadVec6( state.pushVelocity );
	if ( fl & 2 ) savefile->ReadVec3( state.externalForce );
	if ( fl & 4 ) savefile->ReadVec3( state.externalTorque );
...
================
*/
void idPhysics_RigidBody::Save( idSave_I *savefile ) const {
...
================
*/
void idPhysics_RigidBody::Restore( idRestore_I *savefile ) {
...
/*
================
idPhysics_RigidBody::GetBouncyness
================
*/
float idPhysics_RigidBody::GetBouncyness( ) {
	return bouncyness;
}
...
bool idPhysics_RigidBody::Evaluate( int timeStepMSec, int endTimeMSec ) {
	...
	rigidBodyPState_t c_next; //Collided next
	idAngles angles;
	trace_t collision;
	idVec3 impulse;
	idEntity *ent;
...
//	current.i.angularMomentum -= current.pushVelocity.SubVec3( 1 ) * inertiaTensor;
	...
	if (collided) {
		impact = CollisionImpulse( collision, impulse );
		switch ( impact ) {
		case IMP_REST:
		case IMP_TOUCH:
			Rest();
	...
	if ( collided ) {
		...
		if ( ent && ( !cameToRest || !ent->IsAtRest() ) ) {
			...
			if ( impact == IMP_BOUNCE || impact == IMP_TOUCH ) {
...
/*
================
idPhysics_RigidBody::ApplyImpulse2
================
*/
void idPhysics_RigidBody::ApplyImpulse2( const int id, const idVec3 &point, const idVec3 &impulse, const float dstA, const float dstB ) {
	if ( noImpact ) {
		return;
	}
	current.i.linearMomentum += impulse * dstA;
	current.i.angularMomentum += ( point - ( current.i.position + centerOfMass * current.i.orientation ) ).Cross( impulse * dstB);
	Activate();
}
...
/*
================
idPhysics_RigidBody::PointVelocity
================
*/
const idVec3 &idPhysics_RigidBody::PointVelocity( int id, const idVec3 &pt ) const {
	static idVec3 cLinearVelocity;
	idVec3 cAngularVelocity;
	idMat3 inverseWorldInertiaTensor;

	cLinearVelocity = current.i.linearMomentum * inverseMass;
	inverseWorldInertiaTensor = current.i.orientation.Transpose() * inverseInertiaTensor * current.i.orientation;
	cAngularVelocity = inverseWorldInertiaTensor * current.i.angularMomentum;

	cLinearVelocity += cAngularVelocity.Cross( pt - current.i.position - centerOfMass * current.i.orientation);
	return cLinearVelocity;
}
