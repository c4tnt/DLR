...
const float PM_SLICKFRICTION	= 0.5f;
...
void idPhysics_Player::Accelerate( const idVec3 &wishdir, const float wishspeed, const float accel ) {
	...
	if ( gameLocal.skill.GetBoolKey( "strafeRun", "1" ) ) {
		// q2 style
...
void idPhysics_Player::Friction( void ) {
	...
	// spectator friction
	if ( current.movementType == PM_SPECTATOR || current.movementType == PM_FLYING ) {
	...
	// apply ground friction
		...
		if ( !( current.movementFlags & PMF_TIME_KNOCKBACK ) ) {
			if ( groundMaterial && groundMaterial->GetSurfaceFlags() & SURF_SLICK ) {
				...
				drop += control * PM_SLICKFRICTION * frametime;
			} else {
				control = speed < PM_STOPSPEED ? PM_STOPSPEED : speed;
	...
	if ( current.movementType == PM_DEAD ) {
		...
		if ( command.upmove < 0 && !ladder ) {
			...
			if ( current.movementFlags & PMF_DUCKED ) {
				// try to stand up
				end = current.origin - ( PlayerHeight[NORMALHEIGHT] - PlayerHeight[CROUCHHEIGHT] ) * gravityNormal;
				gameLocal.clip.Translation( trace, current.origin, end, clipModel, clipModel->GetAxis(), clipMask, self );
				if ( trace.fraction >= 1.0f ) {
					current.movementFlags &= ~PMF_DUCKED;
				}
		...
		if ( current.movementFlags & PMF_DUCKED ) {
			...
			maxZ = PlayerHeight[CROUCHHEIGHT];
		} else {
			maxZ = PlayerHeight[NORMALHEIGHT];
		}
	}
...
void idPhysics_Player::MovePlayer( int msec ) {
	...
	if ( current.movementType == PM_DEAD ) {
		...
		// airborne
		if ( current.movementType == PM_FLYING ) {
			idPhysics_Player::FlyMove();
		} else {
			idPhysics_Player::AirMove();
		}
...
================
*/
void idPhysics_Player_SavePState( idSave_I *savefile, const playerPState_t &state ) {
...
================
*/
void idPhysics_Player_RestorePState( idRestore_I *savefile, playerPState_t &state ) {
...
================
*/
void idPhysics_Player::Save( idSave_I *savefile ) const {
int i;
	...
	for ( i = 0; i < 3; i++ )
		savefile->WriteFloat( PlayerHeight[i] );
...
================
*/
void idPhysics_Player::Restore( idRestore_I *savefile ) {
int i;
	...
	for ( i = 0; i < 3; i++ )
		savefile->ReadFloat( PlayerHeight[i] );
...
================
*/
void idPhysics_Player::SetPlayerHeight( int parm, const float newHeight ) {
	PlayerHeight[parm] = newHeight;
}
...
================
*/
float idPhysics_Player::GetPlayerHeight( int parm ) const {
	return PlayerHeight[parm];
}
