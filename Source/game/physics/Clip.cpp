...
void idClipModel::FreeTraceModel( int traceModelIndex ) {
	if ( traceModelIndex < 0 || traceModelIndex >= traceModelCache.Num() || traceModelCache[traceModelIndex]->refCount <= 0 ) {
		gameLocal.Warning( "idClipModel::FreeTraceModel: tried to free uncached trace model (idx = %d)", traceModelIndex);
		return;
	}
...
===============
*/
void idClipModel::SaveTraceModels( idSave_I *savefile ) {
	int i;
...
===============
*/
void idClipModel::RestoreTraceModels( idRestore_I *savefile ) {
	int i, num;
...
================
*/
void idClipModel::Save( idSave_I *savefile ) const {
...
================
*/
void idClipModel::Restore( idRestore_I *savefile ) {
...
/*
============
idClip::TranslationSingle
============
*/
bool idClip::TranslationSingle( trace_t &results, const idVec3 &start, const idVec3 &end, idClipModel *traceFor,
	...
	if ( traceFor ) {
		if ( traceFor->renderModelHandle != -1 ) {
			idClip::numRenderModelTraces++;
			TraceRenderModel( results, start, end, radius, trmAxis, traceFor );
		} else {
			idClip::numTranslations++;
			collisionModelManager->Translation( &results, start, end, trm, trmAxis, contentMask,
									traceFor->Handle(), traceFor->origin, traceFor->axis );
		}
	} else {
		idClip::numTranslations++;
		collisionModelManager->Translation( &results, start, end, trm, trmAxis, contentMask, 0, vec3_origin, mat3_default );
