...
================
*/
void idPhysics_Static::Save( idSave_I *savefile ) const {
	byte fl;

	savefile->WriteObject( self );

	fl = 0;
	if ( current.axis.IsIdentity() ) fl |= 1;
	if ( current.localAxis.IsIdentity() ) fl |= 2;
	if ( current.origin == current.localOrigin) fl |= 4;
	if ( current.axis == current.localAxis) fl |= 8;

	savefile->WriteByte( fl );
	savefile->WriteVec3( current.origin );
	if (! (fl&1) ) savefile->WriteMat3( current.axis );
	if (! (fl&4) ) savefile->WriteVec3( current.localOrigin );
	if (! (fl&10) ) savefile->WriteMat3( current.localAxis );
...
================
*/
void idPhysics_Static::Restore( idRestore_I *savefile ) {
	byte fl;
...
/*
================
idPhysics_Static::GetSelf
================
*/
idEntity * idPhysics_Static::GetSelf( void ) {
	return self;
}
...
/*
================
idPhysics_Static::PointVelocity
================
*/
const idVec3 &idPhysics_Static::PointVelocity( int id, const idVec3 &pt ) const {
	return vec3_origin;
}
