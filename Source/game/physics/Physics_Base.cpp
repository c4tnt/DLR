...
================
*/
void idPhysics_Base::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idPhysics_Base::Restore( idRestore_I *savefile ) {
	int i, num;
...
/*
================
idPhysics_Base::PointVelocity
================
*/
const idVec3 &idPhysics_Base::PointVelocity( int id, const idVec3 &pt ) const {
	return vec3_origin;
}
...
int i;
	...
	if( this->water == NULL ) {
		...
		if( gameLocal.clip.EntitiesTouchingBounds(bounds,MASK_WATER,e,4) ) {	//
			for ( i = 0; i < 4; i++ ) {
				if( e[i]->GetPhysics()->IsType(idPhysics_Liquid::Type) ) {
					this->water = static_cast<idPhysics_Liquid *>(e[i]->GetPhysics());
					return 1.0f;
				}
			}
		}
