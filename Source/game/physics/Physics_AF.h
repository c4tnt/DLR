...
public:
	...
	virtual void			Save( idSave_I *saveFile ) const;
	virtual void			Restore( idRestore_I *saveFile );
...
public:
	...
	void					SetSuspension( const float up, const float down, const float k, const float d, const float f, const float cl );
...
protected:
	...
	float					suspensionClamper;			// Suspension low limiter
	float					steerAngle;					// desired steer angle in degrees
...
public:
	...
	void					UpdateClipModels( void );
		...
		// buoyancy stuff WaterPhysics 1.0
...
private:
	...
	impactEffect_t			CollisionImpulse( float timeStep, idAFBody *body, trace_t &collision );
