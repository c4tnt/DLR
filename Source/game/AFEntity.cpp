...
bool idAFAttachment::ConfirmUseDescr( use_descr_t* Desc ) {
bool result;

	if ( !body )
		return false;

	result =  body->ConfirmUseDescr( Desc );
	if ( result ) {
		Desc->ent = body;
	}
	return result;
...
================
*/
void idAFAttachment::Save( idSave_I *savefile ) const {
...
================
*/
void idAFAttachment::Restore( idRestore_I *savefile ) {
...
================
*/
void idAFEntity_Base::Save( idSave_I *savefile ) const {
...
================
*/
void idAFEntity_Base::Restore( idRestore_I *savefile ) {
...
================
*/
impactEffect_t idAFEntity_Base::Collide( const trace_t &collision, const idVec3 &velocity ) {
	float v, f;
...
================
*/
void idAFEntity_Gibbable::Save( idSave_I *savefile ) const {
...
================
*/
void idAFEntity_Gibbable::Restore( idRestore_I *savefile ) {
	bool hasCombatModel;
...
void idAFEntity_Gibbable::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, const char *damageDefName, const float damageScale, const int location ) {
	...
	if ( health < -spawnArgs.GetInt( "gib_health_treshold", "20" ) && spawnArgs.GetBool( "gib" ) ) {
...
void idAFEntity_Gibbable::SpawnGibs( const idVec3 &dir, const char *damageDefName ) {
	...
	for ( i = 0; i < list.Num(); i++ ) {
		...
		list[i]->PostEventSec( &EV_Remove, NULL, 4.0f );
	}
}
...
void idAFEntity_Gibbable::Gib( const idVec3 &dir, const char *damageDefName ) {
	...
	PostEventSec( &EV_Gibbed, NULL, 4.0f );
}
...
================
*/
void idAFEntity_Generic::Save( idSave_I *savefile ) const {
...
================
*/
void idAFEntity_Generic::Restore( idRestore_I *savefile ) {
...
void idAFEntity_Generic::Event_Activate( idEntity *activator ) {
	...
	if ( delay == 0.0f ) {
		...
		PostEventSec( &EV_SetLinearVelocity, NULL, delay, init_velocity );
	}
	...
	if ( delay == 0.0f ) {
		...
		PostEventSec( &EV_SetAngularVelocity, NULL, delay, init_avelocity );
	}
}
...
================
*/
void idAFEntity_WithAttachedHead::Save( idSave_I *savefile ) const {
	head.Save( savefile );
}
...
================
*/
void idAFEntity_WithAttachedHead::Restore( idRestore_I *savefile ) {
...
CLASS_DECLARATION( idAFEntity_Base, idAFEntity_Vehicle )
	EVENT( EV_Use,	idAFEntity_Vehicle::Event_Use )
END_CLASS
...
/*
================
idAFEntity_Vehicle::ConfirmUseDescr
================
*/
bool idAFEntity_Vehicle::ConfirmUseDescr( use_descr_t* Desc )
{
	if (spawnArgs.GetBool( "nouse", "0"))
		return false;	// Not useable

	Desc->CursorType = CURTYPE_VEHICLE;
	Desc->ent = this;
	Desc->ShowInCursor = spawnArgs.GetBool( "use_cursor", "1");
	Desc->ShowInHud = spawnArgs.GetBool( "use_hud", "1");
	Desc->LowerWeapon = (player == NULL);	//Allow player use weapon inside car
	spawnArgs.GetString( "use_act", "Drive", Desc->strAct);
	spawnArgs.GetString( "use_obj", "Vehicle", Desc->strObj);
...
/*
================
idAFEntity_Vehicle::Event_Use
by c4tnt
================
*/
void idAFEntity_Vehicle::Event_Use( idEntity *other, trace_t *trace ) {

	if (other->IsType( idPlayer::Type )) //now only player-drive support. In future idAI will enter the transport same method.
	{
		idPlayer* pl = static_cast< idPlayer * >( other );

		if ( pl->GetBindMaster() && pl->GetBindMaster()->IsType( idAFEntity_Vehicle::Type ) ) {
			pl->Show();
			static_cast<idAFEntity_Vehicle*>(pl->GetBindMaster())->Use( pl );
		} else {
			pl->Hide();
			Use( pl );
		}
...
void idAFEntity_VehicleSimple::Spawn( void ) {
	...
	for ( i = 0; i < 4; i++ ) {
		...
		suspension[i]->SetSuspension(	g_vehicleSuspensionUp.GetFloat(),
										...
										g_vehicleSuspensionClamper.GetFloat());
...
================
*/
void idAFEntity_SteamPipe::Save( idSave_I *savefile ) const {
}
...
================
*/
void idAFEntity_SteamPipe::Restore( idRestore_I *savefile ) {
	Spawn();
}
...
================
*/
void idAFEntity_ClawFourFingers::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idAFEntity_ClawFourFingers::Restore( idRestore_I *savefile ) {
	int i;
