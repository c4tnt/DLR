...
===================
*/
void Cmd_Call_f( const idCmdArgs &args ) {
	idScriptContext scon;

	if ( scon.CompileParamS( args.Args() ) ) {
		idThread *thread = new idThread( &scon, "Console JIT" );
		thread->DelayedStart( 0 );
	}else{
		gameLocal.Printf("No execution\n");
	}
...
===================
Cmd_ASMScript_f
c4tnt: Interactive console disasm
===================
*/
void Cmd_ASMScript_f( const idCmdArgs &args ) {
	const char *	script;
	idStr			text;
	idStr			funcname;
	static int		funccount = 0;
	...
	sprintf( funcname, "ConsoleASMFunction_%d", funccount++ );
	if (!idStr::Icmp( args.Argv(1), "file" ))
	{
		if (!idStr::Icmp( args.Argv(2), "x" ) ) {
			script = args.Args(3);
			sprintf( text, "void %s%s;}\n", funcname.c_str(), script );
			if ( gameLocal.program.CompileText( "console", text, true ) ) {
				gameLocal.program.Disassemble( &funcname, false );
			}
		}else{
			script = args.Args(2);
			sprintf( text, "void %s() {%s;}\n", funcname.c_str(), script );
		...
		if (!idStr::Icmp( args.Argv(1), "x" ) ) {
			script = args.Args(1);
			sprintf( text, "void %s%s;}\n", funcname.c_str(), script );
			if ( gameLocal.program.CompileText( "console", text, true ) ) {
				gameLocal.program.Disassemble( &funcname, true );
			}
		}else{
			script = args.Args();
			sprintf( text, "void %s() {%s;}\n", funcname.c_str(), script );
		...
		if ( gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) ) {
			...
			for( i = 0; i < gameLocal.numClients; i++ ) {
				if ( gameLocal.entities[ i ] ) {
					gameLocal.entities[ i ]->PostEventSec( &EV_Player_SelectWeapon, NULL, 0.5f, gameLocal.entities[ i ]->spawnArgs.GetString( "def_weapon1" ) );
				}
			}
		}
	}
...
==================
*/
void Cmd_Fly_f( const idCmdArgs &args ) {
	char		*msg;
	idPlayer	*player;
...
void Cmd_SetViewpos_f( const idCmdArgs &args ) {
	...
	origin.z -= (static_cast<idPhysics_Player*>(player->GetPhysics()))->GetPlayerHeight(NORMALHEIGHT) - 0.25f;
...
void Cmd_Trigger_f( const idCmdArgs &args ) {
	...
	ent->ProcessEvent( &EV_Activate, NULL, player );
	ent->TriggerGuis();
}
...
==================
*/
static void Cmd_DisasmScript_f( const idCmdArgs &args ) { //c4tnt: function names filter added
	idStr fName;
	int fCount;

	if ( args.Argc() > 1 ) {	
		fName = args.Argv( 1 );
		fCount = gameLocal.program.Disassemble(&fName);
	}else{
		fCount = gameLocal.program.Disassemble(NULL);
	}
	gameLocal.Printf( "Disassembled functions: %d\n", fCount );
}
...
===============
*/
void Cmd_OpenFSGui_f( const idCmdArgs &args ) {
	idStr	id;
	int		i;
	idPlayer				*player;
	...
	if ( args.Argc() != 2 ) {
		common->Printf( "usage: openfsgui <index>\n" );
		player->ListFSGui();		
		return;
	}
	i = (int)atof( args.Argv(1) );

	
	if (!player->OpenFSGui(i))
		player->CloseFSGui();
}
...
===============
Cmd_UpdateEntity_f
Forced entity update
===============
*/
void Cmd_UpdateEntity_f( const idCmdArgs &args ) {
	idPlayer		*player;
	...
	end = start + player->viewAngles.ToForward() * 80.0f;
	...
	for ( i = 0; i < listedClipModels; i++ ) {
		...
		// Primary: check all entitys who might be used directly
		gameLocal.clip.TracePoint( trace, start, end, MASK_SHOT_RENDERMODEL, player );
		if ( ( trace.fraction < 1.0f ) && ( trace.c.entityNum == ent->entityNumber ) ) {
			ent->UpdateVisuals();
			ent->Present();
			gameRenderWorld->DebugBounds( idVec4(1,1,1,0), ent->GetPhysics()->GetAbsBounds(), vec3_origin, 1000 );
			continue;
		}
		//SECONDARY: Check gui on the entity

		if ( !ent->GetRenderEntity() ) {
			continue;
		}
...
===============
*/
void Cmd_SetSkillKey_f( const idCmdArgs &args ) {
	idStr	id;
	idPlayer				*player;
	...
	if ( args.Argc() != 3 ) {
		common->Printf( "usage: setSkillKey <Key> <Val>\n" );
		return;
	}
	gameLocal.skill.SetKey( args.Argv(1), args.Argv(2) );
}
...
===============
*/
void Cmd_ListSkillKeys_f( const idCmdArgs &args ) {
	gameLocal.skill.debugPrint();
}

/*
===============
Cmd_ListPersistantKeys_f

List persistant keys

===============
*/
void Cmd_ListPersistantKeys_f( const idCmdArgs &args ) {
int id;
int i;
const idKeyValue *kv;
idDict* d;
	...
	if ( args.Argc() < 2 ) {
		...
		{
			d = &gameLocal.persistentPlayerInfo[gameLocal.localClientNum];
			i = d->GetNumKeyVals();
			if ( !i ) {
			...
			common->Printf( "usage: ListPersistantKeys <playerNum>\n" );
		}
	} else {
		id = (int)atof( args.Argv(1) );
		if ( id >= 0 && id < MAX_CLIENTS ) 
		{
			d = &gameLocal.persistentPlayerInfo[id];
			i = d->GetNumKeyVals();
			if ( !i ) {
				gameLocal.Printf( "    <empty>\n" );
			} else {
...
/*
===============
Cmd_SetPersistantKey_f

Set persistant key

===============
*/
void Cmd_SetPersistantKey_f( const idCmdArgs &args ) {
int id;

	id = gameLocal.localClientNum;

	if ( args.Argc() < 3 ) {
		if ( id >= 0 && id < MAX_CLIENTS ) {
			common->Printf( "usage: SetPersistantKey [player id] <Key> <Val>\n" );
		} else {
			common->Printf( "usage: SetPersistantKey <player id> <Key> <Val>\n" );
		}
		return;
	...
	if ( args.Argc() < 4 ) {
		if ( gameLocal.localClientNum >= 0 && gameLocal.localClientNum < MAX_CLIENTS ) 
		{
			gameLocal.persistentPlayerInfo[gameLocal.localClientNum].Set( args.Argv(1), args.Argv(2) );;
		} else {
			common->Printf( "usage: SetPersistantKey <player id> <Key> <Val>\n" );
		}
	} else {
		...
		if ( id >= 0 && id < MAX_CLIENTS ) 
		{
			gameLocal.persistentPlayerInfo[gameLocal.localClientNum].Set( args.Argv(2), args.Argv(3) );;
		} else {
			common->Printf( "wrong player id\n" );
		}
	}
...
List level persistant keys

===============
*/
void Cmd_ListLPKeys_f( const idCmdArgs &args ) {
int i;
const idKeyValue *kv;

	i = gameLocal.persistentLevelInfo.GetNumKeyVals();
	if ( !i ) {
		gameLocal.Printf( "    <empty>\n" );
	} else {
		while (i > 0) {
			i--;
			kv = gameLocal.persistentLevelInfo.GetKeyVal( i );
			gameLocal.Printf( "    '%s' : '%s' \n", kv->GetKey().c_str(), kv->GetValue().c_str() );
		}
	}
}
...
Set level persistant key

===============
*/
void Cmd_SetLPKey_f( const idCmdArgs &args ) {
int id;

	id = gameLocal.localClientNum;

	if ( args.Argc() < 3 ) {
			common->Printf( "usage: SetLPKey <Key> <Val>\n" );
		return;
	}
	
	gameLocal.persistentLevelInfo.Set( args.Argv(1), args.Argv(2) );;
}
...
===============
*/
void Cmd_ListRels_f( const idCmdArgs &args ) {

	common->Printf( "---------Relation data dump------------\n" );
	gameLocal.relationdata.List();
	common->Printf( "----------------End--------------------\n" );
}
...
===============
*/
void Cmd_ListSpawnids_f( const idCmdArgs &args ) {
int i, en;
	common->Printf( "---------Spawnid data dump------------\n" );
	for ( i = 0; i < MAX_GENTITIES; i++ ) {
		if ( gameLocal.entities[i] ) {
			en = gameLocal.entities[i]->entityNumber;
			common->Printf( "%d : %s %d\n", en, gameLocal.entities[i]->GetName(), gameLocal.spawnIds[en] );
		}
	}
	common->Printf( "----------------End--------------------\n" );
}
...
===============
*/
void Cmd_SetRelation_f( const idCmdArgs &args ) {
int id;
idEntity *A;
idEntity *B;
	...
	if ( args.Argc() < 4 ) {
			common->Printf( "usage: SetRelation <e1> <e2> <enemy/friend/none>\n" );
		return;
	}
	
	A = gameLocal.FindEntity( args.Argv(1) );
	B = gameLocal.FindEntity( args.Argv(2) );

	if ( A == NULL || B == NULL ) return;

	if ( !idStr::Icmp(args.Argv(3),"enemy") ) {
		gameLocal.relationdata.SetRelation( A,B,R_ENEMY );
	} else if ( !idStr::Icmp(args.Argv(3),"friend") ) {
		gameLocal.relationdata.SetRelation( A,B,R_FRIEND );
	} else if ( !idStr::Icmp(args.Argv(3),"none") ) {
		gameLocal.relationdata.SetRelation( A,B,R_NONE );
	}
}
...
===============
*/
void Cmd_ClearRelation_f( const idCmdArgs &args ) {
int id;
idEntity *A;
	...
	if ( args.Argc() < 2 ) {
			common->Printf( "usage: ClearRelation <e>\n" );
		return;
	}
	...
	gameLocal.relationdata.ClearRelations( A );
}

/*
===============
Cmd_SessionCommand_f

Send frame session command

===============
*/
void Cmd_SessionCommand_f( const idCmdArgs &args ) {
	idPlayer				*player;
	...
	if ( args.Argc() != 2 ) {
		common->Printf( "usage: SessionCommand \"<command>\"\n" );
		return;
	}
	gameLocal.sessionCommand = args.Argv(1);
}
...
==================
*/
void Cmd_SkipTime_f( const idCmdArgs &args ) {
	if ( !gameLocal.GetLocalPlayer() || !gameLocal.CheatsOk( false ) || (gameLocal.cinematicState & CIN_CUT) ) {
		return;
	}
	if ( args.Argc() != 2 ) {
		gameLocal.Printf( "usage: skiptime <time in ms>\n" );
		return;
	}

	gameLocal.cinematicStopTime = gameLocal.time + atoi( args.Argv( 1 ) );
	gameLocal.cinematicMaxSkipTime = gameLocal.cinematicStopTime + 200;
	gameLocal.cinematicState = CIN_CUT|CIN_SKIP;
	gameLocal.msec = USERCMD_MSEC * 4; // one by four
}
...
==================
*/
void Cmd_WhatBinds_f( const idCmdArgs &args ) {

	if ( args.Argc() != 2 ) {
		gameLocal.Printf( "usage: whatbinds <bindname>\n" );
		return;
	}

	gameLocal.Printf ( "This bind applied to following keys: %s\n", common->KeysFromBinding( args.Argv(1)) );
}

/*
==================
Cmd_ListDelayedReturns_f

List sheduled events return objects
==================
*/
void Cmd_ListDelayedReturns_f( const idCmdArgs &args ) {

	idEventReturn::ListReturns();
}
...
void idGameLocal::InitConsoleCommands( void ) {
	...
	cmdSystem->AddCommand( "fly",					Cmd_Fly_f,					CMD_FL_GAME|CMD_FL_CHEAT,	"enables flight" );
	...
	cmdSystem->AddCommand( "call",					Cmd_Call_f,					CMD_FL_GAME|CMD_FL_CHEAT,	"calls script function(s)" );
	cmdSystem->AddCommand( "script",				Cmd_Script_f,				CMD_FL_GAME|CMD_FL_CHEAT,	"executes a line of script" );
	cmdSystem->AddCommand( "scriptCode",			Cmd_ASMScript_f,			CMD_FL_GAME|CMD_FL_CHEAT,	"compiles a line of script" );
	...
	cmdSystem->AddCommand( "openFSGui",				Cmd_OpenFSGui_f,			CMD_FL_GAME|CMD_FL_CHEAT,	"Open a fullscreen gui" );
...
//	cmdSystem->AddCommand( "clientCallVote",		idMultiplayerGame::CallVote_f,	CMD_FL_GAME,			"call a vote: clientCallVote si_.. proposed_value" );
	...
	cmdSystem->AddCommand( "setSkillKey",			Cmd_SetSkillKey_f,			CMD_FL_GAME|CMD_FL_CHEAT,	"Set skill flag" );
	cmdSystem->AddCommand( "listSkillKeys",			Cmd_ListSkillKeys_f,		CMD_FL_GAME,				"Skill flags listing" );
	cmdSystem->AddCommand( "setPersistantKey",		Cmd_SetPersistantKey_f,		CMD_FL_GAME|CMD_FL_CHEAT,	"Set persistant player flag" );
	cmdSystem->AddCommand( "listPersistantKeys",	Cmd_ListPersistantKeys_f,	CMD_FL_GAME,				"Persistant flags listing" );
	cmdSystem->AddCommand( "setLPKey",				Cmd_SetLPKey_f,				CMD_FL_GAME|CMD_FL_CHEAT,	"Set level persistant player flag" );
	cmdSystem->AddCommand( "listLPKeys",			Cmd_ListLPKeys_f,			CMD_FL_GAME,				"Level persistant flags listing" );
	cmdSystem->AddCommand( "SessionCommand",		Cmd_SessionCommand_f,		CMD_FL_GAME|CMD_FL_CHEAT,	"Send session command" );
	cmdSystem->AddCommand( "UpdateEntity",			Cmd_UpdateEntity_f,			CMD_FL_GAME|CMD_FL_CHEAT,	"Force update entity" );
	cmdSystem->AddCommand( "listRels",				Cmd_ListRels_f,				CMD_FL_GAME,				"Relation data list" );
	cmdSystem->AddCommand( "listSpawnids",			Cmd_ListSpawnids_f,			CMD_FL_GAME,				"Spawnids list" );
	cmdSystem->AddCommand( "SetRelation",			Cmd_SetRelation_f,			CMD_FL_GAME|CMD_FL_CHEAT,	"Set relation btw two entitys" );
	cmdSystem->AddCommand( "ClearRelation",			Cmd_ClearRelation_f,		CMD_FL_GAME|CMD_FL_CHEAT,	"Remove all relations for entity" );
	cmdSystem->AddCommand( "skiptime",				Cmd_SkipTime_f,				CMD_FL_GAME|CMD_FL_CHEAT,	"Skip some time" );
	cmdSystem->AddCommand( "whatbinds",				Cmd_WhatBinds_f,			CMD_FL_GAME,				"Show binded keys" );
	cmdSystem->AddCommand( "ListDelayedReturns",	Cmd_ListDelayedReturns_f,	CMD_FL_GAME|CMD_FL_CHEAT,	"List sheduled events return objects" );
...
/*
=================
idCmdSystem::HookCommand
//c4tnt - function hook and unhook
=================
*/
cmdFunction_t idCmdSystem::HookCommand( const char *cmdName, cmdFunction_t new_function ) {
	
	commandDef_t* current = commands;

	while (current != NULL) {
		if ( idStr::Cmp( cmdName, current->name ) == 0 ) {
			cmdFunction_t result = current->function;
			if ( new_function != NULL ) current->function = new_function;
			return result;
		}
