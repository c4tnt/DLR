...
================
*/
void idSound::Save( idSave_I *savefile ) const {
...
================
*/
void idSound::Restore( idRestore_I *savefile ) {
...
void idSound::Spawn( void ) {
	...
	if ( !refSound.waitfortrigger && ( wait > 0.0f ) ) {
		timerOn = true;
		PostEventSec( &EV_Speaker_Timer, NULL, wait + gameLocal.random.CRandomFloat() * random );
	} else {
		timerOn = false;
	}
}
