...
void idTarget_Damage::Event_Activate( idEntity *activator ) {
	...
	if ( spawnArgs.GetBool( "attack_activator", "0" ) ) {
		inflictor = activator;
	} else {
		inflictor = this;
	}
	...
	for( i = 0; i < targets.Num(); i++ ) {
		ent = targets[ i ].GetEntity();
		if ( ent ) {
			ent->Damage( inflictor, this, vec3_origin, damage, 1.0f, INVALID_JOINT );
		}
	}
}
...
CLASS_DECLARATION( idTarget, idTarget_Skill )
	EVENT( EV_Activate, idTarget_Skill::Event_Activate )
END_CLASS
...
================
*/
void idTarget_Skill::Event_Activate( idEntity *activator ) {
	g_skill.SetString(spawnArgs.GetString( "skill" ));
}
...
void idTarget_EndLevel::Event_Activate( idEntity *activator ) {
...
#else
	if ( spawnArgs.GetBool( "endOfGame" ) ) {
		gameLocal.NextLevel( NULL, NULL, false );
		return;
	}
#endif
	if ( !spawnArgs.GetString( "spawn_mark", "", &nextSpawn_cstr ) ) {
		nextSpawn_cstr = NULL;
	}
	...
	gameLocal.NextLevel( nextMap.c_str(), nextSpawn_cstr, spawnArgs.GetBool( "devmap", "0" ) );
}
...
================
*/
void idTarget_FadeEntity::Save( idSave_I *savefile ) const {
...
================
*/
void idTarget_FadeEntity::Restore( idRestore_I *savefile ) {
...
void idTarget_Give::Spawn( void ) {
	if ( spawnArgs.GetBool( "onSpawn" ) ) {
		PostEventMS( &EV_Activate, NULL, 50 );
	}
}
...
================
*/
void idTarget_SetInfluence::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idTarget_SetInfluence::Restore( idRestore_I *savefile ) {
	int i, num;
	int itemNum;
	float set;
...
void idTarget_SetInfluence::Spawn() {
	PostEventMS( &EV_GatherEntities, NULL, 0 );
...
void idTarget_SetInfluence::Event_Flash( float flash, int out ) {
	...
	PostEventSec( &EV_ClearFlash, NULL, flash, flash );
}
...
================
*/
void idTarget_SetInfluence::Event_GatherEntities( void ) {
...
void idTarget_SetInfluence::Event_Activate( idEntity *activator ) {
	...
	if ( spawnArgs.GetBool( "triggerActivate" ) ) {
		if ( restoreOnTrigger ) {
			ProcessEvent( &EV_RestoreInfluence, NULL );
			restoreOnTrigger = false;
			return;
		}
	...
	if ( delay > 0.0f ) {
		PostEventSec( &EV_Activate, NULL, delay, activator );
		delay = 0.0f;
	...
	if ( flashIn ) {
		PostEventSec( &EV_Flash, NULL, 0.0f, flashIn, 0 );
	}

	parm = spawnArgs.GetString( "snd_influence" );
	if ( parm && *parm ) {
		PostEventSec( &EV_StartSoundShader, NULL, flashIn, parm, SND_CHANNEL_ANY );
	}

	if ( switchToCamera ) {
		switchToCamera->PostEventSec( &EV_Activate, NULL, flashIn + 0.05f, this );
	}
	...
	if ( !restoreOnTrigger ) {
		PostEventMS( &EV_RestoreInfluence, NULL, SEC2MS( spawnArgs.GetFloat( "time" ) ) );
	}
}
...
================
*/
void idTarget_SetInfluence::Event_RestoreInfluence( void ) {
	int i, j;
	idEntity *ent;
	idLight *light;
	idSound *sound;
	...
	if ( flashOut ) {
		PostEventSec( &EV_Flash, NULL, 0.0f, flashOut, 1 );
	}

	if ( switchToCamera ) {
		switchToCamera->PostEventMS( &EV_Activate, NULL, 0.0f, this );
	}
...
================
*/
void idTarget_SetFov::Save( idSave_I *savefile ) const {
...
================
*/
void idTarget_SetFov::Restore( idRestore_I *savefile ) {
	float setting;
	...
	if ( spawnArgs.GetBool( "disable" ) ) {
		for( i = 0; i < gameLocal.numClients; i++ ) {
			if ( gameLocal.entities[ i ] ) {
				gameLocal.entities[ i ]->ProcessEvent( &EV_Player_DisableWeapon, NULL );
			}
		}
	} else {
		...
		for( i = 0; i < gameLocal.numClients; i++ ) {
			if ( gameLocal.entities[ i ] ) {
				gameLocal.entities[ i ]->ProcessEvent( &EV_Player_EnableWeapon, NULL );
				if ( weap && weap[ 0 ] ) {
					gameLocal.entities[ i ]->PostEventSec( &EV_Player_SelectWeapon, NULL, 0.5f, weap );
				}
			}
		}
	}
...
================
*/
void idTarget_Tip::Save( idSave_I *savefile ) const {
...
================
*/
void idTarget_Tip::Restore( idRestore_I *savefile ) {
...
void idTarget_Tip::Event_GetPlayerPos( void ) {
	...
	if ( player ) {
		...
		PostEventMS( &EV_TipOff, NULL, 100 );
	}
}
...
void idTarget_Tip::Event_Activate( idEntity *activator ) {
	...
	if ( player ) {
		if ( player->IsTipVisible() ) {
			PostEventSec( &EV_Activate, NULL, 5.1f, activator );
			return;
		}
...
void idTarget_EnableStamina::Event_Activate( idEntity *activator ) {
	for( int i = 0; i < gameLocal.numClients; i++ ) {
		if ( gameLocal.entities[ i ] ) {
			idPlayer *player = static_cast< idPlayer* >( gameLocal.entities[i] );
			player->EnableStamina( spawnArgs.GetBool( "enable" ) );
		}
	}
}
...
void idTarget_FadeSoundClass::Event_Activate( idEntity *activator ) {
	...
	if ( fadeTime ) {
		...
		if ( fadeDuration ) {
			PostEventSec( &EV_RestoreVolume, NULL, fadeDuration );
		}
	}
}
...
================
*/
void idTarget_FadeSoundClass::Event_RestoreVolume( void ) {
	float fadeTime = spawnArgs.GetFloat( "fadeTime" );
	float fadeDB = spawnArgs.GetFloat( "fadeDB" );
	int fadeClass = spawnArgs.GetInt( "fadeClass" );
	// restore volume
