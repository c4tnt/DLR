...
const idEventDef EV_Player_PDAAvaliable( "PDAAvaliable", NULL, 'd' );
...
const idEventDef EV_Player_GetMaxHealth( "getMaxHealth", NULL, 'f' );
...
const idEventDef EV_Player_GetHealthPool( "getHealthPool", NULL, 'f' );
const idEventDef EV_Player_SetHealthPool( "setHealthPool", "f" );

const idEventDef EV_Player_SetBanish( "setBanish" );

const idEventDef EV_Player_FlashlightMonsters( "flashlightMonsters" );
const idEventDef EV_Player_AlertMonsters( "alertMonsters" );

const idEventDef EV_Player_GetFocusEntity( "getFocusEntity", NULL, 'e' );
const idEventDef EV_Player_GetFocusType( "getFocusType", NULL, 'f' );
...
CLASS_DECLARATION( idActor, idPlayer )
	...
	EVENT( EV_Player_PDAAvaliable,			idPlayer::Event_PDAAvaliable)
	...
	EVENT( AI_GetHealth,					idPlayer::Event_GetHealth )
	EVENT( EV_Player_GetArmor,				idPlayer::Event_GetArmor )
	EVENT( EV_Player_SetArmor,				idPlayer::Event_SetArmor )
	EVENT( EV_Player_GetMaxHealth,			idPlayer::Event_GetMaxHealth )
	EVENT( EV_Player_GetMaxArmor,			idPlayer::Event_GetMaxArmor )
	EVENT( EV_Player_SetHealthPool,			idPlayer::Event_SetHealthPool )
	EVENT( EV_Player_GetHealthPool,			idPlayer::Event_GetHealthPool )
	EVENT( EV_Player_SetBanish,				idPlayer::Event_SetBanish )
	EVENT( EV_Player_FlashlightMonsters,	idPlayer::Event_FlashlightMonsters )
	EVENT( EV_Player_AlertMonsters,			idPlayer::Event_AlertMonsters )
	EVENT( EV_Player_GetFocusEntity,		idPlayer::Event_GetFocusEntity )
	EVENT( EV_Player_GetFocusType,			idPlayer::Event_GetFocusType )

	EVENT( EV_GetGuiParm,					idPlayer::Event_GetGuiParm )
	EVENT( EV_GetGuiParmFloat,				idPlayer::Event_GetGuiParmFloat )
	EVENT( EV_GuiNamedEvent,				idPlayer::Event_GuiNamedEvent )
	EVENT( EV_SetGuiParmDirect,				idPlayer::Event_SetGuiParmDirect )
	EVENT( EV_SetGuiFloatDirect,			idPlayer::Event_SetGuiFloatDirect )
END_CLASS
...
#else
	weaponkey = gameLocal.skill.GetKey( "weaponList", "weapon" );
	Give( owner, dict, "weapon", owner->spawnArgs.GetString( weaponkey ), NULL, false );
#endif
...
==============
*/
void idInventory::Save( idSave_I *savefile ) const {
	int i;
...
==============
*/
void idInventory::Restore( idRestore_I *savefile ) {
	int i, num;
...
//	weapon_soulcube			= -1;
	...
	worldDebugLightHandle	= -1;
	memset( &worldDebugLight, 0, sizeof(worldDebugLight));
	fullscreen_guis.Clear();
}
...
//	weapon_soulcube			= SlotForWeapon( "weapon_soulcube" );
	...
	value = spawnArgs.GetString( "weapon_default" );	//Set default weapon slot
	if (value) {
		weapon_default			= SlotForWeapon( value );
	}else{
		weapon_default			= SlotForWeapon( "weapon_fists" );
	}
...
//	focusVehicle			= NULL;
	...
	// player properties
	jumpheight = spawnArgs.GetFloat( "pm_jumpheight", "48" );
	stepsize = spawnArgs.GetFloat( "pm_stepsize", "16" );
	crouch_speed = spawnArgs.GetFloat( "pm_crouchspeed", "80" );
	walk_speed = spawnArgs.GetFloat( "pm_walkspeed", "140" );
	run_speed = spawnArgs.GetFloat( "pm_runspeed", "220" );
	max_stamina = spawnArgs.GetFloat( "pm_stamina", "24" );
	staminathreshold = spawnArgs.GetFloat( "pm_staminathreshold", "4" );
	staminarate = spawnArgs.GetFloat( "pm_staminarate", "0.75" );
	max_airTics = spawnArgs.GetInt( "pm_air", "1800" );
	
	pm_minviewpitch = spawnArgs.GetFloat( "pm_minviewpitch", "-89" );
	pm_maxviewpitch = spawnArgs.GetFloat( "pm_maxviewpitch", "89" );
	pm_normalviewheight = spawnArgs.GetFloat( "pm_normalviewheight", "68" );
	pm_crouchviewheight = spawnArgs.GetFloat( "pm_crouchviewheight", "32" );
	pm_deadviewheight = spawnArgs.GetFloat( "pm_deadviewheight", "10" );
	pm_crouchrate = spawnArgs.GetFloat( "pm_crouchrate", "0.87" );
	pm_bboxwidth = spawnArgs.GetFloat( "pm_bboxwidth", "32" );
	pm_crouchbob = spawnArgs.GetFloat( "pm_crouchbob", "0.5" );
	pm_walkbob = spawnArgs.GetFloat( "pm_walkbob", "0.3" );
	pm_runbob = spawnArgs.GetFloat( "pm_runbob", "0.4" );
	pm_runpitch = spawnArgs.GetFloat( "pm_runpitch", "0.002" );
	pm_runroll = spawnArgs.GetFloat( "pm_runroll", "0.005" );
	pm_bobup = spawnArgs.GetFloat( "pm_bobup", "0.005" );
	pm_bobpitch = spawnArgs.GetFloat( "pm_bobpitch", "0.002" );
	pm_bobroll = spawnArgs.GetFloat( "pm_bobroll", "0.002" );

	physicsObj.SetPlayerHeight( CROUCHHEIGHT, spawnArgs.GetFloat( "pm_crouchheight", "38" ) );
	physicsObj.SetPlayerHeight( NORMALHEIGHT, spawnArgs.GetFloat( "pm_normalheight", "74" ) );
	physicsObj.SetPlayerHeight( DEADHEIGHT, spawnArgs.GetFloat( "pm_deadheight", "20" ) );
	...
	// start out standing
	SetEyeHeight( pm_normalviewheight );
	...
	if ( hud ) {
		hud->SetStateInt( "talkcursor", 0 );
		hud->SetStateString( "combatcursor", "1" );
		hud->SetStateString( "itemcursor", "0" );
		hud->SetStateString( "guicursor", "0" );
	}
	...
	if ( worldDebugLightHandle != -1 ) {
		gameRenderWorld->FreeLightDef( worldDebugLightHandle );
		worldDebugLightHandle = -1;
	}
}
...
void idPlayer::Spawn( void ) {
	...
	gameLocal.DPrintf ( "Player spawn\n" );
	...
	if ( gameLocal.isMultiplayer ) {
		gameLocal.DPrintf ( "MP Player spawn (Init)\n" );
		Init();
		gameLocal.DPrintf ( "MP Player spawn (Hide)\n" );
		Hide();	// properly hidden if starting as a spectator
		if ( !gameLocal.isClient ) {
			// set yourself ready to spawn. idMultiplayerGame will decide when/if appropriate and call SpawnFromSpawnSpot
			gameLocal.DPrintf ( "MP Player spawn (Server,Weapon)\n" );
			SetupWeaponEntity();
			gameLocal.DPrintf ( "MP Player spawn (Server,Spawnspot)\n" );
			SpawnFromSpawnSpot();
			forceRespawn = true;
			assert( spectating );
		}
	...
	/* c4tnt: soulcube hardcode, should be removed
	if ( hud ) {
	...
	if ( inventory.levelTriggers.Num() ) {
		PostEventMS( &EV_Player_LevelTrigger, NULL, 0 );
	}
	...
	healthEating = false;
	if ( !gameLocal.isMultiplayer ) {
		lowhealth = gameLocal.skill.GetIntKey("healthMin", "25");	
		if ( health < lowhealth ) {
			health = lowhealth;
		}
		g_damageScale.SetFloat( gameLocal.skill.GetFloatKey( "damageScale", "1" ) );
		g_armorProtection.SetFloat( gameLocal.skill.GetFloatKey( "armorProtection", "1" ) );
		nextHealthTake = gameLocal.skill.GetIntKey( "healthTakeTime", "0" ) * 1000;
		if ( nextHealthTake > 0 ) {
			healthEating = true;
			nextHealthTake += gameLocal.time;
		}
	}
}
...
/*
==============
idPlayer::SetupFullscreenGuis()
==============
*/
void idPlayer::SetupFullscreenGuis( ) {
const idKeyValue *kv;
idFSGui* fsgui;
idUserInterface* ui;

	gameLocal.Printf( "Loading fullscreen guis:\n" );

	fullscreen_gui_opened = -1;

	kv = NULL;
	while ( kv = spawnArgs.MatchPrefix( "fs_gui", kv ) ) {

		if ( !kv->GetKey().Icmpn( "fs_gui_", 7 ) )
			continue;

		gameLocal.Printf( "FSGui '%s'... ", kv->GetValue().c_str() );

		ui = uiManager->FindGui( kv->GetValue(), true, false, true );
		if (ui) {
			fsgui = &fullscreen_guis.Alloc();
			fsgui->gui = ui;
			fsgui->declname = kv->GetValue();
			fsgui->drawGame = spawnArgs.GetBool( va( "%s_ingame", kv->GetKey().c_str() ), "0");
...
==============
*/
idUserInterface* idPlayer::GetFSGui( int Index) {

	if ( Index >= 0 && Index < fullscreen_guis.Num() ) {
		if ( fullscreen_guis[Index].gui ) {
			return fullscreen_guis[Index].gui;
		}
	}
	return NULL;
}
...
==============
*/
bool idPlayer::OpenFSGui( int Index) {

	if ( fullscreen_gui_opened != -1 )
		CloseFSGui();

	if ( Index >= 0 && Index < fullscreen_guis.Num() ) {
		if ( !fullscreen_guis[Index].gui ) {
			fullscreen_guis[Index].gui = uiManager->FindGui( fullscreen_guis[Index].declname, true, false, true );
		}
		if ( fullscreen_guis[Index].gui ) {
			fullscreen_gui_opened = Index;	
			fullscreen_guis[Index].gui->Activate( true, gameLocal.time );
			return true;
		}
	}
	fullscreen_gui_opened = -1;
...
==============
*/
void idPlayer::CloseFSGui( ) {
	if ( fullscreen_gui_opened >= 0 && fullscreen_gui_opened < fullscreen_guis.Num() ) {
		if ( !fullscreen_guis[fullscreen_gui_opened].gui ) {
			fullscreen_gui_opened = -1;
			return;
		}
		fullscreen_guis[fullscreen_gui_opened].gui->Activate( false, gameLocal.time );
		fullscreen_gui_opened = -1;	
		return;
	}
}
...
==============
*/
void idPlayer::ListFSGui( ) {
int i;

	for (i = 0; i < fullscreen_guis.Num(); i++) {
		gameLocal.Printf("%d: Fullscreen gui %s",i, fullscreen_guis[i].declname.c_str() );
		if (fullscreen_guis[i].drawGame) {
			gameLocal.Printf(", transparent. \n");
		} else {
			gameLocal.Printf(". \n");
		}
	}
...
==============
idPlayer::DrawFSGui()
returns false when gui is transparent
==============
*/
bool idPlayer::DrawFSGui( bool afterpass ) {
	...
	if ( afterpass == fullscreen_guis[fullscreen_gui_opened].drawGame) {
		fullscreen_guis[fullscreen_gui_opened].gui->Redraw( gameLocal.time ); //Redraw
		return true;
	}
	return false;
}
...
===========
*/
void idPlayer::Save( idSave_I *savefile ) const {
	int i;
...
//	savefile->WriteInt( weapon_soulcube );
	...
	savefile->WriteInt( weapon_default );
	...
	savefile->WriteBool( healthEating );
	...
	// c4tnt: to remove or not to remove?
	...
	savefile->WriteInt( focusDescr.CursorType );
	if ( focusDescr.CursorType != CURTYPE_NONE ) {
		focusDescr.ent.Save(savefile);
		savefile->WriteBool( focusDescr.LowerWeapon );
		savefile->WriteBool( focusDescr.ShowInCursor );
		savefile->WriteBool( focusDescr.ShowInHud );
		savefile->WriteString( focusDescr.strAct );
		savefile->WriteString( focusDescr.strObj );
		savefile->WriteInt( focusDescr.subIDX );
		savefile->WriteTrace( focusDescr.trace );
		savefile->WriteBool( focusDescr.Useable );
	}
	...
	savefile->WriteFloat( jumpheight );
	savefile->WriteFloat( stepsize );
	savefile->WriteFloat( crouch_speed );
	savefile->WriteFloat( walk_speed );
	savefile->WriteFloat( run_speed );
	savefile->WriteFloat( max_stamina );
	savefile->WriteFloat( staminathreshold );
	savefile->WriteFloat( staminarate );
	savefile->WriteInt( max_airTics );
	
	savefile->WriteFloat( pm_minviewpitch );
	savefile->WriteFloat( pm_maxviewpitch );
	savefile->WriteFloat( pm_normalviewheight );
	savefile->WriteFloat( pm_crouchviewheight );
	savefile->WriteFloat( pm_deadviewheight );
	savefile->WriteFloat( pm_crouchrate );
	savefile->WriteFloat( pm_bboxwidth );
	savefile->WriteFloat( pm_crouchbob );
	savefile->WriteFloat( pm_walkbob );
	savefile->WriteFloat( pm_runbob );
	savefile->WriteFloat( pm_runpitch );
	savefile->WriteFloat( pm_runroll );
	savefile->WriteFloat( pm_bobup );
	savefile->WriteFloat( pm_bobpitch );
	savefile->WriteFloat( pm_bobroll );
...
===========
*/
void idPlayer::Restore( idRestore_I *savefile ) {
	int	  i;
	int	  num;
	float set;
	bool  b;
...
//	savefile->ReadInt( weapon_soulcube );
	...
	savefile->ReadInt( weapon_default );
	...
	savefile->ReadBool( healthEating );
	...
	savefile->ReadInt( focusDescr.CursorType );
	if ( focusDescr.CursorType != CURTYPE_NONE ) {
		focusDescr.ent.Restore(savefile);
		savefile->ReadBool( focusDescr.LowerWeapon );
		savefile->ReadBool( focusDescr.ShowInCursor );
		savefile->ReadBool( focusDescr.ShowInHud );
		savefile->ReadString( focusDescr.strAct );
		savefile->ReadString( focusDescr.strObj );
		savefile->ReadInt( focusDescr.subIDX );
		savefile->ReadTrace( focusDescr.trace );
		savefile->ReadBool( focusDescr.Useable );
	}
//	savefile->ReadObject( reinterpret_cast<idClass *&>( focusGUIent ) );
//	savefile->ReadObject( reinterpret_cast<idClass *&>( focusUSEent ) );
	// can't save focusUI
//	focusUI = NULL;
//	savefile->ReadObject( reinterpret_cast<idClass *&>( focusCharacter ) );
//	savefile->ReadInt( CursorType );
//	savefile->ReadObject( reinterpret_cast<idClass *&>( focusVehicle ) );
	...
	savefile->ReadFloat( jumpheight );
	savefile->ReadFloat( stepsize );
	savefile->ReadFloat( crouch_speed );
	savefile->ReadFloat( walk_speed );
	savefile->ReadFloat( run_speed );
	savefile->ReadFloat( max_stamina );
	savefile->ReadFloat( staminathreshold );
	savefile->ReadFloat( staminarate );
	savefile->ReadInt( max_airTics );
	
	savefile->ReadFloat( pm_minviewpitch );
	savefile->ReadFloat( pm_maxviewpitch );
	savefile->ReadFloat( pm_normalviewheight );
	savefile->ReadFloat( pm_crouchviewheight );
	savefile->ReadFloat( pm_deadviewheight );
	savefile->ReadFloat( pm_crouchrate );
	savefile->ReadFloat( pm_bboxwidth );
	savefile->ReadFloat( pm_crouchbob );
	savefile->ReadFloat( pm_walkbob );
	savefile->ReadFloat( pm_runbob );
	savefile->ReadFloat( pm_runpitch );
	savefile->ReadFloat( pm_runroll );
	savefile->ReadFloat( pm_bobup );
	savefile->ReadFloat( pm_bobpitch );
	savefile->ReadFloat( pm_bobroll );
...
void idPlayer::SelectInitialSpawnPoint( idVec3 &origin, idAngles &angles ) {
	...
	spot->PostEventMS( &EV_ActivateTargets, NULL, 0, this );
...
void idPlayer::SpawnToPoint( const idVec3 &spawn_origin, const idAngles &spawn_angles ) {
	...
	if ( !spectating ) {
		...
		spec_origin[ 2 ] += physicsObj.GetPlayerHeight(NORMALHEIGHT);
...
void idPlayer::SavePersistantInfo( void ) {
	...
	playerInfo.Set( "previous_map", gameLocal.GetMapName() );
}
...
// new-style
	if ( gameLocal.isMultiplayer ) {
		...
		idDict sa;
		sa = spawnArgs;
		sa.Copy(gameLocal.persistentPlayerInfo[entityNumber]);
		inventory.RestoreInventory( this, sa );
		health = sa.GetInt( "health", "100" );
		if ( !gameLocal.isClient ) {
			idealWeapon = sa.GetInt( "current_weapon", "1" );
		}
	}
...
void idPlayer::DrawHUD( idUserInterface *_hud ) {
	...
	if ( !weapon.GetEntity() || viewLocked() || !_hud || !g_showHud.GetBool() ) {
		return;
	}
	...
	if ( !GuiActive() ) {
		if ( cursor && (weapon.GetEntity()->ShowCrosshair() || focusDescr.CursorType != CURTYPE_NONE ) ) {
...
void idPlayer::UpdateConditions( void ) {
	...
	AI_RUN			= ( usercmd.buttons & BUTTON_RUN ) && ( ( !max_stamina ) || ( stamina > staminathreshold ) );
	AI_DEAD			= ( health <= 0 );
}
...
===============
*/
void idPlayer::FireWeapon( const int WeaponMode ) {
	idMat3 axis;
	idVec3 muzzle;
	...
	if ( !hiddenWeapon && weapon.GetEntity()->IsReady() ) {
		if ( weapon.GetEntity()->AmmoInClip() || weapon.GetEntity()->AmmoAvailable() ) {
			AI_ATTACK_HELD = true;
			weapon.GetEntity()->BeginAttack( WeaponMode );
			// c4tnt: soulcube hardcode, removed
			/*
...
bool idPlayer::Give( const char *statname, const char *value ) {
	...
	if ( !idStr::Icmp( statname, "health" ) ) {
		...
		if ( airTics >= max_airTics ) {
			return false;
		}
		airTics += atoi( value ) / 100.0 * max_airTics;
		if ( airTics > max_airTics ) {
			airTics = max_airTics;
		}
	} else {
...
void idPlayer::UpdatePowerUps( void ) {
	...
	if ( !(gameLocal.cinematicState & CIN_CUT) && influenceActive == 0 && healthEating && gameLocal.time > nextHealthTake && !AI_DEAD && health > gameLocal.skill.GetIntKey( "healthTakeLimit", "0" ) ) {
		assert( !gameLocal.isClient );	// healthPool never be set on client
		health -= gameLocal.skill.GetIntKey( "healthTakeAmt", "0" );
		if ( health < gameLocal.skill.GetIntKey( "healthTakeLimit", "0" ) ) {
			health = gameLocal.skill.GetIntKey( "healthTakeLimit", "0" );
		}
		nextHealthTake = gameLocal.time + gameLocal.skill.GetIntKey( "healthTakeTime", "0" ) * 1000;
		healthTake = true;
	}
#endif
}
...
void idPlayer::Reload( void ) {
	...
	if ( spectating || (gameLocal.cinematicState & CIN_CUT) || influenceActive ) {
		return;
	}
...
void idPlayer::NextWeapon( void ) {
	...
	if ( !weaponEnabled || spectating || hiddenWeapon || (gameLocal.cinematicState & CIN_CUT) || gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) || health < 0 ) {
		return;
	}
...
void idPlayer::SelectWeapon( int num, bool force ) {
	...
	if ( !weaponEnabled || spectating || (gameLocal.cinematicState & CIN_CUT) || health < 0 ) {
		return;
	}
...
idEntity* ent;
	...
	ent = focusDescr.ent.GetEntity();
	if ( ent )
	{
		switch (focusDescr.CursorType)
		{
		case CURTYPE_GUI:
			return ent->FindGUI(focusDescr.subIDX);
		default:
			return NULL;
		}
	}
...
/*
===============
idPlayer::ActiveFullscreenGui
===============
*/
idUserInterface *idPlayer::ActiveFullscreenGui( void ) {
	...
	if ( fullscreen_gui_opened != -1 )
	{
		return GetFSGui( fullscreen_gui_opened );
	}
...
void idPlayer::Weapon_Combat( void ) {
	...
	*/
	...
	AI_WEAPON_FIRED = false;
	if ( !influenceActive ) {
		if ( ( usercmd.buttons & ( BUTTON_ATTACK | BUTTON_ALTATTACK ) ) && !weaponGone ) {
			FireWeapon( ((usercmd.buttons & BUTTON_ATTACK)?WPM_A:0)|((usercmd.buttons & BUTTON_ALTATTACK)?WPM_B:0) );
		} else if ( oldButtons & ( BUTTON_ATTACK | BUTTON_ALTATTACK ) ) {
			AI_ATTACK_HELD = false;
			weapon.GetEntity()->EndAttack();
		}
	}
...
===============
*/
void idPlayer::UseCursor( bool State, bool Held ) {
idEntity	*entptr;
	...
	case CURTYPE_GUI:
		...
		entptr = focusDescr.ent.GetEntity();
		{
			sysEvent_t ev;
			const char *command = NULL;
			bool updateVisuals = false;

			idUserInterface *ui = entptr->FindGUI( focusDescr.subIDX );
			if ( ui ) {
				ev = sys->GenerateMouseButtonEvent( 1, State );
				command = ui->HandleEvent( &ev, gameLocal.time, &updateVisuals );
				if ( updateVisuals && entptr ) {	//c4tnt: condition '&& ui == focusUI' removed because ui always equals to focusUI
					entptr->UpdateVisuals();
				}
			} else {
				return;
			}
			...
			if ( entptr ) {
				HandleGuiCommands( entptr, command, focusDescr.subIDX );
			} else {
				HandleGuiCommands( this, command, 0 );
			}
		}
		break;
	case CURTYPE_TALK:
		if ( State && Held ) {
			entptr = focusDescr.ent.GetEntity();
			if (entptr->IsType(idAI::Type)) {
				((idAI*)entptr)->TalkTo( this );
			}else{
				gameLocal.Error("Tries talk with non-AI object");
			}
		}
		break;
	case CURTYPE_USE:
	case CURTYPE_TAKE:
	case CURTYPE_VEHICLE:
		if ( State && Held ) {
			entptr = focusDescr.ent.GetEntity();
			if ( !entptr )
				break;
			if ( !entptr->RespondsTo( EV_Use ) && !entptr->HasSignal( SIG_USE ) ) {
				break;
			}
			entptr->Signal( SIG_USE );
			entptr->ProcessEvent( &EV_Use, NULL, this, &focusDescr.trace );
		}
		break;
	case CURTYPE_TALK_BUSY:
	case CURTYPE_VEHICLE_BUSY:
	case CURTYPE_USE_BUSY:
	case CURTYPE_USE_LOCKED:
	case CURTYPE_VEHICLE_LOCKED:
		break;
	default:
		gameLocal.Error("Unhandled cursor type");
	}
}
...
/*
===============
idPlayer::Weapon_FSGUI
===============
*/
void idPlayer::Weapon_FSGUI( idUserInterface *ui ) {
	...
	if ( ( oldButtons ^ usercmd.buttons ) & BUTTON_ATTACK ) {
		...
		if ( ui ) {
			...
			command = ui->HandleEvent( &ev, gameLocal.time, &updateVisuals );
			if ( updateVisuals ) {	//c4tnt: condition '&& ui == focusUI' removed because ui always equals to focusUI
				UpdateVisuals();
			}
		}
		if ( gameLocal.isClient ) {
...
void idPlayer::UpdateWeapon( void ) {
	...
	idUserInterface* fsui = ActiveFullscreenGui();
	...
	if ( g_dragEntity.GetBool() ) {
		...
		Weapon_FSGUI( fsui );
	} else if ( focusDescr.LowerWeapon ) { 
		// Lower weapon
		if ( idealWeapon != currentWeapon ) {
			Weapon_Combat();
		}
		StopFiring();
		weapon.GetEntity()->LowerWeapon();
		UseCursor( ( usercmd.buttons & BUTTON_ATTACK ), ((usercmd.buttons ^ oldButtons) & BUTTON_ATTACK ) );
	} else {
		Weapon_Combat();
	}
	...
	if ( force || gameLocal.time > lastSpectateChange ) {
		...
		if ( player && player != this && !player->spectating && !player->IsInTeleport() ) {
			...
			if ( player->physicsObj.IsCrouching() ) {
				newOrig[ 2 ] += pm_crouchviewheight;
			} else {
				newOrig[ 2 ] += pm_normalviewheight;
			}
			...
			SelectInitialSpawnPoint( spawn_origin, spawn_angles );
			spawn_origin[ 2 ] += pm_normalviewheight;
...
===============
*/
bool idPlayer::HandleSingleGuiCommand( idEntity *entityGui, idLexer *src, int eg_num ) {
	idToken token;
	idToken token2;
	...
	if ( token.Icmp( "addhealth" ) == 0 ) {
		int _hpd = HEALTH_PER_DOSE;

		if ( src->CheckTokenString("(") ) {
			if ( !src->ReadToken( &token2 ) ) {
				gameLocal.Error( "Expected int variable in addhealth( %s ...", token2.c_str() );
			}
			if ( !token2.IsNumeric() ) {
				gameLocal.Error( "Expected int variable in addhealth( %s ...", token2.c_str() );
			}
			_hpd = token2.GetIntValue();
			src->ExpectTokenString( ")" );
		}

		if ( entityGui && health < inventory.maxHealth ) {
			int _health = entityGui->spawnArgs.GetInt( "gui_parm1" );
			int amt = ( _health >= _hpd ) ? _hpd : _health;
			_health -= amt;
			entityGui->spawnArgs.SetInt( "gui_parm1", _health );
			if ( entityGui->GetRenderEntity() && entityGui->GetRenderEntity()->gui[ eg_num ] ) {
				entityGui->GetRenderEntity()->gui[ eg_num ]->SetStateInt( "gui_parm1", _health );
			}
			health += amt;
			if ( health > inventory.maxHealth ) {
				health = inventory.maxHealth;
	...
	if ( token.Icmp( "addarmor" ) == 0 ) {
		int _apd = ARMOR_PER_DOSE;

		if ( src->CheckTokenString("(") ) {
			if ( !src->ReadToken( &token2 ) ) {
				gameLocal.Error( "Expected int variable in addarmor( %s ...", token2.c_str() );
			}
			if ( !token2.IsNumeric() ) {
				gameLocal.Error( "Expected int variable in addarmor( %s ...", token2.c_str() );
			}
			_apd = token2.GetIntValue();
			src->ExpectTokenString( ")" );
		}

		if ( entityGui && health < inventory.maxHealth ) {
			int _armor = entityGui->spawnArgs.GetInt( "gui_parm1" );
			int amt = ( _armor >= _apd ) ? _apd : _armor;
			_armor -= amt;
			entityGui->spawnArgs.SetInt( "gui_parm1", _armor );
			if ( entityGui->GetRenderEntity() && entityGui->GetRenderEntity()->gui[ eg_num ] ) {
				entityGui->GetRenderEntity()->gui[ eg_num ]->SetStateInt( "gui_parm1", _armor );
			}
			inventory.armor += amt;
	...
	if ( token.Icmp( "playpdaaudio" ) == 0 ) {
		if ( objectiveSystem && objectiveSystemOpen && pdaAudio.Length() > 0 ) {
			...
			PostEventMS( &EV_Player_StopAudioLog, NULL, ms + 150 );
		}
		return true;
	}
...
==============
*/
impactEffect_t idPlayer::Collide( const trace_t &collision, const idVec3 &velocity ) {
	idEntity *other;
	...
	if ( other ) {
		...
		if ( !spectating ) {
			if ( other->RespondsTo( EV_Touch ) ) {
				other->ProcessEvent( &EV_Touch, NULL, this, &collision );
			}
		} else {
			if ( other->RespondsTo( EV_SpectatorTouch ) ) {
				other->ProcessEvent( &EV_SpectatorTouch, NULL, this, &collision );
			}
		}
	}
	return IMP_BOUNCE;
...
void idPlayer::ClearFocus( void ) {
	ClearUseDescr( focusDescr );
}

/*
================
idPlayer::CursorEnter
================
*/
bool idPlayer::CursorEnter( bool UpdateOnly ) {

	idEntity* ent = focusDescr.ent.GetEntity();

	if (cursor && focusDescr.ShowInCursor) {
		cursor->SetStateInt( "cursor_mode", focusDescr.CursorType );
		cursor->SetStateString( "cursor_act", focusDescr.strAct );
		cursor->SetStateString( "cursor_obj", focusDescr.strObj );
		cursor->SetStateInt( "cursor_useable", focusDescr.Useable );
		if (!UpdateOnly) cursor->HandleNamedEvent( "cursorEnter" );
	}
	if (hud && focusDescr.ShowInHud) {
		hud->SetStateInt( "cursor_mode", focusDescr.CursorType );
		hud->SetStateString( "cursor_act", focusDescr.strAct );
		hud->SetStateString( "cursor_obj", focusDescr.strObj );
		hud->SetStateInt( "cursor_useable", focusDescr.Useable );
		if (!UpdateOnly) hud->HandleNamedEvent( "cursorEnter" );
	}

	if ( focusDescr.CursorType == CURTYPE_TALK || focusDescr.CursorType == CURTYPE_TALK_BUSY ) {
		// Old-style npc talk
		if ( cursor ) {
			cursor->SetStateInt( "talkcursor", focusDescr.CursorType );
		}
		if ( hud ) {
			hud->SetStateString( "npc", focusDescr.strObj );
			if (!UpdateOnly) hud->HandleNamedEvent( "showNPC" );
		}
	}
...
/*
================
idPlayer::CursorLeave
================
*/
void idPlayer::CursorLeave( void ) {
...
const char* command;
	...
	if ( focusDescr.CursorType == CURTYPE_GUI ) {
		// special case: GUI exit
		if ( ent ) {
			ui = ent->FindGUI( focusDescr.subIDX );
			if ( ui ) {
				command = ui->Activate( false, gameLocal.time );
				HandleGuiCommands( ent, command, focusDescr.subIDX );
				StartSound( "snd_guiexit", SND_CHANNEL_ANY, 0, false, NULL ); //c4tnt todo: try ent->StartSound
			}
		}
		ClearUseDescr( focusDescr );
		return;
	...
	if (cursor && focusDescr.ShowInCursor) {
		cursor->HandleNamedEvent( "cursorLeave" );	// clear all inside the event
	}
	if (hud && focusDescr.ShowInHud) {
		hud->HandleNamedEvent( "cursorLeave" );  // clear all inside the event
	}
...
void idPlayer::UpdateFocus( void ) {
	...
	float				m_distance;				// Nearest model distance ( not fraction, real distance )
	use_descr_and_pt	m_Descr[2];
	use_descr_and_pt*	m_dFlip;
	...
	if ( gameLocal.cinematicState & CIN_CUT ) {
		ClearFocus();
		return;
	}

	dbg = g_debugCursor.GetBool();

	// only update the focus character when attack button isn't pressed so players
	// can still chainsaw NPC's
	// WARNING: discarded if ( gameLocal.isMultiplayer || usercmd.buttons & BUTTON_ATTACK ) {
	if ( focusDescr.CursorType == CURTYPE_NONE ) {
	...
	// Find here all cursor candidates

	dir = viewAngles.ToForward();
	end = start + dir * 80.0f;

	// Initial World Trace
	gameLocal.clip.TranslationSingle( trace, start, end, NULL, NULL, mat3_identity, MASK_SHOT_RENDERMODEL );
	idBounds bounds( start );
	bounds.AddPoint( trace.endpos );
	if ( dbg ) gameRenderWorld->DebugCircle( colorBlue, trace.endpos, trace.c.normal, 1, 6, 0, false );
	listedClipModels = gameLocal.clip.ClipModelsTouchingBounds( bounds, -1, clipModelList, MAX_GENTITIES );
	m_distance = (trace.endpos - start).Length();

	ClearUseDescr( m_Descr[1].use_descr );
	m_dFlip = &m_Descr[0];
	for ( i = 0; i < listedClipModels; i++ ) {
		...
		if ( clip ) {
			...
			if ( !ent || ent->IsHidden() || ent == this || clip->GetOwner() == this ) {
				continue;
			}

			ClearUseDescr( m_dFlip->use_descr );
			m_dFlip->use_descr.ent = ent;
			m_dFlip->use_descr.subIDX = 0;

			if ( gameLocal.clip.TranslationSingle( m_dFlip->use_descr.trace, start, end, clip, NULL, mat3_identity, MASK_ALL ) ) {
				m_dFlip->use_descr.trace.c.contents = clip->GetContents();
				distance = (m_dFlip->use_descr.trace.endpos - start).Length();
				int hModelDef = ent->GetModelDefHandle();
				if ( hModelDef != -1 ) {
					m_dFlip->pt = gameRenderWorld->GuiTrace( ent->GetModelDefHandle(), start, end );
					if ( m_dFlip->pt.x != -1 ) {
						ui = ent->FindGUI( m_dFlip->pt.x );
						if ( ui && ui->IsInteractive() ) {
							hasGUI = true;
							m_dFlip->use_descr.CursorType = CURTYPE_GUI;
							m_dFlip->use_descr.subIDX = m_dFlip->pt.guiId - 1;
							distance -= GUI_ZHACK;
						}
					}
				}
				
				markRM = ( ent->ConfirmUseDescr( &m_dFlip->use_descr ) || hasGUI ) && allowFocus;

				if ( distance < m_distance ) {
					if ( markRM ) {
						if ( dbg ) gameRenderWorld->DebugCircle( colorGreen, m_dFlip->use_descr.trace.endpos, m_dFlip->use_descr.trace.c.normal, 1, 6, 0, false );
						m_distance = distance;
						m_dFlip = &m_Descr[(m_dFlip==m_Descr)?1:0];	// Wow, recursion :0
					} else if ( m_dFlip->use_descr.trace.c.contents & MASK_SHOT_RENDERMODEL ) {
						// Rendermodel, clipout focus
						if ( dbg ) gameRenderWorld->DebugCircle( colorYellow, m_dFlip->use_descr.trace.endpos, m_dFlip->use_descr.trace.c.normal, 1, 6, 0, false );
						m_distance = distance;
						ClearUseDescr( m_dFlip->use_descr );		// Clear current usedescr
						m_dFlip = &m_Descr[(m_dFlip==m_Descr)?1:0];	
					}
				}
			}
		}
	}
	m_dFlip = &m_Descr[(m_dFlip==m_Descr)?1:0];	// Switch back to saved buffer
	bUpdate = false;

	if ( focusDescr.CursorType != CURTYPE_NONE ) {
		if ( m_dFlip->use_descr.CursorType == CURTYPE_NONE ) {
			// Check sticky timer first
			if ( focusDescr.stickyTimer > gameLocal.time ) {
				return;
			}
			// Clear focus and exit
			CursorLeave();
			return;
		}
		// Try to recover last focus
		ent = m_dFlip->use_descr.ent.GetEntity();
		if ( m_dFlip->use_descr.ent == focusDescr.ent ) {
			// Same entity
			if ( m_dFlip->use_descr.CursorType == focusDescr.CursorType ) {
				// Same action
				if ( focusDescr.CursorType == CURTYPE_GUI ) {
					// Regenerate GUI
					ui = ent->FindGUI( m_dFlip->use_descr.subIDX );
					if ( ui && m_dFlip->use_descr.subIDX == focusDescr.subIDX ) {
						// update stats
						ProjectOntoGUI( ui, true );
						// set new mouse position
						ui->SetStateInt( "CursorX", m_dFlip->pt.x * SCREEN_WIDTH - ui->CursorX() );
						ui->SetStateInt( "CursorY", m_dFlip->pt.y * SCREEN_HEIGHT - ui->CursorY() );
						// update mouse
						ev = sys->GenerateMouseMoveEvent( m_dFlip->pt.x * SCREEN_WIDTH - ui->CursorX(), m_dFlip->pt.y * SCREEN_HEIGHT - ui->CursorY() );
						command = ui->HandleEvent( &ev, gameLocal.time );
						HandleGuiCommands( ent, command, m_dFlip->pt.guiId );
					} else {
						CursorLeave();
					}
					return;
	...
	if ( m_dFlip->use_descr.CursorType != CURTYPE_NONE ) {
		// Enter the new focus
		ent = m_dFlip->use_descr.ent.GetEntity();
		if ( m_dFlip->use_descr.CursorType == CURTYPE_GUI ) {
			ui = ent->FindGUI( m_dFlip->use_descr.subIDX );
			if ( ui ) {
				m_dFlip->use_descr.LowerWeapon = true;	
				ProjectOntoGUI( ui, false );
				kv = ent->spawnArgs.MatchPrefix( "gui_parm", NULL );
				while( kv ) {
					ui->SetStateString( kv->GetKey(), kv->GetValue() );
					kv = spawnArgs.MatchPrefix( "gui_parm", kv );
				}
				...
				// initial mouse cursor position for gui
				ui->SetStateInt( "CursorX", m_dFlip->pt.x * SCREEN_WIDTH - ui->CursorX() );
				ui->SetStateInt( "CursorY", m_dFlip->pt.y * SCREEN_HEIGHT - ui->CursorY() );

				// move to an absolute position
				ev = sys->GenerateMouseMoveEvent( m_dFlip->pt.x * SCREEN_WIDTH, m_dFlip->pt.y * SCREEN_HEIGHT );
				command = ui->HandleEvent( &ev, gameLocal.time );
				HandleGuiCommands( ent, command, m_dFlip->pt.guiId );

				command = ui->Activate( true, gameLocal.time );
				HandleGuiCommands( ent, command, m_dFlip->pt.guiId );
				StartSound( "snd_guienter", SND_CHANNEL_ANY, 0, false, NULL );
			}
		}
		focusDescr = m_dFlip->use_descr;
		CursorEnter( bUpdate );
	}

	if ( dbg && m_dFlip->use_descr.CursorType ) {
		gameRenderWorld->DebugCircle( colorGreen, m_dFlip->use_descr.trace.endpos, idVec3(0,0,1), 1, 6, 0, false );
		ent = m_dFlip->use_descr.ent.GetEntity();
		if ( ent ) {
			ent->DebugHilight( idVec4( 0.0f, 1.0f, 0.0f, 0.5f ), 0 );
		}
	}
}
...
void idPlayer::CrashLand( const idVec3 &oldOrigin, const idVec3 &oldVelocity ) {
	...
	float		delta;
	float		softDelta, hardDelta, fatalDelta;
	float		dist;
	float		vel, acc;
	float		t;
	float		a, b, c, den;
	...
	if ( waterLevel == WATERLEVEL_WAIST ) {
		delta *= gameLocal.skill.GetFloatKey( "waterWaistFallmod", "0.25" );
	} else if ( waterLevel == WATERLEVEL_FEET ) {
		delta *= gameLocal.skill.GetFloatKey( "waterFeetFallmod", "0.5" );
	}
	...
	if ( gameLocal.isMultiplayer ) {
		fatalDelta	= gameLocal.skill.GetFloatKey( "fatalCrash", "75" );
		hardDelta	= gameLocal.skill.GetFloatKey( "hardCrash", "50" );
		softDelta	= gameLocal.skill.GetFloatKey( "softCrash", "30" );
	} else {
		fatalDelta	= gameLocal.skill.GetFloatKey( "fatalCrash", "65" );
		hardDelta	= gameLocal.skill.GetFloatKey( "hardCrash", "45" );
		softDelta	= gameLocal.skill.GetFloatKey( "softCrash", "30" );
	}
...
void idPlayer::BobCycle( const idVec3 &pushVelocity ) {
	...
	if ( !physicsObj.HasGroundContacts() || influenceActive == INFLUENCE_LEVEL2 || ( gameLocal.isMultiplayer && spectating ) ) {
		...
		if ( physicsObj.IsCrouching() ) {
			bobmove = pm_crouchbob;
			// ducked characters never play footsteps
		} else {
			// vary the bobbing based on the speed of the player
			bobmove = pm_walkbob * ( 1.0f - bobFrac ) + pm_runbob * bobFrac;
		}
	...
	viewBobAngles.pitch += delta * pm_runpitch;
	
	delta = velocity * viewaxis[1];
	viewBobAngles.roll -= delta * pm_runroll;

	// add angles based on bob
	// make sure the bob is visible even at low speeds
	speed = xyspeed > 200 ? xyspeed : 200;

	delta = bobfracsin * pm_bobpitch * speed;
	if ( physicsObj.IsCrouching() ) {
		delta *= 3;		// crouching
	}
	viewBobAngles.pitch += delta;
	delta = bobfracsin * pm_bobroll * speed;
	if ( physicsObj.IsCrouching() ) {
	...
	if ( physicsObj.HasSteppedUp() ) {
		...
		if ( stepUpDelta > 2.0f * stepsize ) {
			stepUpDelta = 2.0f * stepsize;
		}
	...
	bob = bobfracsin * xyspeed * pm_bobup;
	if ( bob > 6 ) {
		bob = 6;
	}
	viewBob[2] += bob;
...
void idPlayer::UpdateViewAngles( void ) {
	...
	if ( !noclip && ( viewLocked() || objectiveSystemOpen ) ) {
	...
	if ( noclip ) {
		...
		if ( viewAngles.pitch > pm_maxviewpitch ) {
			// don't let the player look down enough to see the shadow of his (non-existant) feet
			viewAngles.pitch = pm_maxviewpitch;
		} else if ( viewAngles.pitch < pm_minviewpitch ) {
			// don't let the player look up more than 89 degrees
			viewAngles.pitch = pm_minviewpitch;
		}
	}
...
int idPlayer::GetBaseHeartRate( void ) {
	...
	int rate = idMath::FtoiFast( base + ( ZEROSTAMINA_HEARTRATE - base ) * ( 1.0f - stamina / max_stamina ) );
...
/*
==============
idPlayer::UpdateLiquids
==============
*/
void idPlayer::UpdateLiquids( void ) {	
...
idEntity*		  ent;
	...
	if ( phys->GetWaterLevel() >= WATERLEVEL_HEAD ) {
		//Update water
		liquid = phys->GetWater();
		if ( liquid ) {
			ent = liquid->GetSelf();
			if ( ent->IsType(idLiquid::Type) ) {
				liquidown = dynamic_cast<idLiquid *>(ent);
				playerView.SetLiquid( liquidown );
			}
		} else {
			playerView.SetLiquid( NULL );
		}
	}else{
		playerView.SetLiquid( NULL );
	}
...
void idPlayer::UpdateAir( void ) {	
	...
	int		v_areas = gameLocal.vacuumAreas.Num();
	...
	if ( v_areas > 0 ) {
		...
		if ( num > 0 ) {
			...
			for ( i = 0; i < v_areas; i++ )	{
				if (gameRenderWorld->AreasAreConnected( gameLocal.vacuumAreas[i], areaNum, PS_BLOCK_AIR )) {
					newAirless = true;
					break;
				}
			}
	...
	// check if the player is in water !!DO NOT FORGET TO FIX WATER DEATH AND SOUNDS
	...
	if ( hud ) {
		hud->SetStateInt( "player_air", 100 * airTics / max_airTics );
	}
}
...
void idPlayer::SetClipModel( void ) {
	...
	if ( spectating ) {
		...
		bounds[0].Set( -pm_bboxwidth * 0.5f, -pm_bboxwidth * 0.5f, 0 );
		bounds[1].Set( pm_bboxwidth * 0.5f, pm_bboxwidth * 0.5f, physicsObj.GetPlayerHeight(NORMALHEIGHT) );
	}
...
void idPlayer::ScriptImpulse( int impulse ) {
	const function_t *func;
	idThread *thread;

	func = scriptObject.GetFunction( "impulse" );
	if ( func ) {
		// Call object function with int arg
		thread = new idThread( );
		thread->ParamE( this );		
		thread->ParamF( (float)impulse );
...
//		msg.WriteByte( impulse );
		ClientSendEvent( EVENT_IMPULSE, &msg );
	}

// [Impulse map]
// Impulse0			\
// ...				 > Direct weapon select ( 0 to 12 )
// Impulse12		/
// Impulse13		Reload
// Impulse14		Next weapon
// Impulse15		Previous weapon
// Impulse16		free
// Impulse17		(MP) Ready
// Impulse18		Center view
// Impulse19		PDA, spared with SCORES button
// Impulse20		ToggleTeam
// Impulse21		free
// Impulse22		Spectate
// Impulse23		\
// ...				 > Direct weapon select ( 13 to 16 )
// Impulse26		/
// Impulse27		free
// Impulse28		Vote yes
// Impulse29		Vote no
...
// Impulse39		/
// Impulse40		Use
// Impulse41		Invbar right
// Impulse42		Invbar left
// Impulse43		Use invbar item
// Impulse44		Drop invbar item
// Impulse45		\
// ...				 > free
// Impulse49		/
// Impulse63		Higher programmable _impulse
	...
	if ( impulse >= IMPULSE_23 && impulse <= IMPULSE_26 ) {
	...
	switch( impulse ) {
		...
		case IMPULSE_40: {
//c4tnt: Now we can use anything
//			UseVehicle();
			UseCursor( true, true );	// Hold button
			UseCursor( false, true ); // and unhold button
			break;
		}
		default: {
			ScriptImpulse( impulse );
		}
	} 
}
...
void idPlayer::EvaluateControls( void ) {
	...
	if ( health <= 0 ) {
		...
		if ( !gameLocal.isMultiplayer && !g_testDeath.GetBool() ) {
			...
			gameLocal.GameOverSequence( forceRespawn );
		}
	}
...
void idPlayer::AdjustSpeed( void ) {
	...
	if ( flight ) {
		...
		if ( ( !max_stamina ) || ( stamina > staminathreshold ) ) {
			bobFrac = 1.0f;
		} else if ( staminathreshold <= 0.0001f ) {
			bobFrac = 0.0f;
		} else {
			bobFrac = stamina / staminathreshold;
		}
		speed = walk_speed * ( 1.0f - bobFrac ) + run_speed * bobFrac;
	} else {
		rate = staminarate;
	...
	physicsObj.SetSpeed( speed, crouch_speed );
}
...
void idPlayer::Move( void ) {
	...
	physicsObj.SetMaxStepHeight( stepsize );
	...
	if ( noclip ) {
		...
		physicsObj.SetContents( CONTENTS_BODY );
		physicsObj.SetMovementType( PM_FLYING );
	} else if ( health <= 0 ) {
	...
	if ( spectating ) {
		...
		newEyeOffset = pm_deadviewheight;
	} else if ( physicsObj.IsCrouching() ) {
		newEyeOffset = pm_crouchviewheight;
	} else if ( GetBindMaster() && GetBindMaster()->IsType( idAFEntity_Vehicle::Type ) ) {
		newEyeOffset = 0.0f;
	} else {
		newEyeOffset = pm_normalviewheight;
	}
	...
	if ( EyeHeight() != newEyeOffset ) {
		if ( spectating ) {
			...
			SetEyeHeight( EyeHeight() * pm_crouchrate + newEyeOffset * ( 1.0f - pm_crouchrate ) );
		}
	}

	if ( noclip || (gameLocal.cinematicState & CIN_CUT) || ( influenceActive == INFLUENCE_LEVEL2 ) ) {
		AI_CROUCH	= false;
...
/*
==============
idPlayer::UpdateLights
==============
*/
void idPlayer::UpdateLights( void ) {

	if (g_testHighlight.GetInteger() != 0) {
		// Show testlight
		worldDebugLight.axis = renderView->viewaxis;
		worldDebugLight.origin = renderView->vieworg;
		worldDebugLight.allowLightInViewID = renderView->viewID;
		worldDebugLight.lightId = LIGHTID_WORLD_MUZZLE_FLASH + entityNumber;	//Supress player shadows

		switch ( g_testHighlight.GetInteger() ) {
		case 1:
			worldDebugLight.pointLight = true;
			worldDebugLight.noShadows = false;
			worldDebugLight.shader = declManager->FindMaterial( "lights/ambientlight2", false );
			worldDebugLight.shaderParms[ SHADERPARM_RED ]			= 0.8f;
			worldDebugLight.shaderParms[ SHADERPARM_GREEN ]			= 0.8f;
			worldDebugLight.shaderParms[ SHADERPARM_BLUE ]			= 0.8f;
			worldDebugLight.shaderParms[ SHADERPARM_TIMESCALE ]		= 1.0f;
			worldDebugLight.lightRadius[0]							= 1024;
			worldDebugLight.lightRadius[1]							= 1024;
			worldDebugLight.lightRadius[2]							= 1024;
			break;
		case 2:
			worldDebugLight.pointLight = true;
			worldDebugLight.noShadows = true;
			worldDebugLight.shader = declManager->FindMaterial( "fogs/glare2", false );
			worldDebugLight.shaderParms[ SHADERPARM_RED ]			= 0.8f;
			worldDebugLight.shaderParms[ SHADERPARM_GREEN ]			= 0.8f;
			worldDebugLight.shaderParms[ SHADERPARM_BLUE ]			= 0.8f;
			worldDebugLight.shaderParms[ SHADERPARM_TIMESCALE ]		= 1.0f;
		...
		case 3:
			worldDebugLight.pointLight = true;
			worldDebugLight.noShadows = true;
			worldDebugLight.shader = declManager->FindMaterial( "fogs/pitFog_to_black", false );
			worldDebugLight.shaderParms[ SHADERPARM_RED ]			= 1.0f;
			worldDebugLight.shaderParms[ SHADERPARM_GREEN ]			= 1.0f;
			worldDebugLight.shaderParms[ SHADERPARM_BLUE ]			= 1.0f;
			worldDebugLight.shaderParms[ SHADERPARM_TIMESCALE ]		= 1.0f;
			worldDebugLight.lightRadius[0]							= 4096;
			worldDebugLight.lightRadius[1]							= 4096;
			worldDebugLight.lightRadius[2]							= 4096;
			break;
		case 4:
			worldDebugLight.pointLight = true;
			worldDebugLight.noShadows = true;
			...
			worldDebugLight.shaderParms[ SHADERPARM_BLUE ]			= 1.0f;
			worldDebugLight.shaderParms[ SHADERPARM_TIMESCALE ]		= 1.0f;
			worldDebugLight.lightRadius[0]							= 8192;
			worldDebugLight.lightRadius[1]							= 8192;
			worldDebugLight.lightRadius[2]							= 8192;
			break;
		}

		if ( worldDebugLightHandle != -1 ) {
			gameRenderWorld->UpdateLightDef( worldDebugLightHandle, &worldDebugLight );
		} else {
			worldDebugLightHandle = gameRenderWorld->AddLightDef( &worldDebugLight );
		}		
	} else {
		// Drop testlight
		if ( worldDebugLightHandle != -1 ) {
...
void idPlayer::Think( void ) {
	...
	if ( gameLocal.cinematicState & CIN_SKIP ) {
		return;
	}
	...
	if ( objectiveSystemOpen || (gameLocal.cinematicState & CIN_CUT) || influenceActive ) {
	...
	idUserInterface *gui = ActiveFullscreenGui();
	if ( gui ) {	//c4tnt: need recover && gui != focusUI
		if ( usercmd.mx != oldCmd.mx || usercmd.my != oldCmd.my ) {
			RouteGuiMouse( gui, usercmd.mx - oldCmd.mx, usercmd.my - oldCmd.my );
		}
	}
	...
	if ( !g_stopTime.GetBool() ) {
		...
		// service animations
		if ( !spectating && !af.IsActive() && !(gameLocal.cinematicState & CIN_CUT) ) {
    		UpdateConditions();
			UpdateAnimState();
			CheckBlink();
		}
...
=================
*/
void idPlayer::RouteGuiMouse( idUserInterface *gui, int dx, int dy ) {
	sysEvent_t ev;
	const char *command;

	gui->SetStateInt( "CursorX", gui->CursorX() + dx );
	gui->SetStateInt( "CursorY", gui->CursorY() + dy );

	ev = sys->GenerateMouseMoveEvent( dx, dy );
	command = gui->HandleEvent( &ev, gameLocal.time );
	HandleGuiCommands( this, command, 0 );
	//gameLocal.Printf( "%s", command );
}
...
void idPlayer::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( !g_testDeath.GetBool() && !gameLocal.hasGameOverSequence( false ) ) {
...
void idPlayer::CalcDamagePoints( idEntity *inflictor, idEntity *attacker, const idDict *damageDef,
	...
	damage = GetDamageForLocation( damage, this->health, location );

	idPlayer *player = attacker->IsType( idPlayer::Type ) ? static_cast<idPlayer*>(attacker) : NULL;
	if ( !gameLocal.isMultiplayer ) {
		if ( inflictor != gameLocal.world ) {
			damage *= gameLocal.skill.GetIntKey( "damageScale", "1" );
			if ( damage < 1 ) {
				damage = 1;
			}
		}
...
void idPlayer::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir,
	...
	if ( !fl.takedamage || noclip || spectating || (gameLocal.cinematicState & CIN_CUT) ) {
		return;
	}
	...
	if ( attacker->IsType( idAI::Type ) ) {
		if ( PowerUpActive( BERSERK ) ) {
			//c4tnt: will remove this hardcode and use powerup definitions
			return;
		}
	...
	if ( damage > 0 ) {
		...
		if ( !gameLocal.isMultiplayer ) {
			float scale = gameLocal.skill.GetFloatKey( "damageScale", "1" );
			
			//FIXME: implement this with a skill class
			/*if ( gameLocal.skill.GetFloatKey( "useDynamicProtection", "0" ) ) {
...
void idPlayer::CalculateFirstPersonView( void ) {
	if ( ( pm_modelView.GetInteger() == 1 ) || ( ( pm_modelView.GetInteger() == 2 ) && ( health <= 0 ) ) ) {
		...
		idEntity	*headEnt = NULL;
		if ( head.GetEntity()) {
			headEnt = head.GetEntity();
			if ( headEnt->GetAnimator() ) {
				joint = headEnt->GetAnimator()->GetJointHandle( "tongue2" );
			}
		}

		if ( joint == INVALID_JOINT ) {
			joint = animator.GetJointHandle( "tongue2" );
			if ( joint == INVALID_JOINT ) {
				gameLocal.Printf( "pm_modelView: no 'camera' joint, cvar disabled\n" );
				pm_modelView.SetInteger( 0 );
				GetViewPos( firstPersonViewOrigin, firstPersonViewAxis );
				return;
			}
			GetJointWorldTransform( joint, gameLocal.time, jorigin, jaxis );
			//animator.GetJointTransform( joint, gameLocal.time, jorigin, jaxis );
			maxis = physicsObj.GetAxis();
			morigin = physicsObj.GetOrigin();
		} else {
			headEnt->GetAnimator()->GetJointTransform( joint, gameLocal.time, jorigin, jaxis );
			maxis = headEnt->GetPhysics()->GetAxis();
			morigin = headEnt->GetPhysics()->GetOrigin();
		}
		
		firstPersonViewOrigin = jorigin * maxis + morigin;
		
		tStart = GetEyePosition();
		tEnd = tStart + viewAngles.ToForward() * 80.0f;
	
		gameLocal.clip.TracePoint( trace, tStart, tEnd, MASK_SHOT_RENDERMODEL, this );
		tEnd = trace.endpos - firstPersonViewOrigin;
		tEnd.Normalize();
		ang = tEnd.ToAngles() + viewBobAngles + playerView.AngleOffset();
		firstPersonViewAxis = ang.ToMat3();
	} else {
...
void idPlayer::AddAIKill( void ) {
	...
	ammo_souls = idWeapon::GetAmmoNumForNameSafe( "ammo_souls" );
	if ( ammo_souls < 0 )
		return;
...
==================
*/
void idPlayer::Event_GetButtons( idEventReturn *returnContext ) {
	returnContext->FinallyInt( usercmd.buttons );
}
...
==================
*/
void idPlayer::Event_GetMove( idEventReturn *returnContext ) {
	idVec3 move( usercmd.forwardmove, usercmd.rightmove, usercmd.upmove );
	returnContext->FinallyVector( move );
}
...
================
*/
void idPlayer::Event_GetViewAngles( idEventReturn *returnContext ) {
	returnContext->FinallyVector( idVec3( viewAngles[0], viewAngles[1], viewAngles[2] ) );
}
...
/*
================
idPlayer::Event_FlashlightMonsters
================
*/
void idPlayer::Event_FlashlightMonsters( void ) {
	trace_t	tr;
	idEntity *ent;
	idVec3 end = firstPersonViewOrigin + firstPersonViewAxis[0] * 100.0f;

	gameLocal.clip.TracePoint( tr, firstPersonViewOrigin, end, CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER, this );
	...
	if ( tr.fraction < 1.0f ) {
		...
		if ( ent->IsType( idAI::Type ) ) {
			...
			ent->ProcessEvent( &EV_Touch, NULL, this, &tr );
		}
	}

	// jitter the trace to try to catch cases where a trace down the center doesn't hit the monster
	end += firstPersonViewAxis[1] * idMath::Sin16( MS2SEC( gameLocal.time ) * 31.34f );
	end += firstPersonViewAxis[2] * idMath::Sin16( MS2SEC( gameLocal.time ) * 12.17f );
	gameLocal.clip.TracePoint( tr, firstPersonViewOrigin, end, CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER, this );
...
void idPlayer::Event_AlertMonsters ( void ) {
...
void idPlayer::StartFxFov( float duration ) { 
	fxFov = true;
	PostEventSec( &EV_Player_StopFxFov, NULL, duration );
}
...
==================
*/
void idPlayer::Event_GetIdealWeapon( idEventReturn *returnContext ) {
	const char *weapon;

	if ( idealWeapon >= 0 ) {
		weapon = spawnArgs.GetString( va( "def_weapon%d", idealWeapon ) );
		returnContext->FinallyString( weapon );
	} else {
		returnContext->FinallyString( "" );
	}
}
...
==================
*/
void idPlayer::Event_IsPowerupActive( idEventReturn *returnContext, int powerup ) {
	returnContext->FinallyInt(this->PowerUpActive(powerup) ? 1 : 0);
}
...
void idPlayer::Event_SetHealth( float newHealth ) {
	...
	if ( AI_DEAD || health <= 0 )
		return;			// When you set player health to 0, player dies and you can't resurrect him
...
/*
=====================
idPlayer::Event_SetHealthPool
=====================
*/
void idPlayer::Event_SetHealthPool( float newHealth ) {
...
/*
=====================
idPlayer::Event_GetHealth
=====================
*/
void idPlayer::Event_GetHealth( idEventReturn *returnContext ) {
	if ( AI_DEAD ) {
		returnContext->FinallyFloat( 0 );
	} else {
		returnContext->FinallyFloat( health );
	}
}

/*
=====================
idPlayer::Event_GetHealthPool
=====================
*/
void idPlayer::Event_GetHealthPool( idEventReturn *returnContext ) {
	if ( AI_DEAD || health <= 0 ) {
		returnContext->FinallyFloat( 0 );
	} else {
		returnContext->FinallyFloat( healthPool );
	}
}
...
void idPlayer::Event_SetArmor( float newArmor ) {
	...
	if ( newArmor > inventory.maxarmor ) {
		inventory.armor = inventory.maxarmor;
	} else if ( newArmor <= 0 ) {
		inventory.armor = 0;
	} else {
...
=====================
*/
void idPlayer::Event_GetArmor( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( inventory.armor );
}

/*
=====================
idPlayer::Event_GetMaxHealth
=====================
*/
void idPlayer::Event_GetMaxHealth( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( inventory.maxHealth );
}

/*
=====================
idPlayer::Event_GetMaxArmor
=====================
*/
void idPlayer::Event_GetMaxArmor( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( inventory.maxarmor );
}
/*
==================
...
==================
*/
void idPlayer::Event_WeaponAvailable( idEventReturn *returnContext, const char* name ) {

	returnContext->FinallyInt( WeaponAvailable(name) ? 1 : 0 );
}
...
==================
*/
void idPlayer::Event_GetCurrentWeapon( idEventReturn *returnContext ) {
	const char *weapon;
...
==================
*/
void idPlayer::Event_GetPreviousWeapon( idEventReturn *returnContext ) {
	const char *weapon;
	...
	if ( previousWeapon >= 0 ) {
		...
		returnContext->FinallyString( spawnArgs.GetString( "def_weapon0" ) );
	}
}
...
==================
*/
void idPlayer::Event_GetWeaponEntity( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( weapon.GetEntity() );
}

/*
==================
idPlayer::Event_GetFocusType
==================
*/
void idPlayer::Event_GetFocusType( idEventReturn *returnContext ) {
	returnContext->FinallyInt( focusDescr.CursorType );
}

/*
==================
idPlayer::Event_GetFocusEntity
==================
*/
void idPlayer::Event_GetFocusEntity( idEventReturn *returnContext ) {
	returnContext->FinallyEntity( focusDescr.ent.GetEntity() );
}
...
==================
*/
void idPlayer::Event_InPDA( idEventReturn *returnContext ) {
	returnContext->FinallyInt( objectiveSystemOpen );
}

/*
==================
idPlayer::Event_PDAavaliable
==================
*/
void idPlayer::Event_PDAAvaliable( idEventReturn *returnContext ) {

	if ( objectiveSystem == NULL ) {
		returnContext->FinallyInt( 0 );
		return;
	}
	...
	returnContext->FinallyInt( 1 );
}

/*
==================
idPlayer::Event_SetBanish
==================
*/
void idPlayer::Event_SetBanish( void ) {
	idBounds bounds;

	bounds[0].Set( -32, -32, 0 );
	bounds[1].Set( 32, 32, 128 );
	idClipModel *newClip;
	newClip = new idClipModel( idTraceModel( bounds, 8 ) );
	newClip->Translate( physicsObj.PlayerGetOrigin() );
	newClip->Link( gameLocal.clip, this, 1, newClip->GetOrigin(), GetPhysics()->GetAxis() );
}
...
void idPlayer::Event_GetGuiParm( idEventReturn *returnContext, int guiNum, const char *key ) {

	idUserInterface * t_gui = NULL;

	if ( guiNum == PLAYER_GUI_HUD ) {
		t_gui = hud;
	} else if ( guiNum == PLAYER_GUI_PDA ) {
		t_gui = objectiveSystem;
	} else if ( guiNum == PLAYER_GUI_CURSOR ) {
		t_gui = cursor;
	} else if( guiNum > 0 && guiNum <= MAX_RENDERENTITY_GUI ) {
		t_gui = renderEntity.gui[guiNum-1];
	}
	if (t_gui == NULL){
		returnContext->FinallyString("");
		return;
...
#if API_VERS > 3
	returnContext->FinallyString(t_gui->GetStateString(key));
#else
	returnContext->FinallyString(t_gui->State().GetString(key, ""));
#endif 
}

/*
================
idPlayer::Event_GetGuiParmFloat
================
*/
void idPlayer::Event_GetGuiParmFloat( idEventReturn *returnContext, int guiNum, const char *key ) {
	...
	if (t_gui == NULL){
		returnContext->FinallyFloat(0.0f);
		return;
	}

#if API_VERS > 3
		returnContext->FinallyFloat(t_gui->GetStateFloat(key));
#else
		returnContext->FinallyFloat(t_gui->State().GetFloat(key, "0"));
#endif 
}

/*
================
idPlayer::Event_GuiNamedEvent
================
*/
void idPlayer::Event_GuiNamedEvent( int guiNum, const char *event ) {
	...
	if (t_gui != NULL){
		t_gui->HandleNamedEvent(event);
	}
}

/*
================
idPlayer::Event_SetGuiParmDirect
================
*/
void idPlayer::Event_SetGuiParmDirect( int index, const char *key, const char *val ) {

	idUserInterface * t_gui = NULL;

	if ( index == PLAYER_GUI_HUD ) {
		t_gui = hud;
	} else if ( index == PLAYER_GUI_PDA ) {
		t_gui = objectiveSystem;
	} else if ( index == PLAYER_GUI_CURSOR ) {
		t_gui = cursor;
	} else if( index > 0 && index <= MAX_RENDERENTITY_GUI ) {
		t_gui = renderEntity.gui[index-1];
	}
	if (t_gui != NULL){
		t_gui->SetStateString( key, val );
		t_gui->StateChanged( gameLocal.time );
	}
}

/*
================
idPlayer::Event_SetGuiFloatDirect
================
*/
void idPlayer::Event_SetGuiFloatDirect( int index, const char *key, float f ) {
	idUserInterface * t_gui = NULL;
	...
	if (t_gui != NULL){
		t_gui->SetStateString( key, va( "%f", f ) );
		t_gui->StateChanged( gameLocal.time );
	}
}
...
void idPlayer::ClientPredictionThink( void ) {
	...
	if ( !(gameLocal.cinematicState & CIN_CUT) && weapon.GetEntity() && ( health > 0 ) && !( gameLocal.isMultiplayer && spectating ) ) {
		UpdateWeapon();
	}
...
void idPlayer::ShowTip( const char *title, const char *tip, bool autoHide ) {
	...
	if ( autoHide ) {
		PostEventSec( &EV_Player_HideTip, NULL, 5.0f );
	}
	tipUp = true;
}
...
void idPlayer::Event_LevelTrigger( void ) {
	...
	for ( int i = inventory.levelTriggers.Num() - 1; i >= 0; i-- ) {
		if ( idStr::Icmp( mapName, inventory.levelTriggers[i].levelName) == 0 ){
			...
			if ( ent ) {
				ent->PostEventMS( &EV_Activate, NULL, 1, this );
			}
		}
	}
}
...
void idPlayer::ProjectOntoGUI( idUserInterface *gui, bool update ) const {
int j;
const idKeyValue *kv;
	...
	gui->SetStateInt( "inv_count", inventory.items.Num() );
	for ( j = 0; j < inventory.items.Num(); j++ ) {
		...
		gui->SetStateString( va( "inv_name_%i", j), iname );
		gui->SetStateString( va( "inv_icon_%i", j), iicon );
		gui->SetStateString( va( "inv_text_%i", j), itext );
		kv = item->MatchPrefix("inv_id", NULL);
		if ( kv ) {
			gui->SetStateString( va( "inv_id_%i", j ), kv->GetValue() );
		}
		gui->SetStateInt( iname, 1 );
	}
	...
	int staminapercentage = ( int )( 100.0f * stamina / max_stamina );
	gui->SetStateString( "player_health", va("%i", health ) );
	gui->SetStateString( "player_stamina", va( "%i%%", staminapercentage ) );
	gui->SetStateString( "player_armor", va( "%i%%", inventory.armor ) );
	gui->SetStateString( "player_hr", va("%i", heartRate ) );
}
