...
public:
	...
	bool						EmitSmoke( const idDeclParticle *smoke, const int startTime, const float diversity,
											const idVec3 &origin, const idMat3 &axis ,const int lastActive = 0);

	// spawns automatic smoke prt
	bool						EmitSmokePrt( const idDeclParticle *smoke, const int startTime, const float diversity,
...
private:
	...
	static const int			MAX_SMOKE_PARTICLES = 32768;	//old = 10000
	singleSmoke_t				smokes[MAX_SMOKE_PARTICLES];

	static const int			MAX_AUTO_SMOKES = 128;		//old = 10000
	spawnedSmoke_t				asmokes[MAX_AUTO_SMOKES];
