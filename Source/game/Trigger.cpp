...
void idTrigger::CallScript( void ) const {
	...
	if ( scriptContext.Callable() ) {
		thread = new idThread( &scriptContext, GetName() );
...
================
*/
void idTrigger::Save( idSave_I *savefile ) const {
	scriptContext.Save( savefile );
}
...
================
*/
void idTrigger::Restore( idRestore_I *savefile ) {
	scriptContext.Restore( savefile );
}
...
void idTrigger::Spawn( void ) {
	...
	idStr functext = spawnArgs.GetString( "call", "" );
	if ( functext.Length() ) {
		if (! scriptContext.CompileParamS( functext ) ) {
			gameLocal.Warning( "can't compile trigger '%s' at (%s) call '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString(0), functext.c_str() );
		}
	} else {
		scriptContext.Reset();
	}
...
================
*/
void idTrigger_Multi::Save( idSave_I *savefile ) const {
...
================
*/
void idTrigger_Multi::Restore( idRestore_I *savefile ) {
...
void idTrigger_Multi::Event_Trigger( idEntity *activator ) {
	...
	if ( delay > 0 ) {
		...
		PostEventSec( &EV_TriggerAction, NULL, delay, activator );
	} else {
		TriggerAction( activator );
	}
}
...
void idTrigger_Multi::Event_Touch( idEntity *other, trace_t *trace ) {
	...
	if ( delay > 0 ) {
		// don't allow it to trigger again until our delay has passed
		nextTriggerTime += SEC2MS( delay + random_delay * gameLocal.random.CRandomFloat() );
		PostEventSec( &EV_TriggerAction, NULL, delay, other );
	} else {
		TriggerAction( other );
	}
}
...
const idEventDef EV_SetDescription( "setDescription","s", NULL );

CLASS_DECLARATION( idTrigger, idTrigger_Use )
	EVENT( EV_Use,				idTrigger_Use::Event_Use )
	EVENT( EV_Activate,			idTrigger_Use::Event_Trigger )
	EVENT( EV_TriggerAction,	idTrigger_Use::Event_TriggerAction )
	EVENT( EV_SetDescription,	idTrigger_Use::Event_SetDescription )
	EVENT( EV_Door_Lock,		idTrigger_Use::Event_Lock )
	EVENT( EV_Door_IsLocked,	idTrigger_Use::Event_Locked )
END_CLASS


/*
================
idTrigger_Use::idTrigger_Use
================
*/
idTrigger_Use::idTrigger_Use( void ) {
	wait = 0.0f;
	random = 0.0f;
	delay = 0.0f;
	random_delay = 0.0f;
...
================
*/
void idTrigger_Use::Save( idSave_I *savefile ) const {
	savefile->WriteFloat( wait );
	savefile->WriteFloat( random );
	savefile->WriteFloat( delay );
	savefile->WriteFloat( random_delay );
	...
	savefile->WriteBool( triggerFirst );
	savefile->WriteBool( triggerWithSelf );
	savefile->WriteBool( fire_use );
	savefile->WriteBool( hidden_use );
	savefile->WriteBool( disable_while_use );
	savefile->WriteBool( locked );
	savefile->WriteString( requires );
	savefile->WriteString( objectName );
	savefile->WriteString( actionName );
}

/*
================
idTrigger_Use::Restore
================
*/
void idTrigger_Use::Restore( idRestore_I *savefile ) {
	savefile->ReadFloat( wait );
	savefile->ReadFloat( random );
	savefile->ReadFloat( delay );
	savefile->ReadFloat( random_delay );
	...
	savefile->ReadBool( triggerFirst );
	savefile->ReadBool( triggerWithSelf );
	savefile->ReadBool( fire_use );
	savefile->ReadBool( hidden_use );
	savefile->ReadBool( disable_while_use );
	savefile->ReadBool( locked );
	savefile->ReadString( requires );
	savefile->ReadString( objectName );
	savefile->ReadString( actionName );
}
...
void idTrigger_Use::Spawn( void ) {
	...
	if ( random && ( random >= wait ) && ( wait >= 0 ) ) {
		random = wait - 1;
		gameLocal.Warning( "idTrigger_Use '%s' at (%s) has random >= wait", name.c_str(), GetPhysics()->GetOrigin().ToString(0) );
	}

	if ( random_delay && ( random_delay >= delay ) && ( delay >= 0 ) ) {
		random_delay = delay - 1;
		gameLocal.Warning( "idTrigger_Use '%s' at (%s) has random_delay >= delay", name.c_str(), GetPhysics()->GetOrigin().ToString(0) );
	}
	...
	spawnArgs.GetBool( "use_lowerweapon", "1", fire_use );
	spawnArgs.GetBool( "use_hidden", "0", hidden_use );
	spawnArgs.GetBool( "use_hidewhenwait", "0", disable_while_use );
	spawnArgs.GetString( "use_action", "", actionName);
	spawnArgs.GetString( "use_name", "", objectName);
...
/*
================
idTrigger_Use::TriggerAction
================
*/
void idTrigger_Use::TriggerAction( idEntity *activator ) {
	ActivateTargets( triggerWithSelf ? this : activator );
	CallScript();
...
/*
================
idTrigger_Use::Event_TriggerAction
================
*/
void idTrigger_Use::Event_TriggerAction( idEntity *activator ) {
	TriggerAction( activator );
}

/*
================
idTrigger_Use::Event_Trigger
================
*/
void idTrigger_Use::Event_Trigger( idEntity *activator ) {
	//Clear triggerFirst only	
	if ( triggerFirst ) {
		triggerFirst = false;
		return;
	}
...
/*
================
idTrigger_Use::Event_Use
================
*/
void idTrigger_Use::Event_Use( idEntity *other, trace_t *trace ) {
...
/*
================
idTrigger_Use::GetUseDescr

returns the orientation of the joint in worldspace
================
*/

bool idTrigger_Use::ConfirmUseDescr( use_descr_t* Desc ) {

	if( triggerFirst ) {
		return false;
	}

	if ( ( nextTriggerTime > gameLocal.time ) && disable_while_use ) {
		// can't retrigger until the wait is over
		return false;
	}

	if ( locked ) {
		Desc->CursorType = CURTYPE_USE_LOCKED;
	} else if ( nextTriggerTime > gameLocal.time ) {
		if ( disable_while_use ) {
			Desc->CursorType = CURTYPE_NONE;
		} else {
			Desc->CursorType = CURTYPE_USE_BUSY;
		}
	} else {
		Desc->CursorType = CURTYPE_USE;
	}
	Desc->ent = this;
	Desc->ShowInCursor = !hidden_use;
	Desc->ShowInHud = !hidden_use;
	Desc->LowerWeapon = fire_use;
	Desc->Useable = true;

	Desc->strAct = actionName;
	Desc->strObj = objectName;
	
	Desc->subIDX = 0;
	return ( Desc->CursorType != CURTYPE_NONE );
}

void idTrigger_Use::Event_SetDescription( const char* text ) {
	actionName = text;
}

/*
================
idTrigger_Use::Event_Locked
================
*/
void idTrigger_Use::Event_Locked( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( locked );
	
}

/*
================
idTrigger_Use::Event_Lock
================
*/
void idTrigger_Use::Event_Lock( float f ) {
	locked = (f != 0);
}
...
================
*/
void idTrigger_EntityName::Save( idSave_I *savefile ) const {
	savefile->WriteFloat( wait );
	savefile->WriteFloat( random );
	savefile->WriteFloat( delay );
	savefile->WriteFloat( random_delay );
...
================
*/
void idTrigger_EntityName::Restore( idRestore_I *savefile ) {
	savefile->ReadFloat( wait );
	savefile->ReadFloat( random );
	savefile->ReadFloat( delay );
	savefile->ReadFloat( random_delay );
...
================
*/
void idTrigger_Timer::Save( idSave_I *savefile ) const {
	savefile->WriteFloat( random );
	savefile->WriteFloat( wait );
...
================
*/
void idTrigger_Timer::Restore( idRestore_I *savefile ) {
	savefile->ReadFloat( random );
	savefile->ReadFloat( wait );
...
void idTrigger_Timer::Spawn( void ) {
	...
	if ( on ) {
		PostEventSec( &EV_Timer, NULL, delay );
	}
}
...
void idTrigger_Timer::Event_Timer( void ) {
	...
	if ( wait >= 0.0f ) {
		PostEventSec( &EV_Timer, NULL, wait + gameLocal.random.CRandomFloat() * random );
	}
}
...
================
*/
void idTrigger_Count::Save( idSave_I *savefile ) const {
...
================
*/
void idTrigger_Count::Restore( idRestore_I *savefile ) {
...
================
*/
void idTrigger_Hurt::Save( idSave_I *savefile ) const {
	savefile->WriteBool( on );
	savefile->WriteFloat( delay );
...
================
*/
void idTrigger_Hurt::Restore( idRestore_I *savefile ) {
	savefile->ReadBool( on );
	savefile->ReadFloat( delay );
...
void idTrigger_Fade::Event_Trigger( idEntity *activator ) {
	...
	if ( player ) {
		...
		PostEventMS( &EV_ActivateTargets, NULL, fadeTime, activator );
	}
}
...
CLASS_DECLARATION( idEntity, idTrigger_Touch )
	EVENT( EV_Activate,		idTrigger_Touch::Event_Trigger )
	EVENT( EV_Enable,	idTrigger_Touch::Event_Enable )
	EVENT( EV_Disable,	idTrigger_Touch::Event_Disable )
END_CLASS
...
================
*/
void idTrigger_Touch::Save( idSave_I *savefile ) {
	if ( scriptFunction ) {
...
================
*/
void idTrigger_Touch::Restore( idRestore_I *savefile ) {
	idStr funcname;
...
/*
================
idTrigger_Touch::Event_Enable
================
*/
void idTrigger_Touch::Event_Enable( void ) {
	Enable();
}

/*
================
idTrigger_Touch::Event_Disable
================
*/
void idTrigger_Touch::Event_Disable( void ) {
	Disable();
}
...
/*
================
idTrigger_Touch::CallScript
================
*/
void idTrigger_Touch::CallScript( void ) const {
	idThread *thread;
