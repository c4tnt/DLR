...
idPlayerView::idPlayerView() {
	...
	depthgMaterial = declManager->FindMaterial( "_depth" ); // c4tnt : group depth data, may be not a Venom 6th format (precache) 
	...
	submergedMaterial = NULL;								// c4tnt: sync this over network! And FSGui too
...
==============
*/
void idPlayerView::Save( idSave_I *savefile ) const {
	int i, c;
	const screenBlob_t *blob;

	c = 0;

	for( i = 0; i < MAX_SCREEN_BLOBS; i++ ) {
		if ( screenBlobs[i].material ) {
			c++;
		}
	}
	...
	savefile->WriteMaterial( depthgMaterial ); // c4tnt : ff DLR materials
...
==============
*/
void idPlayerView::Restore( idRestore_I *savefile ) {
	int i, c;
	screenBlob_t *blob;

	savefile->ReadInt( c );

	if ( c < 0 || c >= MAX_SCREEN_BLOBS ) gameLocal.Error( "idPlayerView::Restore: Bad screen splat list" );
	...
	savefile->ReadMaterial( depthgMaterial ); // c4tnt : ff DLR materials
...
void idPlayerView::SetupViewObjects( void ) {
}

void idPlayerView::ClearViewObjects( void ) {
}
...
/*
==================
idPlayerView::ffDepth
c4tnt : z-buffer capture
==================
*/
void idPlayerView::ffDepth( const renderView_t *view ) {

	renderView_t	hackedView = *view;

	renderSystem->CropRenderSize( 512, 512, true );

	hackedView.globalMaterial = declManager->FindMaterial( "textures/dlrsys/depth" );
	gameRenderWorld->RenderScene( &hackedView );
	renderSystem->CaptureRenderToImage( "_depths" );
	renderSystem->UnCrop();
}
...
idRenderWorld *	tmpRenderWorld = NULL;
bool			bDrawHUD = false;
	...
	if ( player->DrawFSGui( false ) ) {	//opaque guis
		return;
	}

	// hack the shake in at the very last moment, so it can't cause any consistency problems
	renderView_t	hackedView = *view;
	renderView_t	hackedView2 = *view;
	hackedView.viewaxis = hackedView.viewaxis * ShakeAxis();

	if ( g_testGlobalMaterial.GetString()[0] ) {
		const idMaterial *mtr = declManager->FindMaterial( g_testGlobalMaterial.GetString(), false );
		if ( !mtr ) {
			common->Printf( "Material not found.\n" );
			g_testGlobalMaterial.SetString( "" );
		} else {
			hackedView2.globalMaterial = mtr;
			hackedView2.forceUpdate = true;
			hackedView2.time = 0;
			gameRenderWorld->RenderScene( &hackedView2 );
		}
	}else{
		gameRenderWorld->RenderScene( &hackedView );
	}
	...
	bDrawHUD = !player->DrawFSGui( true );	//transparent guis will replace HUD

	// draw screen blobs
	if ( !pm_thirdPerson.GetBool() && !g_skipViewEffects.GetBool() ) {
		for ( int i = 0 ; i < MAX_SCREEN_BLOBS ; i++ ) {
			screenBlob_t	*blob = &screenBlobs[i];
			if ( blob->finishTime <= gameLocal.time || blob->material == NULL) {
				blob->material = NULL;			
				continue;
			}
		...
		if ( alpha < 1.0f  ) {
			renderSystem->SetColor4( ( health <= 0.0f ) ? MS2SEC( gameLocal.time ) : lastDamageTime, 1.0f, 1.0f, ( health <= 0.0f ) ? 0.0f : alpha );
	...
	if ( submergedMaterial ) {
		renderSystem->SetColor4( 1.0f, 1.0f, 1.0f, 1.0f );
		renderSystem->DrawStretchPic( 0.0f, 0.0f, 640.0f, 480.0f, 0.0f, 0.0f, 1.0f, 1.0f, submergedMaterial );
	}
	...
	if ( g_testPostProcess.GetString()[0] ) {
		...
		if ( !mtr ) {
			...
			renderSystem->SetColor4( 1.0f, 1.0f, 1.0f, 1.0f );
			renderSystem->DrawStretchPic( 0.0f, 0.0f, 640.0f, 480.0f, 0.0f, 1.0f, 1.0f, 0.0f, mtr );
		}
	}
}

void idPlayerView::SetLiquid( idLiquid* liq ) {
const char* m;
	if ( liq ) {
		m = liq->getSubmergedMtr();
		if (m) 
			submergedMaterial = declManager->FindMaterial( m, false );
	}else{
		submergedMaterial = NULL;
	}
}
...
void idPlayerView::InfluenceVision( idUserInterface *hud, const renderView_t *view ) {
	...
	if ( player->GetInfluenceMaterial() ) {
		...
		int offset =  (int)(25 + sin((float)gameLocal.time ));
