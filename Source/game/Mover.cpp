...
================
*/
void idMover::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idMover::Restore( idRestore_I *savefile ) {
	int i, num;
	...
	if ( hasSpline ) {
		...
		PostEventMS( &EV_PostRestore, NULL, 0, starttime, totaltime, accel, decel, useAngles );
	} 
}
...
void idMover::Spawn( void ) {
	...
	if ( spawnArgs.MatchPrefix( "guiTarget" ) ) {
		if ( gameLocal.GameState() == GAMESTATE_STARTUP ) {
			PostEventMS( &EV_FindGuiTargets, NULL, 0 );
		} else {
...
================
*/
void idMover::Event_SetCallback( idEventReturn *returnContext ) {
...
void idMover::BeginMove( idThread *thread ) {
	...
	ProcessEvent( &EV_ReachedPos, NULL );
}
...
void idMover::BeginRotation( idThread *thread, bool stopwhendone ) {
	...
	ProcessEvent( &EV_ReachedAng, NULL );
}
...
================
*/
void idMover::Event_IsMoving( idEventReturn *returnContext ) {
...
================
*/
void idMover::Event_IsRotating( idEventReturn *returnContext ) {
...
================
*/
void idElevator::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idElevator::Restore( idRestore_I *savefile ) {
	int i, num;
...
void idElevator::Spawn( void ) {
	...
	PostEventMS( &EV_Mover_InitGuiTargets, NULL, 0 );
	controlsDisabled = false;
}
...
void idElevator::Event_Touch( idEntity *other, trace_t *trace ) {
	...
	if ( spawnArgs.GetBool( "trigger" ) && triggerFloor != currentFloor ) {
		PostEventSec( &EV_GotoFloor, NULL, 0.25f, triggerFloor );
	}
}
...
===============
*/
bool idElevator::HandleSingleGuiCommand( idEntity *entityGui, idLexer *src, int eg_num ) {
	idToken token;
	...
	if ( token.Icmp( "changefloor" ) == 0 ) {
		if ( src->ReadToken( &token ) ) {
			...
			if ( newFloor == currentFloor ) {
				...
				idDoor *door = GetDoor( spawnArgs.GetString( "innerdoor" ) );
				if ( door && door->IsOpen() ) {
					PostEventSec( &EV_GotoFloor, NULL, 0.5f, newFloor );
				} else {
					ProcessEvent( &EV_GotoFloor, NULL, newFloor );
				}
			}
			return true;
		}
	...
	if ( fi ) {
		...
		if ( door ) {
			if ( door->IsBlocked() || door->IsOpen() ) {
				PostEventSec( &EV_GotoFloor, NULL, 0.5f, floor );
				return;
			}
		}
		DisableAllDoors();
...
void idElevator::DoneMoving( void ) {
	...
	if ( spawnArgs.GetInt( "pauseOnFloor", "-1" ) == currentFloor ) {
		PostEventSec( &EV_PostArrival, NULL, spawnArgs.GetFloat( "pauseTime" ) );
	} else {
...
================
*/
void idMover_Binary::Save( idSave_I *savefile ) const {
	int i;
...
================
*/
void idMover_Binary::Restore( idRestore_I *savefile ) {
	...
	if ( !team.Length() ) {
		...
		for( ent = gameLocal.spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
			if ( ent->IsType( idMover_Binary::Type ) ) {
				if ( !idStr::Icmp( static_cast<idMover_Binary *>(ent)->team.c_str(), temp ) ) {
					break;
				}
			}
		}
...
================
*/
void idMover_Binary::Event_SetCallback( idEventReturn *returnContext ) {
...
void idMover_Binary::Event_Reached_BinaryMover( void ) {
	...
	if ( moverState == MOVER_1TO2 ) {
		...
		if ( enabled && wait >= 0 && !spawnArgs.GetBool( "toggle" ) ) {
			...
			PostEventSec( &EV_Mover_ReturnToPos1, NULL, wait );
		}
		...
		// close areaportals
		if ( moveMaster == this ) {
			ProcessEvent( &EV_Mover_ClosePortal, NULL );
		}
...
void idMover_Binary::GotoPosition1( void ) {
	...
	if ( moverState == MOVER_POS2 ) {
		...
		if ( !spawnArgs.GetBool( "toggle" ) ) {
			ProcessEvent( &EV_Mover_ReturnToPos1, NULL );
		}
		return;
	}
...
void idMover_Binary::GotoPosition2( void ) {
	...
	if ( moverState == MOVER_POS1 ) {
		...
		// open areaportal
		ProcessEvent( &EV_Mover_OpenPortal, NULL );
		return;
	}
...
void idMover_Binary::Use_BinaryMover( idEntity *activator ) {
	...
	if ( moverState == MOVER_POS2 ) {
		...
		for ( slave = this; slave != NULL; slave = slave->activateChain ) {
			slave->CancelEvents( &EV_Mover_ReturnToPos1 );
			slave->PostEventSec( &EV_Mover_ReturnToPos1, NULL, spawnArgs.GetBool( "toggle" ) ? 0 : wait );
		}
		return;
	}
	...
	for ( idMover_Binary *slave = moveMaster; slave != NULL; slave = slave->activateChain ) {
		...
		if ( b ) {
			...
			while( kv ) {
				idEntity *ent = gameLocal.FindEntity( kv->GetValue() );
				if ( ent ) {
					ent->PostEventMS( &EV_Activate, NULL, 0, moveMaster->GetActivator() );
				}
...
================
*/
void idDoor::Save( idSave_I *savefile ) const {
...
================
*/
void idDoor::Restore( idRestore_I *savefile ) {
...
void idDoor::Spawn( void ) {
	...
	// c4tnt: movevec is new DLR feature
	if ( !spawnArgs.GetVector( "movevec", "0 0 0", movedir ) || movedir.LengthSqr() == 0.0f ) {
	...
	if ( start_open ) {
		...
		PostEventMS( &EV_Door_StartOpen, NULL, 1 );		
	}
	...
	if ( moveMaster == this ) {
		...
		if ( noTouch || health ) {
			...
			PostEventMS( &EV_Mover_MatchTeam, NULL, 0, moverState, gameLocal.time );

			const char *sndtemp = spawnArgs.GetString( "snd_locked" );
			if ( spawnArgs.GetInt( "locked" ) && sndtemp && *sndtemp ) {
				PostEventMS( &EV_Door_SpawnSoundTrigger, NULL, 0 );
			}
		} else {
			// spawn trigger
			PostEventMS( &EV_Door_SpawnDoorTrigger, NULL, 0 );
		}
	}
	...
	if ( locked ) {
		...
		PostEventMS( &EV_Door_Lock, NULL, 0, locked );
	}

	if ( spawnArgs.GetBool( "continuous" ) ) {
		PostEventSec( &EV_Activate, NULL, spawnArgs.GetFloat( "delay" ), this );
	}
		...
		if ( other->IsType( idDoor::Type ) ) {
			...
			if ( other == moveMaster ) {
				if ( door->sndTrigger == NULL ) {
					...
					if ( sndtemp && *sndtemp ) {
						door->PostEventMS( &EV_Door_SpawnSoundTrigger, NULL, 0 );
					}
				}
...
void idDoor::Event_TeamBlocked( idEntity *blockedEntity, idEntity *blockingEntity ) {
	...
	if ( companionDoor ) {
		companionDoor->ProcessEvent( &EV_TeamBlocked, NULL, blockedEntity, blockingEntity );
	}
}
...
================
*/
void idDoor::Event_IsOpen( idEventReturn *returnContext ) {
	bool state;

	state = IsOpen();
	returnContext->FinallyFloat( state );
...
================
*/
void idDoor::Event_Locked( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( spawnArgs.GetInt("locked") );
...
===============
*/
void idPlat::Save( idSave_I *savefile ) const {
	savefile->WriteClipModel( trigger );
	savefile->WriteVec3( localTriggerOrigin );
	savefile->WriteMat3( localTriggerAxis );
}
...
===============
*/
void idPlat::Restore( idRestore_I *savefile ) {
	savefile->ReadClipModel( trigger );
	savefile->ReadVec3( localTriggerOrigin );
	savefile->ReadMat3( localTriggerAxis );
}
...
===============
*/
void idMover_Periodic::Save( idSave_I *savefile ) const {
	savefile->WriteFloat( damage );
	savefile->WriteStaticObject( physicsObj );
}
...
===============
*/
void idMover_Periodic::Restore( idRestore_I *savefile ) {
	savefile->ReadFloat( damage );
	savefile->ReadStaticObject( physicsObj );
	RestorePhysics( &physicsObj );
}
...
void idRotater::Spawn( void ) {
	...
	if ( spawnArgs.GetBool( "start_on" ) ) {
		ProcessEvent( &EV_Activate, NULL, this );
	}
}
...
===============
*/
void idRotater::Save( idSave_I *savefile ) const {
	activatedBy.Save( savefile );
}
...
===============
*/
void idRotater::Restore( idRestore_I *savefile ) {
	activatedBy.Restore( savefile );
}
...
void idRotater::Event_Activate( idEntity *activator ) {
	...
	if ( !spawnArgs.GetBool( "rotate" ) ) {
		...
		spawnArgs.GetFloat( "speed", "100", speed );
		if ( spawnArgs.GetVector( "axial", "0 0 0", r_axis ) ) {
			r_axis.Normalize();
			r.SetOrigin( vec3_origin );
			r.SetVec( r_axis );
			r.SetAngle( speed );
			delta = r.ToAngles();
		} else {
