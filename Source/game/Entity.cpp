...
const idEventDef EV_Use( "<use>", "et" );							//c4tnt: USE key event, see player.cpp
...
const idEventDef EV_AddTarget( "addTarget", "E" );					//c4tnt: add new target to the entity
const idEventDef EV_Untarget( "untarget", "E" );					//remove target from the entity target list
...
const idEventDef EV_SetDirection( "setDirection", "vfd" );
...
const idEventDef EV_ClearKey( "clearKey", "s" );
...
const idEventDef EV_SetGuiParmDirect( "setGuiParmDirect", "dss" );
const idEventDef EV_SetGuiFloatDirect( "setGuiFloatDirect", "dsf" );
...
ABSTRACT_DECLARATION( idClass, idEntity )
	...
	EVENT( EV_Untarget,				idEntity::Event_Untarget )
	...
	EVENT( EV_SetDirection,			idEntity::Event_SetDirection )
	...
	EVENT( EV_ClearKey,				idEntity::Event_ClearKey )
	...
	EVENT( EV_SetGuiParmDirect,		idEntity::Event_SetGuiParmDirect )
	EVENT( EV_SetGuiFloatDirect,	idEntity::Event_SetGuiFloatDirect )
END_CLASS
...
void idEntity::UpdateChangeableSpawnArgs( const idDict *source ) {
	...
	if ( target && target[0] ) {
		...
		PostEventMS( &EV_UpdateCameraTarget, NULL, 0 );
	}
...
void idEntity::Spawn( void ) {
	...
	idVec3				origin;
	idMat3				axis;
	const idKeyValue	*networkSync, *HUBSync;
	...
	HUBSync = spawnArgs.FindKey( "HUBSync" );
	if ( HUBSync ) {
	}
	...
	if ( spawnArgs.MatchPrefix( "target" ) || spawnArgs.MatchPrefix( "guiTarget" ) ) {
		if ( gameLocal.GameState() == GAMESTATE_STARTUP ) {
			PostEventMS( &EV_FindTargets, NULL, 0 );
		} else {
	...
	if ( spawnArgs.GetString( "bind", "", &temp ) ) {
		PostEventMS( &EV_SpawnBind, NULL, 0 );
	}
...
================
*/
void idEntity::Save( idSave_I *savefile ) const {
	int i, j;
	...
	savefile->WriteBool( ( modelDefHandle != -1 ) );
...
================
*/
void idEntity::Restore( idRestore_I *savefile ) {
	int			i, j;
	int			num;
	idStr		funcname;
	bool		setupEntityDef;
	...
	savefile->ReadBool( setupEntityDef );
...
/*
==============
idEntity::GetAASLocation
==============
*/
void idEntity::GetAASLocation( idAAS *aas, idVec3 &pos, int &areaNum ) const {
	idVec3		size;
	idBounds	bounds;
...
================
*/
void idEntity::DebugHilight ( idVec4 &color, float lifetime = 0 ) const {

	int i, j, k;
	idVec4 unalphedColor;
	idVec3 pts[8];
	const idDrawVert *v;
	idWinding w;
	...
	unalphedColor = color;
	unalphedColor.w = 0.0f;

	if ( !renderEntity.hModel ) {
		// Draw colored BBOX
		for ( i = 0; i < 6; i++ ) {
			w.BoundPlaneAxis( renderEntity.bounds, renderEntity.origin, renderEntity.axis, i );
			gameRenderWorld->DebugPolygon( color, w, lifetime, true );
			gameRenderWorld->DebugWinding( unalphedColor, w, vec3_origin, mat3_identity, lifetime );
		}
		return;
	} else {
		// Draw colored model winding

		for ( i = 0; i < renderEntity.hModel->NumSurfaces(); i++ ) {
			surf = renderEntity.hModel->Surface( i );
			if ( surf->shader && surf->shader->IsDrawn() ) {
				for ( j = 0; j < surf->geometry->numIndexes; j += 3 ) {
					for ( k = 0; k < 3; k++ ) {
						v = &surf->geometry->verts[ surf->geometry->indexes[ j + k ] ];
						w.AddPointTransformed( v->xyz, renderEntity.origin, renderEntity.axis );
					}
					gameRenderWorld->DebugPolygon( color, w, lifetime, true );
					w.SetNumPoints( 0 );
				}
...
bool idEntity::RunPhysics( void ) {
	...
	if ( blockedPart ) {
		...
		Signal( SIG_BLOCKED );
		ProcessEvent( &EV_TeamBlocked, NULL, blockedPart, blockingEntity );
		// call the blocked function on the blocked part
		blockedPart->ProcessEvent( &EV_PartBlocked, NULL, blockingEntity );
		return false;
	}
	...
	for ( part = this; part != NULL; part = part->teamChain ) {
		...
		if ( part->physics ) {
			...
			if ( startTime < reachedTime && endTime >= reachedTime ) {
				part->ProcessEvent( &EV_ReachedPos, NULL );
			}
			reachedTime = part->physics->GetAngularEndTime();
			if ( startTime < reachedTime && endTime >= reachedTime ) {
				part->ProcessEvent( &EV_ReachedAng, NULL );
			}
		}
	}
...
================
*/
impactEffect_t idEntity::Collide( const trace_t &collision, const idVec3 &velocity ) {
...
================
*/
bool idEntity::HandleGuiCommands( idEntity *entityGui, const char *cmds, int eg_num ) {
	idEntity *targetEnt;
	bool ret = false;
	if ( entityGui && cmds && *cmds ) {
		...
		while( 1 ) {
			...
			if ( token.Icmp( "activate" ) == 0 ) {
				...
				if ( targets ) {
					...
					if ( ent ) {
						...
						ent->PostEventMS( &EV_Activate, NULL, 0, this );
					}
				}
			...
			if ( token.Icmp( "runScript" ) == 0 ) {
				idScriptContext sContext;
				sContext.Reset();
				if (! sContext.CompileParamL( src ) ) {
					gameLocal.Warning( "can't compile runScript field in gui for entity '%s'", entityGui->name.c_str() );
				}
				if ( sContext.Callable() ) {
					idThread *thread = new idThread( &sContext, entityGui->GetName() );
					thread->DelayedStart( 0 );
				}
				continue;
			}
			...
			if ( !token.Icmp( "turkeyscore" ) ) {
				if ( src.ReadToken( &token2 ) && entityGui->renderEntity.gui[eg_num] ) {
					int score = entityGui->renderEntity.gui[eg_num]->State().GetInt( "score" );
					score += atoi( token2 );
					entityGui->renderEntity.gui[eg_num]->SetStateInt( "score", score );
			...
			if ( !HandleSingleGuiCommand( entityGui, &src, eg_num ) ) {
				...
				for ( i = 0; i < c; i++) {
					...
					if ( targetEnt && targetEnt->HandleSingleGuiCommand( entityGui, &src, eg_num ) ) {
						break;
					}
				}
...
================
*/
bool idEntity::HandleSingleGuiCommand( idEntity *entityGui, idLexer *src, int eg_num ) {
	return false;
}

/*
================
idEntity::ProjectOntoGUI
================
*/
void idEntity::ProjectOntoGUI( idUserInterface *gui ) const {
	//Project gui_parm to
	if ( gui == NULL ) {
		return;
	}
	const idKeyValue *kv = spawnArgs.MatchPrefix( "gui_parm", NULL );
	while( kv ) {
		gui->SetStateString( kv->GetKey(), kv->GetValue() );
...
/*
==============================
idEntity::ActivateGuis

==============================
*/
void idEntity::ActivateGUIS( ) const {
	int	j;

	for ( j = 0; j < MAX_RENDERENTITY_GUI; j++ ) {
		if ( renderEntity.gui[ j ] ) {
			renderEntity.gui[ j ]->Trigger( gameLocal.time );
		}
	}
}
...
void idEntity::Get3DSpotPosition( idVec3& org ) {
	org = GetPhysics()->GetOrigin() + idVec3(0,0,spotGUIElevation);	
};

idUserInterface* idEntity::GetSpotGUI( void ) const {
	return spotGUI;
};

bool idEntity::SetSpotGUI( const char* guiName, float elev ) {

	if ( !guiName || !guiName[0] ) {
		spotGUI = NULL;
		return true;
	}
...
==============================
*/
idUserInterface* idEntity::FindGUI( int index ) const {

	if ( index < 0 || index >= MAX_RENDERENTITY_GUI )
		return NULL;

	return renderEntity.gui[ index ];
}
...
bool idEntity::TouchTriggers( void ) const {
	...
	for ( i = 0; i < numClipModels; i++ ) {
		...
		ent->ProcessEvent( &EV_Touch, NULL, this, &trace );
...
================
*/
void idEntity::Event_GetName( idEventReturn *returnContext ) {
	returnContext->FinallyString( name.c_str() );
...
================
*/
void idEntity::Event_NumTargets( idEventReturn *returnContext ) {
	returnContext->FinallyFloat( targets.Num() );
}
...
================
*/
void idEntity::Event_GetTarget( idEventReturn *returnContext, float index ) {
	int i;

	i = ( int )index;
	if ( ( i < 0 ) || i >= targets.Num() ) {
		returnContext->FinallyEntity( NULL );
	} else {
		returnContext->FinallyEntity( targets[ i ].GetEntity() );
	}
...
/*
================
idEntity::Event_AddTarget
================
*/
void idEntity::Event_AddTarget( idEntity *newtarget ) {
	int i;
	...
	idEntityPtr<idEntity> &e_ptr = targets.Alloc();
	e_ptr = newtarget;
...
/*
================
idEntity::Event_Untarget
================
*/
void idEntity::Event_Untarget( idEntity *target ) {
	int i;
	...
	for( i = targets.Num() - 1; i >= 0; i-- ) {
		if ( !targets[ i ].GetEntity() || targets[ i ] == target ) {
			targets.RemoveIndex( i );
		}
	}
}
...
================
*/
void idEntity::Event_RandomTarget( idEventReturn *returnContext, const char *ignore ) {
	int			num;
	idEntity	*ent;
	int			i;
	int			ignoreNum;
...
================
*/
void idEntity::Event_GetShaderParm( idEventReturn *returnContext, int parmnum ) {
	if ( ( parmnum < 0 ) || ( parmnum >= MAX_ENTITY_SHADER_PARMS ) ) {
		gameLocal.Error( "shader parm index (%d) out of range", parmnum );
	}

	returnContext->FinallyFloat( renderEntity.shaderParms[ parmnum ] );
...
================
*/
void idEntity::Event_GetColor( idEventReturn *returnContext ) {
	idVec3 out;

	GetColor( out );
	returnContext->FinallyVector( out );
...
================
*/
void idEntity::Event_IsHidden( idEventReturn *returnContext ) {
	returnContext->FinallyInt( fl.hidden );
...
================
*/
void idEntity::Event_StartSoundShader( idEventReturn *returnContext, const char *soundName, int channel ) {
	int length;

	StartSoundShader( declManager->FindSound( soundName ), (s_channelType)channel, 0, false, &length );
	returnContext->FinallyFloat( MS2SEC( length ) );
...
================
*/
void idEntity::Event_StartSound( idEventReturn *returnContext, const char *soundName, int channel, int netSync ) {
	int time;
	
	StartSound( soundName, ( s_channelType )channel, 0, ( netSync != 0 ), &time );
	returnContext->FinallyFloat( MS2SEC( time ) );
...
================
*/
void idEntity::Event_GetWorldOrigin( idEventReturn *returnContext ) {
	returnContext->FinallyVector( GetPhysics()->GetOrigin() );
...
================
*/
void idEntity::Event_GetOrigin( idEventReturn *returnContext ) {
	returnContext->FinallyVector( GetLocalCoordinates( GetPhysics()->GetOrigin() ) );
...
/*
================
idEntity::Event_SetDirection
================
*/
void idEntity::Event_SetDirection( idVec3 const &Dir, float relAngle, int ordConst ) {
	idMat3 axis;

	gameLocal.AxisAlongDir( Dir, relAngle, axis, ordConst );
	SetAxis( axis );
}
...
================
*/
void idEntity::Event_GetAngles( idEventReturn *returnContext ) {
...
================
*/
void idEntity::Event_GetLinearVelocity( idEventReturn *returnContext ) {
	returnContext->FinallyVector( GetPhysics()->GetLinearVelocity() );
...
================
*/
void idEntity::Event_GetAngularVelocity( idEventReturn *returnContext ) {
	returnContext->FinallyVector( GetPhysics()->GetAngularVelocity() );
...
================
*/
void idEntity::Event_GetSize( idEventReturn *returnContext ) {
	idBounds bounds;

	bounds = GetPhysics()->GetBounds();
	returnContext->FinallyVector( bounds[1] - bounds[0] );
...
================
*/
void idEntity::Event_GetMins( idEventReturn *returnContext ) {
	returnContext->FinallyVector( GetPhysics()->GetBounds()[0] );
...
================
*/
void idEntity::Event_GetMaxs( idEventReturn *returnContext ) {
	returnContext->FinallyVector( GetPhysics()->GetBounds()[1] );
...
================
*/
void idEntity::Event_Touches( idEventReturn *returnContext, idEntity *ent ) {
	if ( !ent ) {
	...
	returnContext->FinallyInt( myBounds.IntersectsBounds( entBounds ) );
...
/*
================
idEntity::Event_SetGuiParmDirect
================
*/
void idEntity::Event_SetGuiParmDirect( int index, const char *key, const char *val ) {
	if ( index > 0 && index <= MAX_RENDERENTITY_GUI && renderEntity.gui[ index ] ) {
		renderEntity.gui[ index-1 ]->SetStateString( key, val );
		renderEntity.gui[ index-1 ]->StateChanged( gameLocal.time );
	}
}

/*
================
idEntity::Event_SetGuiFloatDirect
================
*/
void idEntity::Event_SetGuiFloatDirect( int index, const char *key, float f ) {
	if ( index > 0 && index <= MAX_RENDERENTITY_GUI && renderEntity.gui[ index ] ) {
		renderEntity.gui[ index-1 ]->SetStateString( key, va( "%f", f ) );
		renderEntity.gui[ index-1 ]->StateChanged( gameLocal.time );
	}
}
...
================
*/
void idEntity::Event_GetNextKey( idEventReturn *returnContext, const char *prefix, const char *lastMatch ) {
	const idKeyValue *kv;
	...
	if ( !kv ) {
		...
		returnContext->FinallyString( kv->GetKey() );
	}
	
}

void idEntity::Event_ClearKey( const char *key ) {
...
================
*/
void idEntity::Event_GetKey( idEventReturn *returnContext, const char *key ) {
	const char *value;

	spawnArgs.GetString( key, "", &value );
	returnContext->FinallyString( value );
}
...
================
*/
void idEntity::Event_GetIntKey( idEventReturn *returnContext, const char *key ) {
	int value;

	spawnArgs.GetInt( key, "0", value );

	// scripts only support floats
	returnContext->FinallyFloat( value );
...
================
*/
void idEntity::Event_GetFloatKey( idEventReturn *returnContext, const char *key ) {
	float value;
...
================
*/
void idEntity::Event_GetVectorKey( idEventReturn *returnContext, const char *key ) {
	idVec3 value;

	spawnArgs.GetVector( key, "0 0 0", value );
	returnContext->FinallyVector( value );
...
================
*/
void idEntity::Event_GetEntityKey( idEventReturn *returnContext, const char *key ) {
	idEntity *ent;
	const char *entname;
	...
	if ( cameraTarget ) {
		...
		while( kv ) {
			...
			if ( ent && idStr::Icmp( ent->GetEntityDefName(), "target_null" ) == 0) {
				...
				//SetAxis(dir.ToMat3());	//c4tnt:For what???? Fixed.
				break;						
			}
...
================
*/
void idEntity::Event_DistanceTo( idEventReturn *returnContext, idEntity *ent ) {
	if ( !ent ) {
		// just say it's really far away
		returnContext->FinallyFloat( MAX_WORLD_SIZE );
	} else {
		float dist = ( GetPhysics()->GetOrigin() - ent->GetPhysics()->GetOrigin() ).Length(); // .LengthFast(); //c4tnt: more precious for scripts
		returnContext->FinallyFloat( dist );
	}
...
================
*/
void idEntity::Event_DistanceToPoint( idEventReturn *returnContext, const idVec3 &point ) {
	float dist = ( GetPhysics()->GetOrigin() - point ).Length(); // .LengthFast(); //c4tnt: more precious for scripts
	returnContext->FinallyFloat( dist );
...
=====================
*/
void idEntity::Event_HasFunction( idEventReturn *returnContext, const char *name ) {
...
================
*/
void idEntity::Event_GetMass( idEventReturn *returnContext, int id ) {
	returnContext->FinallyFloat(physics->GetMass(id));
...
================
*/
void idEntity::Event_IsInLiquid( idEventReturn *returnContext ) {
	returnContext->FinallyInt(physics->GetWater() != NULL);
...
================
* Allows guis to be changed at runtime. Guis that are
...
================
* Forces the engine to initialize a gui even if it is not specified as used in a level.
...
void idEntity::Event_GetGuiParm( idEventReturn *returnContext, int guiNum, const char *key ) {
	if ( guiNum > 0 && guiNum <= MAX_RENDERENTITY_GUI && renderEntity.gui[guiNum-1] ) {
#if API_VERS > 3
		returnContext->FinallyString(renderEntity.gui[guiNum-1]->GetStateString(key));
#else
		returnContext->FinallyString(renderEntity.gui[guiNum-1]->State().GetString(key, ""));
#endif 
		return;
	}
	returnContext->FinallyString("");
...
void idEntity::Event_GetGuiParmFloat( idEventReturn *returnContext, int guiNum, const char *key ) {
	if( guiNum > 0 && guiNum <= MAX_RENDERENTITY_GUI && renderEntity.gui[guiNum-1] ) {
#if API_VERS > 3
		returnContext->FinallyFloat(renderEntity.gui[guiNum-1]->GetStateFloat(key));
#else
		returnContext->FinallyFloat(renderEntity.gui[guiNum-1]->State().GetFloat(key, "0"));
#endif 
		return;
	}
...
void idEntity::WriteGUIToSnapshot( idBitMsgDelta &msg ) const {
	...
	for ( i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
		if ( renderEntity.gui[ i ] ) {
			msg.WriteByte( renderEntity.gui[ i ]->State().GetInt( "networkState" ) );
		} else {
			msg.WriteByte( 0 );
		}
	}
...
/*
================
idEntity::GetUseDescr

returns the orientation of the joint in worldspace
================
*/

bool idEntity::ConfirmUseDescr( use_descr_t* Desc )
{
	if (!spawnArgs.GetBool( "use_enable", "0"))
		return false;	// Not useable

	Desc->CursorType = CURTYPE_USE;
	Desc->ent = this;
	Desc->ShowInCursor = spawnArgs.GetBool( "use_cursor", "1");
	Desc->ShowInHud = spawnArgs.GetBool( "use_hud", "1");
	Desc->LowerWeapon = spawnArgs.GetBool( "use_hideweapon", "1");
	spawnArgs.GetString( "use_act", "", Desc->strAct);
	spawnArgs.GetString( "use_obj", "", Desc->strObj);
	Desc->subIDX = spawnArgs.GetInt( "use_idx", "0");

	return true;
}
/*
================
idEntity::GUISetUseData

returns the orientation of the joint in worldspace
================
*/

void idEntity::GUISetUseData( idUserInterface * hud )
{
	// Nothing changes
}
...
================
*/
void idAnimatedEntity::Save( idSave_I *savefile ) const {
...
================
*/
void idAnimatedEntity::Restore( idRestore_I *savefile ) {
...
================
*/
void idAnimatedEntity::Event_GetJointHandle( idEventReturn *returnContext, const char *jointname ) {
	jointHandle_t joint;

	joint = animator.GetJointHandle( jointname );
	returnContext->FinallyInt( joint );
...
================
*/
void idAnimatedEntity::Event_GetJointPos( idEventReturn *returnContext, jointHandle_t jointnum ) {
	idVec3 offset;
	idMat3 axis;
	...
	returnContext->FinallyVector( offset );
...
================
*/
void idAnimatedEntity::Event_GetJointAngle( idEventReturn *returnContext, jointHandle_t jointnum ) {
	idVec3 offset;
	idMat3 axis;
...
void ClearUseDescr( use_descr_t &c ) {
	c.CursorType = CURTYPE_NONE;
	c.ent = NULL;
	c.LowerWeapon = false;
	c.ShowInCursor = false;
	c.ShowInHud = false;
	c.strAct.Clear();
	c.strObj.Clear();
	c.subIDX = 0;
