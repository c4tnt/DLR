...
idCVar net_clientShowSnapshot( "net_clientShowSnapshot", "0", CVAR_SYSTEM | CVAR_NOCHEAT | CVAR_INTEGER, "", 0, 3, idCmdSystem::ArgCompletion_Integer<0,3> );
idCVar net_clientShowSnapshotRadius( "net_clientShowSnapshotRadius", "128", CVAR_SYSTEM | CVAR_NOCHEAT | CVAR_FLOAT, "" );
...
void idGameLocal::ServerClientBegin( int clientNum ) {
	...
	DPrintf( " Server->Client handshake [%d] \n", clientNum );
	...
	DPrintf( " Server->Client handshake OK [%d] \n", clientNum );
...
void idGameLocal::ServerWriteSnapshot( int clientNum, int sequence, idBitMsg &msg, byte *clientInPVS, int numPVSClients ) {
	...
	if ( net_clientShowSnapshot.GetInteger() > 0 ) {
		//c4tnt: Server show snaphot
		int bytes = msg.GetNumBitsWritten() >> 3;

		if ( bytes > (2 << 20) ) {
			gameLocal.Printf( "Snapshot (%d) for client (%d) = %d Mb of %d MB\n", sequence, clientNum, bytes >> 20, overall >> 20 );
		} else if ( bytes > (2 << 10) ) {
			gameLocal.Printf( "Snapshot (%d) for client (%d) = %d Kb of %d Kb\n", sequence, clientNum, bytes >> 10, overall >> 10 );
		} else {
			gameLocal.Printf( "Snapshot (%d) for client (%d) = %d bytes of %d bytes\n", sequence, clientNum, bytes, overall );
		}
	}
}
	...
	for ( i = msg.ReadBits( GENTITYNUM_BITS ); i != ENTITYNUM_NONE; i = msg.ReadBits( GENTITYNUM_BITS ) ) {
		...
		if ( !ent || ent->GetType()->typeNum != typeNum || ent->entityDefNumber != entityDefNumber || spawnId != spawnIds[ i ] ) {
			...
			if ( entityDefNumber >= 0 ) {
				...
				//common->Warning( "Entity from server: %s (%d)", classname, spawnId );
				...
				//common->Warning( "End entity from server." );
			} else {
				//common->Warning( "Entity from server(TI) %d ", spawnId );
				ent = SpawnEntityType( *typeInfo, &args, true );
				//common->Warning( "classname: %s", typeInfo->classname, spawnId );
...
//	common->Warning("ClientProcessReliableMessage");
	...
	switch( id ) {
		...
		case GAME_RELIABLE_MESSAGE_TCHAT: { // (client should never get a TCHAT though)
			...
			msg.ReadString( name, sizeof( name ) );
			msg.ReadString( text, sizeof( text ) );
			// c4tnt: prevent client crash on long strings
			name[127] = 0;
			text[127] = 0;
			mpGame.AddChatLine( "%s^0: %s\n", name, text );
			break;
		...
		case GAME_RELIABLE_MESSAGE_SPAWNID_COLLAPSE: {
			// c4tnt: added to remove spawnid limitation
			// 1. clear entities with spawnids btw SpawnBoundLow and SpawnBoundHigh
			// 2. map spawnid tail to the freed area
			// 3. decreases spawnid tail to new spawnid tail value and remove entities with a spawnids values more than new tail value
			int freeSpawnBoundLow = msg.ReadBits(32);
			int freeSpawnBoundHigh = msg.ReadBits(32);
			int NewSpawnid = msg.ReadBits(32);
		}													 
		case GAME_RELIABLE_MESSAGE_HUDMSG: {
			// c4tnt: client recieves hudmessage
			clientHMSG_t loadMsg;
			break;
		}													 
		default: {
...
//	common->Warning("ClientProcessReliableMessage OK");
}
...
//	gameLocal.Warning( "Client Prediction" );

	ret.sessionCommand[ 0 ] = '\0';

	player = static_cast<idPlayer *>( entities[clientNum] );
	if ( !player ) {
//		gameLocal.Warning( "Client Prediction OK" );
		return ret;
	}
