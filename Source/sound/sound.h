...
public:
	int						getNumLeadins() const { return numLeadins; }
	int						getNumEntries() const { return numEntries; }
	const char *			getLeadingName( int index ) const { if ( index >= 0 && index < numLeadins && leadins[index]) return leadins[index]->fileName; else return ""; }
	const char *			getEntryName( int index ) const { if ( index >= 0 && index < numEntries && entries[index]) return entries[index]->fileName; else return ""; }
};
...
typedef struct sound_channel_s {
	bool enabled;
	int skip[2];					// start time?
	soundShaderParms_t params;
	idSoundSample* soundSample;
	int channel;
	idSoundShader* currentShader;
	char skip2[52];
} sound_channel_t;
