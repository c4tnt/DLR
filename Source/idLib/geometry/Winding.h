...
public:
					...
					explicit idWinding( const idBounds &b, int side );
	...
	void			BoundPlane( const idBounds &b, int side );
	void			BoundPlaneAxis( const idBounds &b, const idVec3 &origin, const idMat3 &axis, int side );
	...
	void			AddPointTransformed( const idVec3 &v, const idVec3 &origin, const idMat3 &axis  );
...
ID_INLINE idWinding::idWinding( const idBounds &b, int side ) {
	BoundPlane( b, side );
}
...
ID_INLINE void idWinding::AddPointTransformed( const idVec3 &v, const idVec3 &origin, const idMat3 &axis  ) {
	if ( !EnsureAlloced(numPoints+1, true) ) {
		return;
	}
	p[numPoints].ToVec3() = v * axis + origin;
	numPoints++;
}
