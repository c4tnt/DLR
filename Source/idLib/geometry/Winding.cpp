...
/*
=============
idWinding::BoundPlane
=============
*/
void idWinding::BoundPlane( const idBounds &b, int side ) {
	int i;
	...
	p[1][(side>>1)%3] = b[(~side)&1][(side>>1)%3];
	p[2][(side>>1)%3] = b[(~side)&1][(side>>1)%3];
	p[2][((side>>1)+1)%3] = b[(~side)&1][((side>>1)+1)%3];
	p[3][(side>>1)%3] = b[side&1][(side>>1)%3];
}

/*
=============
idWinding::BoundPlaneAxis
=============
*/
void idWinding::BoundPlaneAxis( const idBounds &b, const idVec3 &origin, const idMat3 &axis, int side ) {
	int i;
	idVec3	_bpts[2];
	...
	if ( side & 1) {
		p[3][(side>>1)%3] = _bpts[1][(side>>1)%3];
		p[2][(side>>1)%3] = _bpts[1][(side>>1)%3];
		p[2][((side>>1)+1)%3] = _bpts[1][((side>>1)+1)%3];
		p[1][((side>>1)+1)%3] = _bpts[1][((side>>1)+1)%3];
	} else {
		p[1][(side>>1)%3] = _bpts[1][(side>>1)%3];
		p[2][(side>>1)%3] = _bpts[1][(side>>1)%3];
		p[2][((side>>1)+1)%3] = _bpts[1][((side>>1)+1)%3];
		p[3][((side>>1)+1)%3] = _bpts[1][((side>>1)+1)%3];
	}
	for ( i = 0; i < 4; i++ ) {
		p[i].ToVec3() *= axis;
		p[i].ToVec3() += origin;
