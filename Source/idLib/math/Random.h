...
public:
	...
	bool				Chance( float chance );		// random boolean for randomized events ( chance - from 0 to 1 )
...
ID_INLINE bool idRandom::Chance( float chance ) {
	seed = 69069 * seed + 1;
	return ( ( seed & idRandom::MAX_RAND ) < (int)(idRandom::MAX_RAND * chance) );
}
...
ID_INLINE int idRandom::RandomInt( int max ) {
	...
	return ( seed & idRandom::MAX_RAND ) % max;
}
