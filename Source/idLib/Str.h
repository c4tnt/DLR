...
public:
	...
	const char *		c_str( int start ) const;
	...
	int					fCmp( const char *text ) const;
	int					Cmpf( const char *filter ) const;
	int					Cmpn( const char *text, int n ) const;
	int					CmpPrefix( const char *text ) const;

						// case insensitive compare
	int					Icmp( const char *text ) const;
	int					Icmpf( const char *text ) const;
	...
	static int			Cmpf( const char *s, const char *f );
	static int			Cmp( const char *s1, const char *s2 );
	static int			Cmpn( const char *s1, const char *s2, int n );
	static int			Icmp( const char *s1, const char *s2 );
	static int			Icmpf( const char *s1, const char *s2 );
...
ID_INLINE const char *idStr::c_str( int start ) const {
	if ( start < len ) {
		return &data[ start ];
	} else {
		return &data[ len ];
...
ID_INLINE int idStr::fCmp( const char *text ) const {
	assert( text );
	return idStr::Cmpf( text, data );
}

ID_INLINE int idStr::Cmpf( const char *filter ) const {
	assert( filter );
	return idStr::Cmpf( data, filter );
}
...
ID_INLINE int idStr::Icmpf( const char *text ) const {
	assert( text );
	return idStr::Icmpf( data, text );
}
