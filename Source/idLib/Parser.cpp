...
void idParser::PushScript( idLexer *script ) {
	...
	//Make compatible
	script->CopyPunctuations( idParser::scriptstack );
...
/*
================
idParser::Immediate_include
================
*/
bool idParser::Immediate_include( const char* c_path, bool relative ) {
	idLexer *script;
	idToken token;
	idStr path;
...
//	idParser::punctuations = p; //c4tnt:discarded
}
...
int idParser::LoadFile( const char *filename, bool OSPath ) {
	...
	script->SetFlags( idParser::flags );
	script->SetPunctuations( NULL );	//c4tnt: temporary defaulted
//	script->CopyPunctuations( idParser::scriptstack );
	script->next = NULL;
...
int idParser::LoadMemory(const char *ptr, int length, const char *name ) {
	...
	script->SetFlags( idParser::flags );
	script->SetPunctuations( NULL ); //Defaulted
	script->next = NULL;
...
//	int i;
//	c4tnt: all commented code is depricated

//	if ( !idParser::punctuations ) {
	if ( idParser::scriptstack ) {
//		idLexer lex;
		return idParser::scriptstack->GetPunctuationFromId( id );
	}
...
//	if ( !idParser::punctuations ) {
	if ( idParser::scriptstack ) {
//		idLexer lex;
		return idParser::scriptstack->GetPunctuationId( p );
	}
