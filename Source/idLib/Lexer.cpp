...
/*
================
idLexer::CreatePFlagTable
================
*/
void idLexer::CreatePFlagTable( ) {
int i;

	if (p_flags) {
		Mem_Free( p_flags );	//Remove old
	}

	if (punctuations) {
		i = 0;
		while (punctuations[i].p) {
			i++;
		}
		p_flags = (int*) Mem_Alloc( i * sizeof(int) );
		memset(p_flags, 0, i * sizeof(int)); //zeroes
	}else{
		p_flags = NULL;
	}
}
...
/*
================
idLexer::CopyPunctuations
================
*/
void idLexer::CopyPunctuations( const idLexer *lex ) {
	if (!lex)
		return;

#ifdef PUNCTABLE
	if (lex->punctuations) {
		idLexer::CreatePunctuationTable( lex->punctuations );
	}
	else {
		idLexer::CreatePunctuationTable( default_punctuations );
	}
#endif //PUNCTABLE
	if (lex->punctuations) {
		idLexer::punctuations = lex->punctuations;
	}
	else {
		idLexer::punctuations = default_punctuations;
	}
